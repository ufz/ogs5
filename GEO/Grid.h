...
private:
	...
	bool calcNearestPointInGridCell(double const* const pnt, size_t const* const coords, double& sqr_min_dist,
    ...
    : GEOLIB::AABB(), _grid_quad_to_node_map(NULL)
{
		...
		else
		{
			...
			else
			{
				...
				else
				{
					...
					{
						_n_steps[0]
						    = static_cast<size_t>(ceil(sqrt(n_pnts * delta[0] / (max_num_per_grid_cell * delta[2]))));
						_n_steps[1] = 1;
					...
					{
						_n_steps[0]
						    = static_cast<size_t>(ceil(sqrt(n_pnts * delta[0] / (max_num_per_grid_cell * delta[1]))));
	...
	else
	{
		...
		_n_steps[0] = static_cast<size_t>(
		    ceil(pow(n_pnts * delta[0] * delta[0] / (max_num_per_grid_cell * delta[1] * delta[2]), 1. / 3.)));
		_n_steps[1] = static_cast<size_t>(ceil(_n_steps[0] * delta[1] / delta[0]));
		_n_steps[2] = static_cast<size_t>(ceil(_n_steps[0] * delta[2] / delta[0]));
	}
...
POINT const* Grid<POINT>::getNearestPoint(double const* const pnt) const
{
	...
	if (calcNearestPointInGridCell(pnt, coords, sqr_min_dist, nearest_pnt))
	{
		...
		if (dists[0] >= min_dist && dists[1] >= min_dist && dists[2] >= min_dist && dists[3] >= min_dist
		    && dists[4] >= min_dist
		    && dists[5] >= min_dist)
		{
			return nearest_pnt;
	...
	for (coords[0] = min_coords[0]; coords[0] < max_coords[0] + 1; coords[0]++)
	{
		for (coords[1] = min_coords[1]; coords[1] < max_coords[1] + 1; coords[1]++)
		{
			...
			for (coords[2] = min_coords[2]; coords[2] < max_coords[2] + 1; coords[2]++)
			{
				// copy pnts
				const size_t n_pnts(
				    _grid_quad_to_node_map[coords0_p_coords1_x_steps0 + coords[2] * steps0_x_steps1].size());
				for (size_t k(0); k < n_pnts; k++)
				{
	...
	for (size_t i(0); i < _n_steps[0]; i++)
	{
		for (size_t j(0); j < _n_steps[1]; j++)
		{
			for (size_t k(0); k < _n_steps[2]; k++)
			{
				...
				points->push_back(
				    new GEOLIB::Point(llf[0] + (i + 1) * dx, llf[1] + (j + 1) * dy, llf[2] + (k + 1) * dz));
...
template <typename POINT>
bool Grid<POINT>::calcNearestPointInGridCell(double const* const pnt, size_t const* const coords, double& sqr_min_dist,
                                             POINT*& nearest_pnt) const
{
...
template <typename POINT>
void Grid<POINT>::getPointCellBorderDistances(double const* const pnt, double dists[6],
