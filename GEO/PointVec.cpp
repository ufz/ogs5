...
PointVec::PointVec(const std::string& name, std::vector<Point*>* points, std::map<std::string, size_t>* name_id_map,
    ...
    : TemplateVec<Point>(name, points, name_id_map), _type(type), _sqr_shortest_dist(std::numeric_limits<double>::max())
{
	assert(_data_vec);
...
size_t PointVec::uniqueInsert(Point* pnt)
{
	...
	for (k = 0; k < n; k++)
		if (fabs((*((*_data_vec)[k]))[0] - (*pnt)[0]) < eps && fabs((*((*_data_vec)[k]))[1] - (*pnt)[1]) < eps
...
void PointVec::makePntsUnique(std::vector<GEOLIB::Point*>* pnt_vec, std::vector<size_t>& pnt_id_map, double eps)
{
