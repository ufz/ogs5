...
bool Polygon::isPntInPolygon(GEOLIB::Point const& pnt) const
{
	...
	{
		for (std::list<Polygon*>::const_iterator it(_simple_polygon_list.begin()); it != _simple_polygon_list.end();
		     ++it)
		{
...
bool Polygon::isPartOfPolylineInPolygon(const Polyline& ply) const
{
	...
	for (size_t k(0); k < ply_size - 1; k++)
	{
		...
		{
			if (MathLib::lineSegmentIntersect(*(getPoint(j)), *(getPoint(j + 1)), *(ply.getPoint(k)),
			                                  *(ply.getPoint(k + 1)), *s))
			{
				delete s;
				return true;
...
bool Polygon::getNextIntersectionPointPolygonLine(GEOLIB::Point const& a, GEOLIB::Point const& b,
                                                  GEOLIB::Point* intersection_pnt, size_t& seg_num) const
{
	...
	else
	{
		     ...
		     ++it)
		{
			...
			{
				if (MathLib::lineSegmentIntersect(*(polygon->getPoint(k)), *(polygon->getPoint(k + 1)), a, b,
				                                  *intersection_pnt))
				{
					seg_num = k;
					return true;
...
void Polygon::ensureCWOrientation()
{
	...
	for (size_t k(0); k < n_pnts; k++)
		if ((*(tmp_polygon_pnts[k]))[0] <= (*(tmp_polygon_pnts[min_x_max_y_idx]))[0])
		{
			if ((*(tmp_polygon_pnts[k]))[0] < (*(tmp_polygon_pnts[min_x_max_y_idx]))[0])
				min_x_max_y_idx = k;
			else if ((*(tmp_polygon_pnts[k]))[1] > (*(tmp_polygon_pnts[min_x_max_y_idx]))[1])
				min_x_max_y_idx = k;
		}
	...
	else
	{
		if (0 == min_x_max_y_idx)
			orient = MathLib::getOrientation(tmp_polygon_pnts[n_pnts - 1], tmp_polygon_pnts[0], tmp_polygon_pnts[1]);
		else
			orient = MathLib::getOrientation(tmp_polygon_pnts[n_pnts - 2], tmp_polygon_pnts[n_pnts - 1],
			                                 tmp_polygon_pnts[0]);
	}
...
void Polygon::splitPolygonAtIntersection(std::list<Polygon*>::iterator polygon_it)
{
	...
	while (polygon_it != _simple_polygon_list.end())
	{
		...
		if (!is_simple)
		{
			...
			{
				std::cerr << "ERROR in Polygon::splitPolygonAtIntersection polygon0"
				          << "\n";
				exit(1);
			}
			...
			{
				std::cerr << "ERROR in Polygon::splitPolygonAtIntersection polygon1"
				          << "\n";
				exit(1);
			}
...
bool operator==(Polygon const& lhs, Polygon const& rhs)
{
	...
	else
	{
		...
		// *** ATTENTION
		std::cerr << "operator==(Polygon const& lhs, Polygon const& rhs) - not tested case (implementation is probably "
		             "buggy) - please contact thomas.fischer@ufz.de mentioning the problem"
		          << "\n";
		// in second polygon
		if (lhs.getPointID(1) == rhs.getPointID(k - 1))
		{
