...
void Station::addProperty(std::string pname,
                          double (*getFct)(void*),
...
Station* Station::createStation(const std::string& line)
{
    ...
    if (fields.size() >= 3)
    {
        ...
        station->_name = *it;
        (*station)[0] =
            strtod((replaceString(",", ".", *(++it))).c_str(), NULL);
        (*station)[1] =
            strtod((replaceString(",", ".", *(++it))).c_str(), NULL);
        if (++it != fields.end())
            (*station)[2] =
    ...
    for (size_t i = 0; i < bounds.size(); i++)
    {
        for (size_t j = 0; j < _properties.size(); j++)
            if (_properties[j].name.compare(bounds[i].getName()) == 0)
            {
                ...
                value = (*get)((void*)this);
                if (!(value >= bounds[i].getMin() &&
                      value <= bounds[i].getMax()))
                    return false;
            }
    }
    return true;
    ...
    : Station(x, y, z), _zCoord(0), _depth(0), _date(0)
{
    ...
    addProperty(
        "depth", &StationBorehole::getDepth, &StationBorehole::setDepth);
...
int StationBorehole::readStratigraphyFile(
    const std::string& path, std::vector<std::list<std::string> >& data)
{
    std::string line;
    ...
    if (!in.is_open())
    {
        std::cout << "StationBorehole::readStratigraphyFile() - Could not open "
                     "file..."
                  << "\n";
        return 0;
    }
...
int StationBorehole::addStratigraphy(const std::string& path,
                                     StationBorehole* borehole)
{
    ...
    if (readStratigraphyFile(path, data))
    {
        ...
        //	for (size_t i=0; i<size; i++)
        //	{
        //		if ((borehole->_soilLayerThickness[i] == -1)
        //||(borehole->_soilName[i].compare("") == 0))
        //		{
...
int StationBorehole::addLayer(std::list<std::string> fields,
                              StationBorehole* borehole)
{
    if (fields.size() >=
        4) /* check if there are enough fields to create a borehole object */
    {
        if (fields.front().compare(borehole->_name) ==
            0) /* check if the name of the borehole matches the name in the data
                */
        {
            fields.pop_front();
            ...
            std::cerr << "StationBorehole::addLayer - assuming correct order"
                      << "\n";
            double thickness(
                strtod(replaceString(",", ".", fields.front()).c_str(), 0));
            fields.pop_front();
...
int StationBorehole::addStratigraphy(const std::vector<GEOLIB::Point*>& profile,
                                         ...
                                         soil_names)
{
    ...
    std::cout << "Error in StationBorehole::addStratigraphy() - Length of "
                 "parameter vectors does not match."
              << "\n";
    return 0;
}

int StationBorehole::addStratigraphies(const std::string& path,
    ...
    if (readStratigraphyFile(path, data))
    {
        ...
        for (size_t i = 0; i < nBoreholes; i++)
        {
            ...
            if (fields.size() >= 4)
            {
                ...
                fields.pop_front();
                static_cast<StationBorehole*>((*boreholes)[it])
                    ->addSoilLayer(thickness, soil_name);
            }
            else
                std::cout << "StationBorehole::addStratigraphies() - "
...
StationBorehole* StationBorehole::createStation(const std::string& line)
{
    ...
    if (fields.size() >= 5)
    {
        ...
        fields.pop_front();
        (*borehole)[0] =
            strtod((replaceString(",", ".", fields.front())).c_str(), NULL);
        fields.pop_front();
        (*borehole)[1] =
            strtod((replaceString(",", ".", fields.front())).c_str(), NULL);
        fields.pop_front();
...
void StationBorehole::createSurrogateStratigraphies(
    std::vector<Point*>* boreholes)
{
