...
Station::Station(Station const& src)
    : Point(src.getData()), _name(src._name), _type(src._type), _station_value(src._station_value),
...
StationBorehole::StationBorehole(double x, double y, double z) : Station(x, y, z), _zCoord(0), _depth(0), _date(0)
{
...
int StationBorehole::addStratigraphy(const std::string& path, StationBorehole* borehole)
{
	...
	if (readStratigraphyFile(path, data))
	{
		...
		//
		//			cout << "StationBorehole::addStratigraphy() - Profile incomplete (Borehole " << borehole->_name <<
		//",
		// Layer " << (i+1) << " missing).\n";
		//
		//			return 0;
...
int StationBorehole::addStratigraphy(
    const std::vector<GEOLIB::Point*>& profile, const std::vector<std::string> soil_names)
{
	...
	if (readStratigraphyFile(path, data))
	{
		...
		for (size_t i = 0; i < nBoreholes; i++)
		{
			...
			if (fields.size() >= 4)
			{
				...
				std::cout << "StationBorehole::addStratigraphies() - Unexpected file format..."
				          << "\n";
			// return 0;
		}
	}
...
StationBorehole* StationBorehole::createStation(
    const std::string& name, double x, double y, double z, double depth, std::string date)
{
