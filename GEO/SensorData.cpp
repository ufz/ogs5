...
SensorData::SensorData(const std::string& file_name) : _start(0), _end(0), _step_size(0), _time_unit(TimeStepType::NONE)
{
	this->readDataFromFile(file_name);
}

SensorData::SensorData(std::vector<size_t> time_steps)
    : _start(time_steps[0]), _end(time_steps[time_steps.size() - 1]), _step_size(0), _time_unit(TimeStepType::NONE),
...
void SensorData::addTimeSeries(const std::string& data_name, std::vector<float>* data,
...
void SensorData::addTimeSeries(SensorDataType::type data_name, std::vector<float>* data,
                               const std::string& data_unit_string)
{
	if (_step_size > 0)
	{
		if (((_end - _start) / _step_size) != data->size())
			std::cout << "Warning in SensorData::addTimeSeries() - Lengths of time series does not match number of "
			             "time steps."
			          << "\n";
	}
...
int SensorData::readDataFromFile(const std::string& file_name)
{
	...
	for (size_t i = 0; i < nDataArrays; i++)
	{
		...
		this->_data_type_index.insert(
		    std::pair<SensorDataType::type, size_t>(_vec_names.back(), _data_type_index.size()));
	}
...
SensorDataType::type SensorData::convertString2SensorDataType(const std::string& s)
{
	...
	else if ((s.compare("Precipitation") == 0) || (s.compare("PRECIPITATION") == 0))
