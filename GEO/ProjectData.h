 ...
 * i.e. all geometric data (stored in a GEOObjects-object), all the meshes, FEM
 * Conditions (i.e. Boundary Conditions, Source Terms and Initial Conditions),
 * etc. ProjectData does not administrate any of the objects, it is just a
 * "container class" to store them all in one place. For each class of object
 * stored in this container exists an add-, get- and remove-method.
 *
...
public:
    ...
    const std::map<std::string, MeshLib::CFEMesh*>& getMeshObjects() const
    {
        return _msh_vec;
    }
    ...
    /// Checks if the name of the mesh is already exists, if so it generates a
    /// unique name.
    ...
    /// Returns the FEM Condition set on a GeoObject with the given name and
    /// type from a certain geometry.
    ...
    const std::vector<FEMCondition*> getConditions(
        FiniteElement::ProcessType pcs_type = FiniteElement::INVALID_PROCESS,
        std::string geo_name = "",
        FEMCondition::CondType type = FEMCondition::UNSPECIFIED) const;

    /// Removes the FEM Condition set on a GeoObject with the given name and
    /// type from a certain geometry.
