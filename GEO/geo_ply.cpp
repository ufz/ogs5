...
**************************************************************************/
CGLPolyline::CGLPolyline(const std::string& ply_name) : name(ply_name), _set_eps(false), epsilon(0.01)
{
	//	closed = false;
	type = 2;
	computeline = false;
...
bool CGLPolyline::PointExists(CGLPoint* point, CGLPoint* point1)
{
	...
	{
		dist2 = ((point_vector[i]->x_pix - point->x_pix) * (point_vector[i]->x_pix - point->x_pix)
		         + (point_vector[i]->y_pix - point->y_pix) * (point_vector[i]->y_pix - point->y_pix));
		...
		if (line_string.find("$POINTS") != string::npos) // subkeyword found
		{
			...
			while (i == 1)
			{
				...
				else
				{
					...
					// itoa((int)lvalue,m_strnameid,10); //CC 09/05 convert integer to string ultoa convert unsigned
					// long to string
...
void InterpolationAlongPolyline(CGLPolyline* plyL, std::vector<double>& bcNodalValue)
{
	...
	// Last point
	if (fabs(plyL->point_vector[SizeCGLPoint - 1]->getPropert()) > MKleinsteZahlen)
	{
		ss0.push_back(sl);
	...
	for (i = 0; i < number_of_nodes; i++)
		bcNodalValue[plyL->getOrderedPoints()[i]] = csp->interpolation(plyL->getSBuffer()[i]);
	...
	for (size_t k(0); k < plyL->getSBuffer().size(); k++)
		std::cout << "\t" << plyL->getOrderedPoints()[k] << " " << plyL->getSBuffer()[k] << " "
		          << bcNodalValue[plyL->getOrderedPoints()[k]] << " (bcNodalValue[" << plyL->getOrderedPoints()[k]
		          << "])"
		          << "\n";
...
void CGLPolyline::GetPointOrderByDistance()
{
	...
	//		std::cout << "OrderedPoint[" << k << "]: " << OrderedPoint[k] << ", sbuffer[" << k << "]: " << sbuffer[k] <<
	//", ibuffer[" << k << "]: " << ibuffer[k] <<"\n";
	//	}
}
