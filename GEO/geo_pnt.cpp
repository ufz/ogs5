...
int CGLPoint::IsPointExist()
{
	...
	{
		double Same = sqrt (
		        (gli_points_vector[i]->x - x) * (gli_points_vector[i]->x - x) +
		        (gli_points_vector[i]->y -
		         y) * (gli_points_vector[i]->y - y) +
...
double CGLPoint::PointDis(CGLPoint* m_p2)
{
	return sqrt((x -
	             m_p2->x) *
	            (x - m_p2->x) + (y - m_p2->y) * (y - m_p2->y) + (z - m_p2->z) * (z - m_p2->z));
...
void GEO_Search_DoublePoints(double tolerance)
{
	...
	for (j = 0; j < pointsvectorsize; j++)
	{
		...
		for (k = 0; k < j; k++)
		{
			...
			point_distance = EuklVek3dDistCoor ( x2check,
			                                     y2check,
			                                     z2check,
			                                     x2check0,
			                                     y2check0,
	...
	if(f)
	{
		...
		for(i = 0; i < gli_points_vector_size; i++)
			if(gli_points_vector[i]->id > -1) //OK
				fprintf(f,
				        " %ld %#16.11g %#16.11g %#16.11g $MD %g $ID %s\n", \
				        gli_points_vector[i]->id, \
				        gli_points_vector[i]->x,
				        gli_points_vector[i]->y,
				        gli_points_vector[i]->z, \
