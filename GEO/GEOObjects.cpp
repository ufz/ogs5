...
bool GEOObjects::appendPointVec(std::vector<Point*> const& new_points, std::string const& name,
...
bool GEOObjects::removePointVec(const std::string& name)
{
	...
	{
		std::cout << "GEOObjects::removePointVec() - There are still Polylines or Surfaces depending on these points."
		          << "\n";
		return false;
	}

	for (std::vector<PointVec*>::iterator it(_pnt_vecs.begin()); it != _pnt_vecs.end(); it++)
...
std::vector<Point*>* GEOObjects::filterStationVec(const std::string& name, const std::vector<PropertyBounds>& bounds)
{
	for (std::vector<PointVec*>::iterator it(_pnt_vecs.begin()); it != _pnt_vecs.end(); it++)
...
{
	for (std::vector<PolylineVec*>::iterator it = _ply_vecs.begin(); it != _ply_vecs.end(); ++it)
		if ((*it)->getName().compare(name) == 0)
		{
			delete *it;
			_ply_vecs.erase(it);
...
{
	for (std::vector<SurfaceVec*>::iterator it(_sfc_vecs.begin()); it != _sfc_vecs.end(); it++)
		if ((*it)->getName().compare(name) == 0)
		{
			delete *it;
			_sfc_vecs.erase(it);
...
const std::string GEOObjects::getElementNameByID(const std::string& geometry_name, GEOLIB::GEOTYPE type,
                                                 size_t id) const
{
	std::string name("");
	switch (type)
