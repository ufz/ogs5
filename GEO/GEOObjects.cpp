                             ...
                             double eps)
{
    ...
    _pnt_vecs.push_back(
        new PointVec(name, points, pnt_id_name_map, PointVec::POINT, eps));
}
    ...
    const std::string& name) const
{
    ...
    */
    std::cout << "GEOObjects::getPointVec() - No entry found with name \""
              << name << "\"."
              << "\n";
    return NULL;
}
    ...
    /*
        ...
        for (size_t i = 0; i < size; i++)
            if (_pnt_vecs[i]->getName().compare(name) == 0)
                return _pnt_vecs[i];
    */
    std::cout << "GEOObjects::getPointVecObj() - No entry found with name \""
              << name << "\"."
              << "\n";
    return NULL;
}
...
bool GEOObjects::removePointVec(const std::string& name)
{
    ...
    {
        std::cout << "GEOObjects::removePointVec() - There are still Polylines "
                     "or Surfaces depending on these points."
                  << "\n";
        return false;
    }
    ...
    std::cout << "GEOObjects::removePointVec() - No entry found with name \""
              << name << "."
              << "\n";
    return false;
}
...
std::vector<Point*>* GEOObjects::filterStationVec(
...
const std::vector<Point*>* GEOObjects::getStationVec(
    const std::string& name) const
{
...
#ifndef NDEBUG
    std::cout
        << "DEB: GEOObjects::getPolylineVec() - No entry found with name \""
        << name << "."
        << "\n";
#endif
    return NULL;
...
#ifndef NDEBUG
    std::cout
        << "DEB: GEOObjects::getPolylineVecObj() - No entry found with name \""
        << name << "\"."
        << "\n";
#endif
    return NULL;
    ...
    const std::string& name) const
{
    ...
    std::cout << "GEOObjects::getSurfaceVec() - No entry found with name \""
              << name << "."
              << "\n";
    return NULL;
}
...
const SurfaceVec* GEOObjects::getSurfaceVecObj(const std::string& name) const
{
    ...
    for (size_t i = 0; i < size; i++)
        if (_sfc_vecs[i]->getName().compare(name) == 0)
            return _sfc_vecs[i];
    std::cout << "GEOObjects::getSurfaceVecObj() - No entry found with name \""
              << name << "\"."
              << "\n";
    return NULL;
}
...
const std::string GEOObjects::getElementNameByID(
    const std::string& geometry_name, GEOLIB::GEOTYPE type, size_t id) const
{
    std::string name("");
    switch (type)
    {
                                 ...
                                 std::string& merged_geo_name)
{
            ...
            if (dynamic_cast<GEOLIB::Station*>((*pnts)[0]) == NULL)
            {
                ...
                for (size_t k(0); k < n_pnts; k++)
                    merged_points->push_back(
                        new GEOLIB::Point(((*pnts)[k])->getData()));
            }
            if (n_geo_names - 1 > j)
            {
    ...
    std::vector<size_t> const& id_map(
    ...
    for (size_t j(0); j < n_geo_names; j++)
    {
        ...
        if (plys)
        {
            ...
            {
                GEOLIB::Polyline* kth_ply_new(
                    new GEOLIB::Polyline(*merged_points));
    ...
    for (size_t j(0); j < n_geo_names; j++)
    {
        ...
        if (sfcs)
        {
            ...
            {
                GEOLIB::Surface* kth_sfc_new(
                    new GEOLIB::Surface(*merged_points));
