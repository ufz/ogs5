...
namespace GEOLIB
{
Triangle::Triangle(std::vector<Point*> const& pnt_vec) : _pnts(pnt_vec), _initialized(false), _longest_edge(0.0)
{
...
bool Triangle::containsPoint(const double* pnt, double eps) const
{
	...
	return MathLib::isPointInTriangle(&p, _pnts[_pnt_ids[0]], _pnts[_pnt_ids[1]], _pnts[_pnt_ids[2]], eps);
}
