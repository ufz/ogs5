...
namespace GEOLIB
{
SurfaceGrid::SurfaceGrid(Surface const* const sfc) : AABB(sfc->getAABB()), _triangles_in_grid_box(NULL)
{
		...
		else
		{
			...
			else
			{
				...
				else
				{
					...
					{
						_n_steps[0]
						    = static_cast<size_t>(ceil(sqrt(n_triangles * delta[0] / (n_tris_per_box * delta[2]))));
						_n_steps[1] = 1;
					...
					else
					{
						...
						{
							_n_steps[0]
							    = static_cast<size_t>(ceil(sqrt(n_triangles * delta[0] / (n_tris_per_box * delta[1]))));
						...
						else
						{
							...
							_n_steps[1] = static_cast<size_t>(
							    ceil(sqrt((double)n_triangles / n_tris_per_box * delta[1] / delta[2])));
							_n_steps[2]
							    = static_cast<size_t>(ceil(n_triangles * delta[2] / (n_tris_per_box * delta[1])));
						}
					}
				}
			}
	...
	else
	{
		...
		_n_steps[0] = static_cast<size_t>(
		    ceil(pow(n_triangles * delta[0] * delta[0] / (n_tris_per_box * delta[1] * delta[2]), 1. / 3.)));
...
void SurfaceGrid::writeSurfaceGridData(std::ostream& os) const
{
	...
	for (std::size_t k = 0; k <= _n_steps[2]; k++)
	{
		for (std::size_t j = 0; j <= _n_steps[1]; j++)
		{
			for (std::size_t i = 0; i <= _n_steps[0]; i++)
			{
				os << k * n_grid_plane + j * (_n_steps[0] + 1) + i << " " << _min_pnt[0] + i * _step_sizes[0] << " "
				   << _min_pnt[1] + j * _step_sizes[1] << " " << _min_pnt[2] + k * _step_sizes[2] << "\n";
			}
		}
	}
...
void SurfaceGrid::writeTrianglesInGridCell(std::size_t i, std::size_t j, std::size_t k, std::ostream& os) const
{
	const std::size_t cell_id(k * _n_steps[0] * _n_steps[1] + j * _n_steps[0] + i);
...
bool SurfaceGrid::isPntInSurface(const double* pnt, double eps) const
{
	...
	std::vector<Triangle const*> const& triangles(
	    _triangles_in_grid_box[i + j * _n_steps[0] + k * _n_steps[0] * _n_steps[1]]);
	bool nfound(true);
