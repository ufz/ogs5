...
public:
	...
	QuadTree(POINT const& ll, POINT const& ur, size_t max_points_per_node)
	    : _father(NULL), _ll(ll), _ur(ur), _depth(0), _is_leaf(true), _max_points_per_node(max_points_per_node)
	{
	...
	QuadTree<POINT> const* getChild(Quadrant quadrant) const { return _childs[quadrant]; }
	/**
...
private:
	...
	QuadTree(POINT const& ll, POINT const& ur, QuadTree* father, size_t depth, size_t max_points_per_node)
	    : _father(father), _ll(ll), _ur(ur), _depth(depth), _is_leaf(true), _max_points_per_node(max_points_per_node)
	{
		// init childs
		for (size_t k(0); k < 4; k++)
			_childs[k] = NULL;
