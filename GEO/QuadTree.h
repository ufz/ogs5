...
public:
          ...
          _max_points_per_node(max_points_per_node)
    {
        ...
        //#ifndef NDEBUG
        //		std::cerr << "lower left: " << _ll << ", upper right: " << _ur <<
        //", depth " << _depth << "\n"; #endif
    }
    ...
    QuadTree<POINT> const* getFather() { return _father; }
    QuadTree<POINT> const* getChild(Quadrant quadrant) const
    {
        return _childs[quadrant];
    }
    /**
     * Method calculates the maximum depth of the QuadTree instance. It is used
     * within the method GMSHAdaptiveMeshDensity::getSteinerPoints().
     * @param max_depth (input/output) at the entry max_depth contains the
...
private:
     ...
     * @return
     */
    QuadTree(POINT const& ll, POINT const& ur, QuadTree* father, size_t depth,
    ...
    void splitNode()
    {
        ...
        _childs[0] = new QuadTree<POINT>(mid_point, _ur, this, _depth + 1,
                                         _max_points_per_node);  // north east
        POINT h_ll(mid_point), h_ur(mid_point);
        h_ll[0] = _ll[0];
        h_ur[1] = _ur[1];
        _childs[1] = new QuadTree<POINT>(h_ll, h_ur, this, _depth + 1,
                                         _max_points_per_node);  // north west
        _childs[2] = new QuadTree<POINT>(_ll, mid_point, this, _depth + 1,
                                         _max_points_per_node);  // south west
        h_ll = _ll;
        h_ll[0] = mid_point[0];
        h_ur = _ur;
        h_ur[1] = mid_point[1];
        _childs[3] = new QuadTree<POINT>(h_ll, h_ur, this, _depth + 1,
