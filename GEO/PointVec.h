...
public:
    ...
    PointVec(const std::string& name, std::vector<Point*>* points,
             ...
             double rel_eps = sqrt(std::numeric_limits<double>::min()));
    ...
    std::vector<Point*>* filterStations(
        const std::vector<PropertyBounds>& bounds) const;
...
private:
    ...
    /**
     * squared shortest distance - calculated by
     * calculateShortestAndLargestDistance, possible update by uniqueInsert
     */
