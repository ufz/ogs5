...
namespace GEOLIB
{
Polyline::Polyline(const std::vector<Point*>& pnt_vec) : GeoObject(), _ply_pnts(pnt_vec)
{
	...
	if (_ply_pnt_ids.size() > 1)
	{
		...
		else
		{
			if (pos == _ply_pnt_ids.size() - 1)
			{
				...
				double act_dist(
				    sqrt(MathLib::sqrDist(_ply_pnts[_ply_pnt_ids[_ply_pnt_ids.size() - 2]], _ply_pnts[pnt_id])));
				double dist_until_now(0.0);
...
Polyline* Polyline::closePolyline(const Polyline& ply)
{
	...
	std::cout << "Error in Polyline::closePolyline() - Input polyline needs to be composed of at least three points..."
	          << "\n";
	return NULL;
}
