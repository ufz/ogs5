    ...
    while (!local_ply_vec.empty())
    {
             ...
             it != local_ply_vec.end(); ++it)
        {
            if (pnt_vec == (*it)->getPointsVec())
            {
                ...
                std::cout
                    << "Error in Polyline::contructPolylineFromSegments() - "
                       "Line segments use different point "
                       "vectors..."
                    << "\n";
        }
...
bool Polyline::pointsAreIdentical(const std::vector<Point*>& pnt_vec, size_t i,
