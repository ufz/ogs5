    ...
    : _ply(ply), _mesh(mesh), _linear_nodes(0)
{
    ...
    // located within the threhold the ployline, we do not forced to fill the
    // _msh_node_ids. Therefore, the repeating loop is skipped. repeat until at
    // least one relevant node was found WW while (_msh_node_ids.empty())
    {
        ...
        for (size_t k = 0; k < ply->getNumberOfPoints() - 1; k++)
        {
            ...
            for (size_t j = 0; j < n_nodes; j++)
            {
                ...
                // (_ply->getPoint(k+1) - _ply->getPoint(k)) at the k-th line
                // segment of the polyline, i.e. 0 <= lambda <= 1?
