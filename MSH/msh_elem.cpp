...
/**************************************************************************
   ...
   06/2005 WW Implementation
**************************************************************************/
CElem::CElem(size_t Index, CElem* onwer, int Face) : CCore(Index), normal_vector(NULL), owner(onwer)
{
	int i, j, k, n, ne;
	...
	for (i = 0; i < nnodes; i++)
	{
		...
		for (j = 0; j < ne; j++)
		{
			...
			if ((faceIndex_loc[i] == edgeIndex_loc[0] && faceIndex_loc[k] == edgeIndex_loc[1])
			    || (faceIndex_loc[i] == edgeIndex_loc[1] && faceIndex_loc[k] == edgeIndex_loc[0]))
			{
				edges[i] = owner->edges[j];
				if (faceIndex_loc[i] == edgeIndex_loc[1] && faceIndex_loc[k] == edgeIndex_loc[0])
...
**************************************************************************/
CElem::CElem(size_t Index, CElem* m_ele_parent) : CCore(Index), normal_vector(NULL)
{
...
CElem::CElem(CElem const& elem)
    : CCore(elem.GetIndex()), mat_vector(elem.mat_vector),
      // matgroup_view (elem.matgroup_view),
      selected(elem.selected), normal_vector(new double[3]), representative_length(elem.representative_length),
      courant(elem.courant), neumann(elem.neumann), geo_type(elem.geo_type), owner(elem.owner), ele_dim(elem.ele_dim),
...
//		edges[k] = new CNode ((elem.nodes[k])->GetIndex(), (elem.nodes[k])->X(), (elem.nodes[k])->Y(),
//(elem.nodes[k])->Z());
//	}
...
void CElem::WriteIndex_TEC(std::ostream& os) const
{
	std::string deli = "  ";
	if (geo_type == MshElemType::LINE)
		os << nodes_index[0] + 1 << deli << nodes_index[1] + 1 << deli << nodes_index[1] + 1 << deli
		   << nodes_index[0] + 1;

	else if (geo_type == MshElemType::TRIANGLE)
		os << nodes_index[0] + 1 << deli << nodes_index[1] + 1 << deli << nodes_index[2] + 1 << deli
		   << nodes_index[0] + 1;
	else if (geo_type == MshElemType::PRISM)
		os << nodes_index[0] + 1 << deli << nodes_index[0] + 1 << deli << nodes_index[1] + 1 << deli
		   << nodes_index[2] + 1 << deli << nodes_index[3] + 1 << deli << nodes_index[3] + 1 << deli
		   << nodes_index[4] + 1 << deli << nodes_index[5] + 1 << deli;
	else if (geo_type == MshElemType::PYRAMID)
		os << nodes_index[0] + 1 << deli << nodes_index[1] + 1 << deli << nodes_index[2] + 1 << deli
		   << nodes_index[3] + 1 << deli << nodes_index[4] + 1 << deli << nodes_index[4] + 1 << deli
...
double CElem::calcVolume() const
{
	...
	if (this->geo_type == MshElemType::LINE) // Line
	{
		...
		elemVolume = ComputeDetTex(nodes[0]->getData(), nodes[1]->getData(), nodes[2]->getData(),
	...
	else if (this->geo_type == MshElemType::PRISM)
	{
		...
		// kg44 reactivated ---------Here the direction of flow needs to be taken into account, we need rep length in
		// x,y,z direction
	}
	else if (this->geo_type == MshElemType::PYRAMID)
	{
