...
void FEMRead(const string& file_base_name, vector<MeshLib::CFEMesh*>& mesh_vec, GEOLIB::GEOObjects* geo_obj,
             string* unique_name)
{
	int msize;
	int mrank;
...
// kg44 quick fix to compile PETSC with version PETSCV3.4
	...
	rc = MPI_File_open(MPI_COMM_WORLD, &fname_new_base[0], MPI_MODE_RDONLY, MPI_INFO_NULL, &fh);
	if (rc)
	{
		if (mrank == 0)
...
void FEMRead_BIN(const int msize, const int mrank, const string& file_base_name, vector<MeshLib::CFEMesh*>& mesh_vec,
...
void FEMRead_ASCII(const int msize, const int mrank, const string& file_base_name, vector<MeshLib::CFEMesh*>& mesh_vec,
                   GEOLIB::GEOObjects* geo_obj, string* unique_name)
{
	// 0 long size_sbd_nodes = 0;
	// 1 long size_sbd_nodes_l = 0;
...
int CFEMesh::calMaximumConnectedNodes()
{
	...
	MPI_Allgatherv(&max_connected_nodes, 1, MPI_INT, i_recv, i_cnt, i_disp, MPI_INT, MPI_COMM_WORLD);
