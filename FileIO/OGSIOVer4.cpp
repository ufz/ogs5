...
std::string readPoints(std::istream& in, std::vector<Point*>* pnt_vec, bool& zero_based_indexing,
...
void readPolylinePointVector(const std::string& fname, std::vector<Point*>& pnt_vec, Polyline* ply,
                         ...
                         std::vector<std::string>& errors)
{
	...
	do
	{
		...
		{ // read the point ids
			in >> line;
			if (type != 100)
				while (!in.eof() && line.size() != 0 && (line.find("#") == std::string::npos)
...
std::string readPolylines(std::istream& in, std::vector<Polyline*>* ply_vec,
                          ...
                          bool zero_based_indexing, const std::vector<size_t>& pnt_id_map, const std::string& path,
                          std::vector<std::string>& errors)
{
	if (!in)
	{
	...
	while (!in.eof() && tag.find("#POLYLINE") != std::string::npos)
		tag = readPolyline(in, ply_vec, ply_vec_names, pnt_vec, zero_based_indexing, pnt_id_map, path, errors);

	return tag;
}

void readTINFile(const std::string& fname, Surface* sfc, std::vector<Point*>& pnt_vec, std::vector<std::string>& errors)
{
	// open file
	...
	while (std::getline(in, line).good())
	{
		...
		{
			std::cerr << "Could not read coords of 1st point of triangle \"" << id << "\".\n";
		...
		{
			std::cerr << "Could not read coords of 2nd point of triangle \"" << id << "\".\n";
			errors.push_back(std::string("readTIN error: ")
		...
		{
			std::cerr << "Could not read coords of 3rd point of triangle \"" << id << "\".\n";
			errors.push_back(std::string("readTIN error: ")
		...
		if (MathLib::calcTriangleArea(p0, p1, p2) < d_eps)
		{
			...
			errors.push_back(std::string("readTIN: Triangle ") + number2str(id) + std::string(" has zero area."));
			delete sfc;
			sfc = NULL;
			return;
		}
...
std::string readSurface(std::istream& in, std::vector<Polygon*>& polygon_vec, std::vector<Surface*>& sfc_vec,
                        std::map<std::string, size_t>& sfc_names, const std::vector<Polyline*>& ply_vec,
                        const std::map<std::string, size_t>& ply_vec_names, std::vector<Point*>& pnt_vec,
				...
				else
				{
					if (type == 3)
					{
						std::cerr
						    ...
						    << "\n";
						errors.push_back(
						    "[readSurface] surface type 3: flat surface with any normal direction - - reading not "
						    "implemented");
					}
					if (type == 2)
					{
...
**************************************************************************/
std::string readSurfaces(std::istream& in, std::vector<Surface*>& sfc_vec, std::map<std::string, size_t>& sfc_names,
                         const std::vector<Polyline*>& ply_vec, const std::map<std::string, size_t>& ply_vec_names,
                         std::vector<Point*>& pnt_vec, const std::string& path, std::vector<std::string>& errors)
{
	if (!in.good())
	{
	...
	while (!in.eof() && tag.find("#SURFACE") != std::string::npos)
	{
		...
		tag = readSurface(in, polygon_vec, sfc_vec, sfc_names, ply_vec, ply_vec_names, pnt_vec, path, errors);
	...
	//		}
	//	}
	//	std::cout << "readSurfaces: \"Polygon forest\" consists of " << polygon_forest.size() << " trees" << "\n";
	//
...
bool readGLIFileV4(const std::string& fname, GEOObjects* geo, std::string& unique_name,
                   std::vector<std::string>& errors)
{
		...
		if (plys_vec)
		{
			...
			for (size_t k(0); k < plys->size(); k++)
			{
				...
				for (size_t l(0); l < (*plys)[k]->getNumberOfPoints(); l++)
					os << "\t\t" << pnts_id_offset[j] + ((*plys)[k])->getPointID(l) << "\n";
				plys_cnt++;
			}
		}
	}
