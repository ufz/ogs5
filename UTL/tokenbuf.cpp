...
TokenBuf :: TokenBuf(std::istream& stream, int internal_buf_size)
	: input_stream(stream), buf_max_size(internal_buf_size), buf_read_pos(0), buf_num_bytes(0),
...
TokenBuf :: done()
{
	if((buf_read_pos == buf_num_bytes && !last_token_valid &&
	    input_stream.eof()) || input_stream.bad())
		return true;
	else
		return false;
}
...
void
TokenBuf :: getNextToken(bool ignore_line_break)
{
	int i;
	bool got_it = false;
	int status = 0;
	...
	if(!ignore_line_break && (buf[buf_read_pos] == '\r' || buf[buf_read_pos] == '\n'))
	{
...
std::string
TokenBuf :: peek(bool ignore_line_break)
{
	int i;
...
std::string
TokenBuf :: consume(bool ignore_line_break)
{
	std::string rv;
...
void
TokenBuf :: getline(char* linebuf, int max_num)
{
	int i, pos;
	bool reading = true;
...
bool
TokenBuf :: get_non_empty_line(char* linebuf, int max_num)
{
	bool valid = true;
