...
TokenBuf::TokenBuf(std::istream& stream, int internal_buf_size)
    : input_stream(stream), buf_max_size(internal_buf_size), buf_read_pos(0), buf_num_bytes(0), last_token_valid(false)
{
...
bool TokenBuf::done()
{
	if ((buf_read_pos == buf_num_bytes && !last_token_valid && input_stream.eof()) || input_stream.bad())
		return true;
	else
		return false;
}
