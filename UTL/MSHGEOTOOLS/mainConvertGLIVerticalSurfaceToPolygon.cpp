...
int main(int argc, char* argv[])
{
    if (argc < 5)
    {
        std::cout << "Program " << argv[0]
                  << " converts a vertical surface described by a polyline "
                     "embedded in a mesh to a polygon"
                  << std::endl;
        std::cout << "For this reason the polyline is projected to the bottom "
                     "surface and top surface "
                     "of the mesh. The resulting two polylines are linked to a "
                     "closed polyline, i.e., a polygon."
                  << std::endl;
        std::cout << "Usage: " << argv[0]
                  << " --mesh ogs_meshfile --geometry ogs_geometry_as_gli_file "
                     "[--polyline-id-lower ply_id_lower --polyline-id-upper "
                     "ply_id_upper]"
                  << std::endl;
        return -1;
    }
    ...
    std::cout << "fetched polylines: " << std::flush << plys->size()
              << std::endl;
    if (!plys)
    {
    ...
    for (size_t k(ply_id_lower); k < ply_id_upper; k++)
    {
        ...
        if (!closed)
        {
            std::cout << "converting polyline " << k
                      << " to polygon (closed polyline) " << std::endl;
            GEOLIB::Polygon* polygon(NULL);
            extract_mesh_nodes.getPolygonFromPolyline(*((*plys)[k]), geo,
                                                      unique_name, polygon);
            std::string* polygon_name(new std::string);
            geo->getPolylineVecObj(unique_name)
                ->getNameOfElementByID(k, *polygon_name);
            (*polygon_name) += "-Polygon";
            geo->getPolylineVecObj(unique_name)
                ->push_back(polygon, polygon_name);
        }
    }
