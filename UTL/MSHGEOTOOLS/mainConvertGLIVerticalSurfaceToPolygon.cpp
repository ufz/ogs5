...
int main(int argc, char* argv[])
{
	if (argc < 5)
	{
		...
		std::cout << "For this reason the polyline is projected to the bottom surface and top surface "
		          ...
		          << std::endl;
		std::cout << "Usage: " << argv[0] << " --mesh ogs_meshfile --geometry ogs_geometry_as_gli_file "
		                                     "[--polyline-id-lower ply_id_lower --polyline-id-upper ply_id_upper]"
		          << std::endl;
		return -1;
	}
	...
	for (size_t k(ply_id_lower); k < ply_id_upper; k++)
	{
		...
		if (!closed)
		{
			std::cout << "converting polyline " << k << " to polygon (closed polyline) " << std::endl;
