                              ...
                              double const* const max)
{
	...
	stop = clock();
	std::cout << "query all: " << pnts_in_range.size() << " points in range " << q_min << " x " << q_max << " took "
	          << (stop - start) / (double)(CLOCKS_PER_SEC) << " seconds" << std::endl;
	pnts_in_range.clear();
	...
	oct_tree.getPointsInRange(q_min, q_max, pnts_in_range);
	stop = clock();
	std::cout << "query NEU: " << pnts_in_range.size() << " points in range " << q_min << " x " << q_max << " took "
	          << (stop - start) / (double)(CLOCKS_PER_SEC) << " seconds" << std::endl;
	...
	oct_tree.getPointsInRange(q_min, q_max, pnts_in_range);
	stop = clock();
	std::cout << "query NWU: " << pnts_in_range.size() << " points in range " << q_min << " x " << q_max << " took "
	          << (stop - start) / (double)(CLOCKS_PER_SEC) << " seconds" << std::endl;
	cnt += pnts_in_range.size();
	pnts_in_range.clear();
	...
	oct_tree.getPointsInRange(q_min, q_max, pnts_in_range);
	stop = clock();
	std::cout << "query SWU: " << pnts_in_range.size() << " points in range " << q_min << " x " << q_max << " took "
	          << (stop - start) / (double)(CLOCKS_PER_SEC) << " seconds" << std::endl;
	cnt += pnts_in_range.size();
	pnts_in_range.clear();
	...
	oct_tree.getPointsInRange(q_min, q_max, pnts_in_range);
	stop = clock();
	std::cout << "query SEU: " << pnts_in_range.size() << " points in range " << q_min << " x " << q_max << " took "
	          << (stop - start) / (double)(CLOCKS_PER_SEC) << " seconds" << std::endl;
	cnt += pnts_in_range.size();
	pnts_in_range.clear();
	...
	oct_tree.getPointsInRange(q_min, q_max, pnts_in_range);
	stop = clock();
	std::cout << "query NEL: " << pnts_in_range.size() << " points in range " << q_min << " x " << q_max << " took "
	          << (stop - start) / (double)(CLOCKS_PER_SEC) << " seconds" << std::endl;
	cnt += pnts_in_range.size();
	pnts_in_range.clear();
	...
	oct_tree.getPointsInRange(q_min, q_max, pnts_in_range);
	stop = clock();
	std::cout << "query NWL: " << pnts_in_range.size() << " points in range " << q_min << " x " << q_max << " took "
	          << (stop - start) / (double)(CLOCKS_PER_SEC) << " seconds" << std::endl;
	cnt += pnts_in_range.size();
	pnts_in_range.clear();
	...
	oct_tree.getPointsInRange(q_min, q_max, pnts_in_range);
	stop = clock();
	std::cout << "query SWL: " << pnts_in_range.size() << " points in range " << q_min << " x " << q_max << " took "
	          << (stop - start) / (double)(CLOCKS_PER_SEC) << " seconds" << std::endl;
	cnt += pnts_in_range.size();
	pnts_in_range.clear();
	...
	oct_tree.getPointsInRange(q_min, q_max, pnts_in_range);
	stop = clock();
	std::cout << "query SEL: " << pnts_in_range.size() << " points in range " << q_min << " x " << q_max << " took "
	          << (stop - start) / (double)(CLOCKS_PER_SEC) << " seconds" << std::endl;
	cnt += pnts_in_range.size();
	pnts_in_range.clear();
                 ...
                 std::vector<size_t>& idx_found_nodes)
{
	...
	std::cout << "\tinserting " << n_nodes_oct_tree << " points ... " << std::flush;
	clock_t start(clock());
	for (size_t k(0); k < n_nodes_oct_tree; k++)
	{
	...
	std::cout << "done,  " << (stop - start) / (double)(CLOCKS_PER_SEC) << " seconds, "
	          << (mem_after - mem_before) / (1024 * 1024) << " MBytes" << std::endl;
#endif
...
int main(int argc, char* argv[])
{
	...
	if (argc < 3)
	{
		std::cout << "Usage: " << argv[0] << " --mesh-in meshfile " << std::flush;
...
#ifndef WIN32
	std::cout << "mem for pure mesh data structures: " << (mem_with_mesh - mem_without_mesh) / (1024 * 1024) << " MB"
	          << std::endl;
	clock_t stop = clock();
	std::cout << "reading mesh took " << (stop - start) / (double)(CLOCKS_PER_SEC) << " s" << std::endl;
#endif
...
//	testNaiveAlgorithm(mesh, pnts_for_search, idx_found_nodes_linear_alg);
	...
	std::cout << "MeshGrid Mem: " << (mem_with_mesh_grid - mem_without_mesh_grid) / (1024 * 1024) << " MB" << std::endl;
#endif
