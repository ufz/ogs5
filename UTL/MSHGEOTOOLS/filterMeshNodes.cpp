...
int main(int argc, char* argv[])
{
    if (argc < 7)
    {
        ...
        std::cout << argv[0] << "\n\t--nodes-with-area nodes_with_area_file"
                  << std::endl;
    ...
    in1.close();
    std::cout << "read " << ids1.size() << " ids from file " << argv[4]
              << std::endl;
    ...
    std::vector<bool> not_found_ids_within_ids0(ids0.size(),
                                                true);  // per default
    ...
    if (!out)
    {
        std::cout << "could not open file " << argv[6] << " for output"
                  << std::endl;
        return -1;
    }
    out.precision(12);
    ...
    if (!not_found_ids.empty())
    {
        ...
        std::cout << "number of not found ids: " << not_found_ids.size()
                  << std::endl;
    }
        ...
        if (tmp.find("--mesh") != std::string::npos)
        {
            ...
            if (out_mesh_nodes_as_pnts)
            {
                ...
                for (size_t k(0); k < ids0_size; k++)
                {
                    ...
                    out_mesh_nodes_as_pnts << k << " " << pnt[0] << " "
                                           << pnt[1] << " " << pnt[2]
                                           << std::endl;
                }
