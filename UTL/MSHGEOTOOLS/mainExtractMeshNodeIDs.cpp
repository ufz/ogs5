...
int main(int argc, char* argv[])
{
    if (argc < 5)
    {
        std::cout
            << "program " << argv[0]
            << " takes a *layered* mesh file and closed polylines from a "
               "geometry and "
        ...
        std::cout
            << "Usage: " << std::endl
            << argv[0]
            << "\n\t--mesh ogs_meshfile\n\t--geometry ogs_geometry_as_gli_file"
            << std::endl;
        return -1;
    }
    ...
    //			geo->getPolylineVecObj(unique_name)->getNameOfElementByID(k,
    //*polygon_name);
    ...
    //	while (bounding_polygon_id < n_plys && !
    //(*plys)[bounding_polygon_id]->isClosed()) { 		bounding_polygon_id++;
    //	}
    //
