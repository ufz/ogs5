...
static char const svnid[] =
  "$Id: isotopes.c 4 2009-04-21 17:29:29Z delucia $";
...
/* ---------------------------------------------------------------------- */
{
  ...
  r =
    (master_isotope_ptr->ratio / 1000. + 1.0) * master_isotope_ptr->standard;
 ...
 */
  ...
  for (i = 0; i < count_master_isotope; i++)
  {
	...
	if ((master_isotope[j]->elt == master_isotope[i]->elt) &&
	    (master_isotope[j]->minor_isotope == TRUE))
	{
	  output_msg (OUTPUT_MESSAGE, "%10s\t%12.5e\t%12.5e\t%12.5e\t%12s\n",
		      master_isotope[j]->name,
		      (double) (master_isotope[j]->moles / mass_water_aq_x),
 ...
 */
  ...
  for (i = 0; i < punch.count_isotopes; i++)
  {
    ...
    {
      fpunchf (sformatf ("I_%s", punch.isotopes[i].name), "%12.4e\t", iso);
    }
    else
    {
      fpunchf (sformatf ("I_%s", punch.isotopes[i].name), "%20.12e\t", iso);
    }
 ...
 */
  ...
  for (i = 0; i < punch.count_calculate_values; i++)
  {
    ...
    if (punch.high_precision == FALSE)
    {
      fpunchf (sformatf ("V_%s", punch.calculate_values[i].name), "%12.4e\t",
	       result);
    }
    else
    {
      fpunchf (sformatf ("V_%s", punch.calculate_values[i].name), "%20.12e\t",
	       result);
    }
  }
  return (OK);
 ...
 */
  ...
  else
  {
    ...
    {
      space ((void **) ((void *) &calculate_value), count_calculate_value,
	     &max_calculate_value, sizeof (struct calculate_value *));
    }
 ...
 */
{
  struct calculate_value *calculate_value_ptr;
  calculate_value_ptr =
    (struct calculate_value *) PHRQ_malloc (sizeof (struct calculate_value));
  if (calculate_value_ptr == NULL)
    malloc_error ();
/*
 *   set pointers in structure to NULL, variables to zero
 ...
 */
  ...
  basic_run (cmd, calculate_value_ptr->linebase, calculate_value_ptr->varbase,
	     calculate_value_ptr->loopbase);
  calculate_value_ptr->linebase = NULL;
  calculate_value_ptr->varbase = NULL;
  calculate_value_ptr->loopbase = NULL;
  return (OK);
