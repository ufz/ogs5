...
static char const svnid[] = "$Id: parse.c 4 2009-04-21 17:29:29Z delucia $";
 ...
 */
  for (i = 0; i < count_elts; i++)
  {
    ...
    {
      sprintf (error_string, "Equation does not balance for element, %s.",
	       (elt_list[i].elt)->name);
      error_msg (error_string, CONTINUE);
      oops++;
    }
 ...
 */
    ...
    if (errno == ERANGE)
    {
      sprintf (error_string, "Error in character string for charge, %s.",
	       charge);
      error_msg (error_string, CONTINUE);
      return (ERROR);
    }
 ...
 */
  if (abs (i) > 1)
  {
    ...
    {
      sprintf (error_string,
	       "Error converting charge to character string, %s.", charge);
      error_msg (error_string, CONTINUE);
      return (ERROR);
    }
  }
 ...
 */
  if (isdigit ((int) c) || c == '+' || c == '-' || c == '.')
  {
    ...
    *coef = strtod (token, &ptr1);
    ...
    {
      sprintf (error_string, "Error converting coefficient in get_coef, %s.",
	       token);
      error_msg (error_string, CONTINUE);
      return (ERROR);
    }
