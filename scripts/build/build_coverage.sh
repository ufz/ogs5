...
cmake $CMAKE_ARGS -DOGS_BUILD_TESTS=ON -DOGS_COVERAGE=ON \
	"$CMAKE_GENERATOR" $SOURCE_LOCATION
cmake $SOURCE_LOCATION

cmake --build . --config Debug $BUILD_ARGS
if [ $JENKINS_URL ]; then
	cmake --build . --config Debug --target gtests_cobertura $BUILD_ARGS
else
	cmake --build . --config Debug --target gtests $BUILD_ARGS
fi
