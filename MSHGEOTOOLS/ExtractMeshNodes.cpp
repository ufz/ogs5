...
namespace MeshLib
{
ExtractMeshNodes::ExtractMeshNodes(const CFEMesh* msh) : _msh(msh), _gli_pnt_offset(0)
{
}
...
void ExtractMeshNodes::writeMeshNodeIDs(std::ostream& os, std::ostream& gli_out, const GEOLIB::Polygon& polygon)
{
	...
	for (size_t k(0); k < node_indices.size(); k++)
	{
		...
		gli_out << k + _gli_pnt_offset << " " << coords[0] << " " << coords[1] << " " << coords[2] << "\n";
	}
                                                  ...
                                                  const GEOLIB::Polygon& polygon)
{
	...
	// write last point
	gli_out << n_nodes + _gli_pnt_offset << " " << std::scientific << nodes_as_points[nodes_as_points.size() - 1]
	        << " $NAME " << nodes_as_points[nodes_as_points.size() - 1].getID() << "\n";
	n_nodes++;
	_gli_pnt_offset += n_nodes;
}

void ExtractMeshNodes::getOrthogonalProjectedMeshNodesAlongPolyline(
    GEOLIB::Polyline const& polyline, std::vector<GEOLIB::PointWithID>& nodes_as_points) const
{
	// get all nodes of mesh
...
void ExtractMeshNodes::getTopMeshNodesAlongPolylineAsPoints(const GEOLIB::Polyline& polyline,
...
void ExtractMeshNodes::getBottomMeshNodesAlongPolylineAsPoints(const GEOLIB::Polyline& polyline,
...
void ExtractMeshNodes::getProjectedPolylineFromPolyline(GEOLIB::Polyline const& ply_in, GEOLIB::GEOObjects* geo_obj,
                                                        std::string const& name, GEOLIB::Polyline*& ply_out,
                                                        size_t layer) const
{
	...
	if (!all_projected_ply_pnts.empty())
	{
		...
		{
			selected_projected_ply_pnts.push_back(
			    new GEOLIB::Point(all_projected_ply_pnts[pnt_limits[k] + layer].getData()));
		}
...
void ExtractMeshNodes::writeMesh2DNodeIDAndArea(std::ostream& os, std::ostream& gli_out, const GEOLIB::Polygon& polygon)
{
		...
		for (size_t j(0); j < n_mesh_elem_ids; j++)
		{
			...
			for (size_t i(0); i < n_of_nodes_of_element && found; i++)
			{
				std::vector<size_t>::iterator it(
				    std::find(node_ids.begin(), node_ids.end(), mesh_element_node_indices[i]));
				if (it == node_ids.end())
					found = false;
			}
			if (found)
	...
	for (std::vector<size_t>::const_iterator it(node_ids.begin()); it != node_ids.end(); it++)
	{
		...
		gli_out << n_nodes + _gli_pnt_offset << " " << std::scientific << pnt[0] << " " << pnt[1] << " " << pnt[2]
		        << " $NAME " << *it << "\n";
		n_nodes++;
	}
	_gli_pnt_offset += n_nodes;
...
void ExtractMeshNodes::writeNearestMeshNodeToPoint(std::ostream& os, std::ostream& gli_out, GEOLIB::Point const& pnt)
{
	...
	gli_out.precision(14);
	gli_out << _gli_pnt_offset << " " << std::scientific << coords[0] << " " << coords[1] << " " << coords[2] << "\n";

	_gli_pnt_offset++;
}

void ExtractMeshNodes::getMeshNodeIDsWithinPolygon(GEOLIB::Polygon const& polygon, std::vector<size_t>& node_ids) const
{
