...
public:
	 ...
	 */
	Histogram(std::vector<T> const& data, const unsigned int nr_bins = 16, const bool computeHistogram = true)
	...
	void update()
	{
		...
		{
			itEnd = std::upper_bound(it, (DataCI)_data.end(), _min + (bin + 1) * _bin_width);
...
{
	os << h.getNrBins() << " " << h.getMinimum() << " " << h.getMaximum() << " ";
	std::copy(h.getBinCounts().begin(), h.getBinCounts().end(), std::ostream_iterator<T>(os, " "));
	return os << std::endl;
}
