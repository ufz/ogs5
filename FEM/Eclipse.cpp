...
bool CReadTextfiles_ECL::Read_Text(std::string Filename)
{
	...
	else
	{
		...
		while ((abort == false) && (in_file.eof() == false))
		{
			...
			{
				cout << " Error - increase MAX_ZEILEN in order to read ECLIPSE data file "
				     << "\n";
				cout << " Or shorten the line in " << Filename.data() << ": " << tempstring << " to " << MAX_ZEILEN
				     << " characters "
				     << "\n";
				exit(0);
			}
			...
			if (position < Result.length())
			{
				...
				if (position_e < Result.length())
					Result = Result.substr(0, position_e) + "0"
					         + Result.substr(position_e, Result.length() - (position_e));
				else
					Result = Result + "0";
			}
			else
...
void CECLIPSEData::ReadEclipseGrid(std::string Filename)
{
	...
	for (long i = 0; i < TextFile->NumberOfRows - 1; i++)
	{
		...
		if (tempstring.compare("RADIAL  ") == 0)
		{
			...
			{
				cout << "The phrase below the keyword RADIAL does not contain TRUE or FALSE! The program is aborted!"
				     << "\n";
				// system("Pause");
				exit(0);
			}
		...
		if (tempstring.compare("COORDS  ") == 0)
		{
			...
			in >> m_eclipseblock->column >> m_eclipseblock->row >> m_eclipseblock->layer >> m_eclipseblock->index
			...
			//	cout << "\n" << "The grid output format (GRIDFILE) for Eclipse should be set to 2 0 instead of 1 1! The
			...
			if (m_eclipseblock->active == 1)
			{
				...
				in >> m_eclipseblock->x_coordinates[0] >> m_eclipseblock->y_coordinates[0]
				    >> m_eclipseblock->z_coordinates[0] >> m_eclipseblock->x_coordinates[1];
				in.clear();

				in.str((string)TextFile->Data[i + 4 + Corners_row]);
				in >> m_eclipseblock->y_coordinates[1] >> m_eclipseblock->z_coordinates[1]
				    >> m_eclipseblock->x_coordinates[2] >> m_eclipseblock->y_coordinates[2];
				in.clear();

				in.str((string)TextFile->Data[i + 5 + Corners_row]);
				in >> m_eclipseblock->z_coordinates[2] >> m_eclipseblock->x_coordinates[3]
				    >> m_eclipseblock->y_coordinates[3] >> m_eclipseblock->z_coordinates[3];
				in.clear();

				in.str((string)TextFile->Data[i + 6 + Corners_row]);
				in >> m_eclipseblock->x_coordinates[4] >> m_eclipseblock->y_coordinates[4]
				    >> m_eclipseblock->z_coordinates[4] >> m_eclipseblock->x_coordinates[5];
				in.clear();

				in.str((string)TextFile->Data[i + 7 + Corners_row]);
				in >> m_eclipseblock->y_coordinates[5] >> m_eclipseblock->z_coordinates[5]
				    >> m_eclipseblock->x_coordinates[6] >> m_eclipseblock->y_coordinates[6];
				in.clear();

				in.str((string)TextFile->Data[i + 8 + Corners_row]);
				in >> m_eclipseblock->z_coordinates[6] >> m_eclipseblock->x_coordinates[7]
				    >> m_eclipseblock->y_coordinates[7] >> m_eclipseblock->z_coordinates[7];
				in.clear();

				// m_eclipseblock->CalcBarycentre(); // For block to node interpolation
				// m_eclipseblock->CalculateFaceCentres();

				// cout << m_eclipseblock->x_coordinates[0] << " " << m_eclipseblock->x_coordinates[1] << " " <<
	...
	if (Radialmodell == true)
	{
		for (long i = 0; i < this->elements; i++)
			for (int j = 0; j < int(this->eclgrid[i]->x_coordinates.size()); j++)
			{
				...
				// cout << "Element: " << i << " Point: " << j << " alpha: " << this->eclgrid[i]->y_coordinates[j] << "
				...
				// cout << " radius: " << radius << " x: " << this->eclgrid[i]->x_coordinates[j] << " y: " <<
		...
		for (long i = 0; i < this->elements; i++)
		{
			...
			for (int j = 0; j < int(this->eclgrid[i]->x_coordinates.size()); j++)
			{
				if ((int(this->eclgrid[i]->x_coordinates[j]) >= 0) && (sum > -0.00000001)
				    && (sum < 0.0000001)) // SB had them at *10^-1, redo wtp
					this->RadialModellIpos = true;
				else
				{
					cout << "The Radialmodell is not perpendicular to the positive I axis (East direction)!!"
					     << "\n";
					cout.flush();
					exit(0);
				}
...
void CECLIPSEData::DetermineNeighbourElements(string Projectname)
{
		...
		if ((TextFile->NumberOfRows - 2) == this->elements)
			for (long i = 0; i < TextFile->NumberOfRows - 2; i++)
			{
				...
				if (Element == i)
				{
					...
					// this->eclgrid[i]->x_barycentre << " " << atof(this->SplittedString[2].data()) << " " <<
					// this->eclgrid[i]->y_barycentre << " " << atof(this->SplittedString[3].data()) << " " <<
					// this->eclgrid[i]->z_barycentre << "\n";
					// cout << i << ": " << X << " " << this->eclgrid[i]->x_barycentre << " " << Y << " " <<
					// this->eclgrid[i]->y_barycentre << " " << Z << " " << this->eclgrid[i]->z_barycentre << "\n";
					// cout << epsilon << " " << abs(X - this->eclgrid[i]->x_barycentre) << " " << abs(Y -
					// this->eclgrid[i]->y_barycentre) << " " << abs(Z - this->eclgrid[i]->z_barycentre) << "\n";
					// if ((abs(atof(this->SplittedString[1].data()) - this->eclgrid[i]->x_barycentre) < epsilon) &&
					// (abs(atof(this->SplittedString[2].data()) - this->eclgrid[i]->y_barycentre) < epsilon) &&
					// (abs(atof(this->SplittedString[3].data()) - this->eclgrid[i]->z_barycentre) < epsilon)) {
					if ((abs(X - this->eclgrid[i]->x_barycentre) < epsilon)
						...
						// if ((atof(this->SplittedString[1].data()) == this->eclgrid[i]->x_barycentre) &&
						// (atof(this->SplittedString[2].data()) == this->eclgrid[i]->y_barycentre) &&
						// (atof(this->SplittedString[3].data()) == this->eclgrid[i]->z_barycentre)) {
						for (int j = 0; j < 6; j++)
	...
	if ((error == true) || (Error_NeighbourFile == true))
	{
		...
		// check
		// cout << this->eclgrid[i]->index << ": " << this->eclgrid[i]->NeighbourElement[0] << " " <<
		// this->eclgrid[i]->NeighbourElement[1] << " " << this->eclgrid[i]->NeighbourElement[2] << " " <<
		// this->eclgrid[i]->NeighbourElement[3] << " " << this->eclgrid[i]->NeighbourElement[4] << " " <<
...
int CECLIPSEData::ReadDataFromInputFile(std::string Filename)
{
	...
	if (error == true)
	{
		cout << "The file: " << Filename << "could not been read! The program is canceled"
		     << "\n";
		// system("Pause");
		exit(0);
	}
	...
	for (long l = 0; l < TextFile->NumberOfRows; l++)
	{
		tempstring_l = TextFile->Data[l].substr(
		    0, 8); // WTP: This longer tempstring is needed to compare longer Keywords such as "WCONINJE"
		...
		if (tempstring.compare("INCLUDE") == 0)
		{
			do
			{
				zeilen = zeilen + 1;
			         ...
			         == ""); // look for possible empty rows between the Keyword and the items
		...
		if (this->existWells == true)
			if (tempstring_l.compare("WCONINJE") == 0)
			{
				...
				for (long unsigned j = l + 1; j < (l + this->ecl_well.size() + 1); j++)
				{
		...
		if (tempstring.compare("AQUANCON") == 0)
		{
			...
			while ((TextFile->Data[l + zeile] != "") && (TextFile->Data[l + zeile] != "/"))
			{
				...
				if (TextFile->Data[l + zeile].substr(0, 2) != "--") // ignore comments
				{
					...
					if (number > 200)
					{
						cout << "There were more than 200 bc found in the Eclipse model. Please make sure that the "
						        "definitions at the keyword AQUANCON are correct!"
						     << "\n";
						return false;
					}
						...
						for (int j = j1; j <= j2; j++)
							for (int i = i1; i <= i2; i++)
							{
								...
								for (long m = 0; m < this->elements; m++)
									if ((this->eclgrid[m]->column == i) && (this->eclgrid[m]->row == j)
								...
								this->eclgrid[m_BoundaryConditions->connected_element]
								    ->ConnectedBoundaryCondition.push_back(index_boundary);
							}
					}
...
void CECLIPSEData::ReadEclipseData(std::string Filename, long Timestep)
{
			...
			for (unsigned int k = 0; k < this->Variables.size(); k++)
				if (tempstring.compare(this->Variables[k]) == 0)
				{
					...
					else
					{
						...
						if ((this->Variables[k].compare("PRESSURE") == 0) || (this->Variables[k].compare("PCOW") == 0)
						...
						if (tempNumber == this->elements)
						{
							...
							for (long l = j + 1; l < j + temprows + 1; l++)
							{
								if (l < (j + temprows))
								{
									std::string Dataline[4]; // Definition einen strings mit 4 Arrays, Arrays immer in
									...
									{
										rowindex = rowindex + 1; // zaehlt die gesplitteten Zeilen in der Zeile, in
										// Variante KB4 entspricht das 4
										this->Data[rowindex - 1][k]
										    = atof(Dataline[m].data()) * Multiplier; // schreibt die Zeile aus Dataline
										// mit allen m und dem Keyword k in
										// das double Data rein
										// cout << "Element: " << rowindex << " Variable: " << this->Variables[k] << "
						...
						else
						{
							...
							// cout << "Error while reading the data file. The number of data points doesn't fit to the
							// grid." << "\n";
							temprows = int(ceil(tempNumber / 4.0));
							// Read data for identified variable
							long rowindex = 0;
							for (long l = j + 1; l < j + temprows + 1; l++)
							{
								...
								else
								{
									...
									for (unsigned int m = 0; m < SplittedString.size(); m++)
									{
										...
										this->Data[rowindex - 1][k] = atof(SplittedString[m].data()) * Multiplier;
										//	cout << "Element: " << rowindex << " Variable: " << this->Variables[k] << "
										// Value: " << this->Data[rowindex-1][timestep][k] << "\n";
									}
								}
							}
							j = j + temprows;
	...
	if (this->E100 == true)
	{
		...
		if (int(this->Phases.size()) == 2)
		{
			...
			{
				cout << "\n";
				cout << "GAS-WATER System can not be considered with ECLIPSE E100 and GeoSys"
				     << "\n";
				// system("Pause");
				exit(0);
			}
		...
		{
			cout << "\n";
			cout << "Currently not more than 2 phases are considered for reading eclipse pressure"
			     << "\n";
			cout << "Three phases are only valid if a water saturation exists only at the boundaries of the model "
			        "domain!!";
			// system("Pause");
			// exit(0);
		...
		if (this->Phases.size() == 3)
		{
			...
			// cout << "Element: " << i << " RS: " << this->Data[i][this->GetVariableIndex("RS")] << " SWAT: " <<
		...
		if (this->Phases.size() > 3)
		{
			cout << "Currently not more than 3 phases are considered for reading eclipse data"
			     << "\n";
			// system("Pause");
			exit(0);
		}
...
void CECLIPSEData::CalculateRSfromMassFraction_E300()
{
	...
	{
		cout << "Not all variables are existing in the eclipse output files that are necessary to write data back to "
		        "e300!"
		     << "\n";
		// system("Pause");
		exit(0);
	...
	for (long i = 0; i < this->elements; i++)
	{
		...
		// RS [m�_CO2 / m�_liquid] = (V_liquid * rho_liquid * xw_CO2_liquid) / (rho_CO2_surface * V_liquid) = rho_liquid
		// [kg_liq / m�_liq] * xw_CO2_liquid [kg_CO2 / kg_liq] / rho_CO2_surface [kg_CO2 / m�_CO2]
...
/*-------------------------------------------------------------------------
   ...
   Modification:
   -------------------------------------------------------------------------*/
bool CECLIPSEData::ReplaceASectionInFile(std::string Filename, string Keyword, vector<std::string> Data,
                                         bool CheckLengthOfSection)
{
	...
	for (long i = 0; i < TextFile->NumberOfRows; i++)
		if (TextFile->Data[i].substr(0, Keyword.length()) == Keyword)
		{
			...
			{
				cout << "Replacing a Section in the Eclipse input file is not possible because the section length "
				        "doesn't fit!"
				     << "\n";
				success = false;
				return success;
		...
		if (TextFile->Data[i].substr(0, Keyword_well.length()) == Keyword_well)
		{
			...
			for (long unsigned j = i + zeilen + 1; j < (i + this->ecl_well.size() + zeilen + 1); j++)
			{
				if (this->actual_time == 0)
				{
					...
					std::string outline = this->ecl_well[jj]->name + "   " + this->ecl_well[jj]->phase + "   "
					                      + this->ecl_well[jj]->open_flag + "   " + this->ecl_well[jj]->control_mode
					                      + "   " + this->WellRates[jj] + "   " + " / ";
...
bool CECLIPSEData::CorrespondingElements()
{
	...
	for (unsigned long i = 0; i < m_msh->ele_vector.size(); i++)
		for (long j = 0; j < this->elements; j++)
		{
			...
			if ((grav_c[0] == this->eclgrid[j]->x_barycentre) && (grav_c[1] == this->eclgrid[j]->y_barycentre)
...
bool CECLIPSEData::CompareElementsGeosysEclipse()
{
	...
	for (unsigned long i = 0; i < m_msh->ele_vector.size(); i++)
	{
		...
		//	a_node = ele_nodes[j];
		//	cout << "\n";
		//	cout << a_node->X() << " " << a_node->Y() << " " << a_node->Z() << " ECL " <<
		// this->eclgrid[i]->x_coordinates[j] << " " << this->eclgrid[i]->y_coordinates[j]  << " " <<
...
bool CECLIPSEData::CreateFaces(void)
{
	...
	// in case of radial models i+...direction of radius, j+...direction of theta -> the faces are build with the same
	// points as for rectangular models, but its not necessarily the same direction
	cout << "        CreateFaces() ";
	...
	for (unsigned long i = 0; i < m_msh->ele_vector.size(); i++)
	{
		...
		// if ((i == 608) || (i == 645))
		//	cout << i << "\n";
		// non-radial elements: Right face of the Element (i-direction) (Nodes: 1,5,2,6) -> with this order of the nodes
		// the normal vector shows automatically to the positve i-direction
		// radial elements: Right face of the Element (i-direction) (Nodes: 6,5,2,1) -> with this order of the nodes the
		// normal vector shows automatically to the positve i-direction
		...
		for (unsigned long j = 0; j < vec_face_nodes.size(); j++)
		{
			...
			m_msh->nod_vector[vec_face_nodes[j]->GetIndex()]->distance_to_connected_faces.push_back(
			    sqrt(MathLib::sqrDist(vec_face_nodes[j]->getData(), m_face->GetFaceGravityCentre())));
		}

		// non-radial elements: Upper face of the Element (j-direction) (Nodes: 3,2,7,6) -> with this order of the nodes
		// the normal vector shows automatically to the positve j-direction
		// radial elements: Upper face of the Element (j-direction) (Nodes: 6,2,7,3) -> with this order of the nodes the
		// normal vector shows automatically to the positve j-direction
		m_face = new CFaces(int(this->Phases.size()));
		m_face->index = long(faces.size());
		m_face->model_axis = "J+";
		// store all nodes of the face in a vector
		...
		// non-radial elements: Bottom face of the Element (k-direction) (Nodes: 4,5,7,6) -> with this order of the
		// nodes the normal vector shows automatically to the positve k-direction
		// radial elements: Bottom face of the Element (k-direction) (Nodes: 6,5,7,4) -> with this order of the nodes
		// the normal vector shows automatically to the positve k-direction
		m_face = new CFaces(int(this->Phases.size()));
		m_face->index = long(faces.size());
		m_face->model_axis = "K+";
		// store all nodes of the face in a vector
		...
		// add connected elements to the face (first the top (z) element, second the bottom element, because the order
		// of z is in Eclipse opposit)
		element_indices.clear();
		element_indices.push_back(i);
		...
		// non-radial elements: Left face of the Element at the left (x) site of the grid (i-direction) (Nodes: 0,4,3,7)
		// -> with this order of the nodes the normal vector shows automatically to the positve i-direction
		// radial elements: Left face of the Element at the left (x) site of the grid (i-direction) (Nodes: 7,4,3,0) ->
		// with this order of the nodes the normal vector shows automatically to the positve i-direction
		// if (this->eclgrid[i]->column == 1){
		...
		// non-radial elements: Lower (y) face of the Element at the lower (y) site of the grid (j-direction) (Nodes:
		// 0,1,4,5) -> with this order of the nodes the normal vector shows automatically to the positve j-direction
		// radial elements: Lower (y) face of the Element at the lower (y) site of the grid (j-direction) (Nodes:
		// 5,1,4,0) -> with this order of the nodes the normal vector shows automatically to the positve j-direction
		// if (this->eclgrid[i]->row == 1){
		...
		// non-radial elements: Top (z) face of the Element at the top (z) site of the grid (k-direction) (Nodes:
		// 0,1,3,2) -> with this order of the nodes the normal vector shows automatically to the positve k-direction
		// radial elements: Top (z) face of the Element at the top (z) site of the grid (k-direction) (Nodes: 2,1,3,0)
		// -> with this order of the nodes the normal vector shows automatically to the positve k-direction
		// if (this->eclgrid[i]->layer == 1 /* SB->BG :changed - please check "if (this->eclgrid[i]->layer ==
		// this->layers"*/){
	...
	for (long i = 0; i < long(this->faces.size()); i++)
	{
		...
		if (m_face->model_axis == "I+")
		{
			...
			if (this->RadialModellJpos == true)
			{
				...
				else
				{
					...
					else
					{
						...
						{
							cout << "This phase is not considered yet in GetFlowForFaces"
							     << "\n";
							// system("Pause");
							exit(0);
						}
			...
			m_face->phases[phase_index]->q_norm
			    = (this->Data[element_index][variable_index] / m_face->face_area) / 86400.0;
			m_face->Calculate_components_of_a_vector(0, phase_index, Radialmodell);
			// cout << " Flow through face " << m_face->index << " element " << element_index << " :  " <<
		...
		// if ((element_index == 609) || (element_index == 610) || (element_index == 611) || (element_index == 646) ||
		// (element_index == 647) || (element_index == 648) || (element_index == 683) || (element_index == 684) ||
		// (element_index == 685)  || (element_index == 720)  ||  (element_index == 721)  ||  (element_index == 722))
		//	cout << "Zentrum" << "\n";
		...
		if (m_face->connected_elements.size() == 1)
		{
			...
			{
				cout << "There is more than 1 boundary condition assigned to the cell " << ele_index << "\n";
				// system("Pause");
				exit(0);
			}
			if (this->eclgrid[ele_index]->ConnectedBoundaryCondition.size() == 1)
			{
				...
				if (m_face->model_axis == this->BC[bc_index]->boundary_position)
				{
					...
					if (m_face->phases[phase_index]->q_norm == 0)
					{
						...
						//}
						m_face->phases[phase_index]->q_norm
						    = (this->BC[bc_index]->value[0] * vz / m_face->face_area) / 86400.0;
						m_face->Calculate_components_of_a_vector(0, phase_index, Radialmodell);
						// cout << " Flow through face " << m_face->index <<  " element " << element_index << " :  " <<
					...
					{
						cout << "There is already a flow assigned to the boundary face, which shouldn't happen "
						     << m_face->index << "\n";
						// system("Pause");
						exit(0);
					}
		...
		// cout << ":           " << m_face->q_norm << ":   " << m_face->q[0] << ", " << m_face->q[1] << ", " <<
		// m_face->q[2] << "\n";
	}
...
bool CECLIPSEData::GetVelForFaces(void)
{
	...
	for (unsigned long i = 0; i < this->faces.size(); i++)
	{
		...
		// cout << ": " << m_face->q_norm << "\n";
		// if(fabs(m_face->v_norm) > MKleinsteZahl) cout << " q/v: " << m_face->q_norm/m_face->face_area/0.5
		// /m_face->v_norm ;
		// cout << "\n";
	}
	return true;
...
bool CECLIPSEData::CalcBlockBudget(int phase_index)
{
	...
	for (unsigned i = 0; i < this->eclgrid.size(); i++)
	{
		...
		if ((fabs(flow / flow_max) > 1.0e-3) && (fabs(flow) > 1.0e-10))
		{
			...
			cout << " Error in budget for block " << i << " :  Sum_flow: " << flow << ", max_flow : " << flow_max
			     << ", rel_error: " << flow / flow_max << ", max_error: " << max_error << "\n";
		}
...
bool CECLIPSEData::MakeNodeVector(void)
{
	...
	if (this->NodeData.size() < 1)
		for (unsigned long i = 0; i < m_msh->nod_vector.size(); i++)
		{
			...
			// Test output
			//		cout << " Node " << i << " (X,Y,Z): (" << m_NodeData->x << ", " << m_NodeData->y << ", "<<
			// m_NodeData->z << ") ";
			//		for (long k = 0; k < 3; k++){
			//			cout << m_NodeData->phase_density[k] << " " << m_NodeData->phase_pressure[k] << " " <<
			// m_NodeData->phase_saturation[k] << " ";
			//		}
			//		cout << "\n";
			//		// Test output
			//		cout << " Node " << i << " (X,Y,Z): (" << this->NodeData[this->NodeData.size()-1]->x << ", " <<
			// this->NodeData[this->NodeData.size()-1]->y << ", "<< this->NodeData[this->NodeData.size()-1]->z << ") ";
			//		for (long k = 0; k < 3; k++){
			//			cout << this->NodeData[this->NodeData.size()-1]->phase_density[k] << " " <<
	...
	//							//cout << " Node " << i << " contributed by face " << m_face->index << ", " <<
	// m_face->model_axis
	//<<
	//"  , " << m_face->q_norm << ": ";
	...
	//						//cout << " Warning - no faces for direction (I=0,J=1,K=2): " << k << " at node " << i << "
	// with
	...
	//				vector <string> vec_string;
	//				double v_geosys[3];
	//				vec_string.push_back("Node; X; Y; Z; v_x_ECL; v_y_ECL; v_z_ECL; v_ECL; v_x_Geos; v_y_Geos; v_z_Geos;
	// v_Geos");
	//
	//				for(unsigned long i=0; i< this->NodeData.size();i++){
	//					m_NodeData = this->NodeData[i];
	//					//cout << " NodeData["<< i << "]:  (X,Y,Z): (" << m_NodeData->x << ", " << m_NodeData->y << ", "
	//<<
	// m_NodeData->z << "):        " ;
	//					tempstring="";
	//					temp.str(""); temp.clear(); temp << i; tempstring = temp.str();
	...
	//					//cout << "\n";
	//					//calculate v
	////					temp.str(""); temp.clear(); temp << sqrt(pow(m_NodeData->Flow[0],2) +
	...
	//					}
	//					//calculate v
	//					temp.str(""); temp.clear(); temp << sqrt(pow(v_geosys[0], 2) + pow(v_geosys[1], 2) +
	// pow(v_geosys[2],
...
/*-------------------------------------------------------------------------
   ...
   Modification:
   -------------------------------------------------------------------------*/
void CECLIPSEData::InterpolateDataFromFacesToNodes(long ele_nr, double* n_vel_x, double* n_vel_y, double* n_vel_z,
                                                   int phase_index)
{
	CFEMesh* m_msh = fem_msh_vector[0]; // SB: ToDo hart gesetzt
	MeshLib::CElem* m_ele = NULL;
	...
	bool choose;
	int face_flag
	    = 1; // option: 0...take all faces at one node, 1...take 2 faces: only the faces which belong to the element
	// todo eingabeoption
	...
	/* Go through GeoSys mesh, and get each element (=Eclipse block). For each block, look at all corner points
	   (=Nodes) of the element, as phase velocities are needed there. For each node, look at all faces connected to the
	   node.
	   From each face, get the flow accross the face, and make a weighted sum in the nodes.
	   Faces are not considered, if the are perpendicular to the flow direction, as they then would always contribute
	   zero
	...
	// if ((ele_nr == 646) || (ele_nr == 647) || (ele_nr == 648) || (ele_nr == 683) || (ele_nr == 684) || (ele_nr ==
	// 685)  || (ele_nr == 720)  ||  (ele_nr == 721)  ||  (ele_nr == 722))
	//	cout << "Zentrum" << "\n";
	...
	{ // Get the connected node
		...
		for (unsigned int j = 0; j < m_node->connected_faces.size(); j++)
		{
			...
			if (choose)
			{
				// Testoutput
				// cout << "\n";
				// cout << " Node " << nod_ind << " contributed by face " << m_face->index << ", " << m_face->model_axis
				// << "  , " << m_face->phases[phase_index]->q_norm << ": ";
	...
	//	cout << " Node "<< nod_index[i] << ":  (X,Y,Z): (" << m_msh->nod_vector[nod_index[i]]->X() << ", " <<
...
void CECLIPSEData::InterpolateDataFromBlocksToNodes(CRFProcess* m_pcs, std::string path, int phase_index)
{
	...
	// WW long variable_index_pressure = -1, variable_index_phase_pressure = -1, variable_index_saturation = -1,
	// time_index = 0, variable_index_Gas_dissolved = -1, variable_index_Gas_density = -1, variable_index_Water_density
	// = -1, variable_index_Oil_density = -1;
	long variable_index_phase_pressure = -1, variable_index_saturation = -1, variable_index_Gas_dissolved = -1,
	     variable_index_Gas_density = -1, variable_index_Water_density = -1, variable_index_Oil_density = -1,
	...
	else
	{
		...
		else
		{
			if (this->Phases[phase_index] == "GAS")
			{
				...
				{
					variable_index_vapor_mass_fraction = this->GetVariableIndex("YFW2");
				}
			}
			else
			{
	...
	for (unsigned long i = 0; i < m_msh->nod_vector.size(); i++)
	{
		...
		for (unsigned int j = 0; j < m_node->getConnectedElementIDs().size(); j++)
		{
			...
			//	//cout << " Node " << i << "\n";
			//	//cout << " :  connected block: " << m_node->connected_elements[j] << " (X,Y,Z): " <<
			// m_block->x_barycentre << ", " << m_block->y_barycentre << ", " << m_block->z_barycentre << " with
			// saturation " << sat;
	...
	// Test Output Nodes
	// if ((m_pcs->Tim->step_current==0) || (m_pcs->Tim->step_current==1) || (m_pcs->Tim->step_current==2) ||
	// (m_pcs->Tim->step_current==5) || (m_pcs->Tim->step_current==10) || (m_pcs->Tim->step_current==20) ||
	// (m_pcs->Tim->step_current==30) || (m_pcs->Tim->step_current==40) || (m_pcs->Tim->step_current==50) ||
	// (m_pcs->Tim->step_current==100) || (m_pcs->Tim->step_current==999) || (m_pcs->Tim->step_current==1000)) {
	vector<string> vec_string;
	std::string tempstring;
	ostringstream temp;
	double Val;
	...
	if (this->Phases.size() > 1)
		if (phase_index > 0)
		{
			...
			// vec_string.push_back("Node; X; Y; Z; P-Geosys_phase1; P-Geosys_phase2; Saturation_Geosys_phase1;
			// P-Eclipse_phase1; P-Eclipse_phase2; Saturation_Eclipse_phase1");
			vec_string.push_back(
			    "Node; X; Y; Z; P-Geosys_phase1; P-Geosys_phase2; Saturation_Geosys_phase1; P-Eclipse_phase1; "
			    "P-Eclipse_phase2; Saturation_Eclipse_phase1; q_x1; q_y1; q_z1; q_x2; q_y2; q_z2");
			for (unsigned long i = 0; i < this->NodeData.size(); i++)
			{
				m_NodeData = this->NodeData[i];
				// cout << " NodeData[" << i << "]:  (X,Y,Z): (" << m_NodeData->x << ", " << m_NodeData->y << ", " <<
				...
				////Velocities
				// temp.str(""); temp.clear(); temp.precision(12); temp << this->NodeData[i]->q[0][0]; tempstring += ";
				// " + temp.str();
				// temp.str(""); temp.clear(); temp.precision(12); temp << this->NodeData[i]->q[0][1]; tempstring += ";
				// " + temp.str();
				// temp.str(""); temp.clear(); temp.precision(12); temp << this->NodeData[i]->q[0][2]; tempstring += ";
				// " + temp.str();
				// temp.str(""); temp.clear(); temp.precision(12); temp << this->NodeData[i]->q[1][0]; tempstring += ";
				// " + temp.str();
				// temp.str(""); temp.clear(); temp.precision(12); temp << this->NodeData[i]->q[1][1]; tempstring += ";
				// " + temp.str();
				// temp.str(""); temp.clear(); temp.precision(12); temp << this->NodeData[i]->q[1][2]; tempstring += ";
				// " + temp.str();
...
void CECLIPSEData::WriteDataToGeoSys(CRFProcess* m_pcs, std::string path)
{
	...
	if (int(this->Phases.size()) == 1)
	{
		...
		index_water_density = m_pcs->GetNodeValueIndex(
		    "DENSITY1"); // DENSITY1 is not a variable for some flow process, e.g. LIQUID_FLOW
	...
	else if (int(this->Phases.size()) > 1)
	{
		...
		switch (int(this->Phases.size()))
		{
			case 2:
				...
				break;
			case 3:
				// Assumption that there are 3 phases but water is only used for the boundaries -> the oil and gas phase
				// are the relevant one for the exchange with OGS
				phase1 = 1;
				phase2 = 2;
				break;
			default:
		...
		for (long i = 0; i < nnodes; i++)
		{
			// if ((this->NodeData[i]->x == 499.85001) && (this->NodeData[i]->y == 599.84998) && (this->NodeData[i]->z
			// == -2875))
			//	cout << "Punkt 44" << "\n";
			// calculating capillary pressure (difference of the two phase pressures)
			// cout << " Node " << i << " (X,Y,Z): (" << this->NodeData[i]->x << ", " << this->NodeData[i]->y << ", "<<
			// this->NodeData[i]->z << ") ";
			// for (long k = 0; k < 3; k++){
			//	cout << this->NodeData[i]->phase_density[k] << " " << this->NodeData[i]->phase_pressure[k] << " " <<
			...
			{
				// the water phase is only used for boundary conditions, the wetting phase is phase 2 and the
				// non-wetting phase is phase 3
		...
		if (this->GetVariableIndex("RS") >= 0)
		{
			...
			if (this->ProcessIndex_CO2inLiquid == -1)
				...
				for (int i = 0; i < int(pcs_vector.size()); i++)
				{
					...
					if (n_pcs->nod_val_name_vector[0] == this->dissolved_co2_pcs_name_ECL) // SB redo wtp
						this->ProcessIndex_CO2inLiquid = i;
				}
			if (this->ProcessIndex_CO2inLiquid == -1)
			{
				cout << "In the model exists dissolved gas but there is no dissolved C in water defined. Please ad the "
			...
			indexConcentration = pcs_vector[this->ProcessIndex_CO2inLiquid]->GetNodeValueIndex(
			                         pcs_vector[this->ProcessIndex_CO2inLiquid]->pcs_primary_function_name[0])
			                     + 1; // +1: new timelevel
			// WTP_CB: new way to get the total number of nodes in the mesh since nod_val_vector is not suitable anymore
			// for(unsigned long i = 0; i < pcs_vector[this->ProcessIndex_CO2inLiquid]->nod_val_vector.size(); i++)
			for (long i = 0; i < nnodes; i++)
			{
				// recalculate dissolve gas: c_CO2 [mol/m^3] = RS [m^3gas/m^3water] * gas_density[kg/m^3] /
				// (Molweight_CO2 [g/mol] * 1e-3 [kg/g])
				...
				if (indexConcentration > 0)
					pcs_vector[this->ProcessIndex_CO2inLiquid]->SetNodeValue(i, indexConcentration - 1,
					                                                         value); // 2012.07.02
				// cout << "Node: " << i << " Druck: " << m_pcs->GetNodeValue(i, index_pressure2) << " RS: " <<
				// this->NodeData[i]->Gas_dissolved << " Dichte: " << this->SurfaceCO2Density << " C(4): " << value <<
				// "\n";
				if (value < 0)
				{
					cout << "Node: " << i << " Druck: " << m_pcs->GetNodeValue(i, index_pressure2)
					     << " RS: " << this->NodeData[i]->CO2inLiquid << " Dichte: " << this->SurfaceCO2Density
					     << " dissolved CO2: " << value << "\n";
		...
		if (this->GetVariableIndex("YFW2") >= 0)
		{
			...
			if (this->ProcessIndex_CO2inGas == -1)
			{
				...
				{
					n_pcs = pcs_vector[i];
					if (n_pcs->nod_val_name_vector[0] == this->dissolved_co2_ingas_pcs_name_ECL)
					{
						this->ProcessIndex_CO2inGas = i;
						transportphase
						    = cp_vec[pcs_vector[this->ProcessIndex_CO2inGas]->pcs_component_number]->transport_phase;
						break;
					}
				}
			}
			if (this->ProcessIndex_CO2inGas == -1)
			{
				cout << "In the model exists dissolved gas but there is no C in the gas phase defined. Please ad the "
			...
			// get index of species concentration in nodevaluevector of this process
			indexConcentration_gas = pcs_vector[this->ProcessIndex_CO2inGas]->GetNodeValueIndex(
			                             pcs_vector[this->ProcessIndex_CO2inGas]->pcs_primary_function_name[0])
			                         + 1; // +1: new timelevel
			// WTP_CB: new way to get the total number of nodes in the mesh since nod_val_vector is not suitable anymore
			// for(unsigned long i = 0; i < pcs_vector[this->ProcessIndex_CO2inGas]->nod_val_vector.size(); i++){
			for (long i = 0; i < nnodes; i++)
			{
				// Calculate CO2 Concentration in Gas
				double mass_CO2
				    = this->NodeData[i]->VaporComponentMassFraction * this->NodeData[i]->phase_density[phase2];
				...
				{ // NAPL Phase, i.e. mol/m³REV
					// CB_merge_0513
					if (m_rei)
						porosity = m_rei->node_porosity[i]; // CB: Todo, generalize
					value_gas *= this->NodeData[i]->phase_saturation[2] * porosity;
				}
				pcs_vector[this->ProcessIndex_CO2inGas]->SetNodeValue(i, indexConcentration_gas, value_gas);
				// CB: also set the old TL. This is required by Satu update from NAPL dissolution
				// pcs_vector[this->ProcessIndex_CO2inGas]->SetNodeValue(i, indexConcentration_gas-1, value_gas);
				// cout << "Node: " << i << " Druck: " << m_pcs->GetNodeValue(i, index_pressure2) << " RS: " <<
				// this->NodeData[i]->Gas_dissolved << " Dichte: " << this->SurfaceCO2Density << " C(4): " << value <<
				// "\n";
				if (value_gas < 0)
		...
		if (this->E100 && this->dissolved_co2_ingas_pcs_name_ECL.size() > 0)
		{
			...
			indexConcentration_gas = pcs_vector[this->ProcessIndex_CO2inGas]->GetNodeValueIndex(
			                             pcs_vector[this->ProcessIndex_CO2inGas]->pcs_primary_function_name[0])
			                         + 1; // +1: new timelevel
			transportphase = cp_vec[pcs_vector[this->ProcessIndex_CO2inGas]->pcs_component_number]->transport_phase;
			// WTP_CB: new way to get the total number of nodes in the mesh since nod_val_vector is not suitable anymore
			// for(unsigned long i = 0; i < pcs_vector[this->ProcessIndex_CO2inGas]->nod_val_vector.size(); i++){
			for (long i = 0; i < nnodes; i++)
			{
				// Calculate CO2 Concentration in Gas, gas is pure CO2 so mass fraction is 1 by definition
				double mass_CO2 = 1.0 * this->NodeData[i]->phase_density[phase2];
				value_gas = mass_CO2 / (this->Molweight_CO2 / 1000);
				if (transportphase == 3)
...
std::string CECLIPSEData::ExecuteEclipse(long Timestep, CRFProcess* m_pcs, std::string folder)
{
	...
	if (Timestep == 1)
	{
		...
		if (root_folder != geosys_folder)
		{
			...
			cout << "Warning: The Eclipse simulator model path is not part of the GeoSys model path!!!"
			     << "\n";
			cout << root_folder << "\n";
			cout << geosys_folder << "\n";
			// system("Pause"); // SB redo wtp
	...
	if (this->UsePrecalculatedFiles == false
	    || (this->UsePrecalculatedFiles == true && this->UseSaveEclipseDataFiles == true))
	{
		Keyword = "TSTEP";
		tempstring = "1*";
		temp.str("");
		...
		{
			cout << "Replacing a section in the file: " << File << " didn't work for Keyword TSTEP!"
			     << "\n";
			// system("Pause"); // SB redo wtp
			exit(0);
		}
		...
		if (this->ecl_well.size() != 0)
		{
			for (int ii = 0; ii < int(this->ecl_well.size()); ii++)
			{
				...
				{
					std::string outline = this->ecl_well[ii]->name + "   " + this->ecl_well[ii]->phase + "   "
					                      + this->ecl_well[ii]->open_flag + "   " + this->ecl_well[ii]->control_mode
					                      + "   " + this->WellRates[ii] + "   " + " / ";
			...
			// if (this->WellRates.size() != 0){
			//	std::string outline = this->ecl_well[ii]->name + "   " + this->ecl_well[ii]->phase + "   " +
			// this->ecl_well[ii]->open_flag + "   " + this->ecl_well[ii]->control_mode + "   " +  this->WellRates[ii] +
			//"   " + " / ";
			//	vecData.push_back(outline);
			//	}
			//}
	...
	if (Timestep == 1)
	{
		...
		if (int(this->Phases.size()) == 0)
		{
			...
			for (int i = 0; i < TextFile->NumberOfRows; i++)
			{
				...
				if (this->SplittedString.size() > 0)
				{
					...
					if (this->SplittedString[0] == "CNAMES")
					{
						...
						{
							cout << "The OGS-ECL code can currently only consider the components H2O, CO2 and NaCl in "
							        "this order in E300!"
							     << "\n";
							// //system("Pause"); // SB redo wtp
							exit(0);
			...
			if (this->E100 == true)
			{
				...
				if (Water_phase_exists == false)
				{
					...
					cout << "The program is canceled because no water phase is defined in ECLIPSE!"
					     << "\n";
					// system("Pause");
					exit(0);
				}
				...
				// of course there should be a consistency check afterwards to check if every needed variable is
				// applicable
				...
				if (int(this->Phases.size()) == 2)
				{
					...
					else
					{
						...
						{
							cout << "Currently only Oil-Water or Oil-Gas is available because there is no output "
							        "option in Eclipse E100 for the capillary pressure!"
							     << "\n";
							// //system("Pause"); // SB redo wtp
							exit(0);
						}
			...
			else
			{
				...
				this->Variables.push_back("YFW1"); // mass fraction of component 1 in gas phase
				this->Variables.push_back("YFW2"); // mass fraction of component 2 in gas phase
				this->Variables.push_back("YFW3"); // mass fraction of component 3 in gas phase
		...
		if (this->UsePrecalculatedFiles == false)
		{
			...
			{
				tempstring
				    = EclipseExe + " ." + projectname.substr(root_folder.length(), root_folder.length()); // SB redo WTP
				// tempstring = EclipseExe + " " + projectname;
				if (system(tempstring.c_str()))
				{
					DisplayMsgLn("Warnung: Eclipse doesn't run properly!!! ");
	...
	{
		// change Restart file
		// Changes are included in the restart File, because there it is possible to define new porosities ore
		// permeabilties
		...
		// tempstring = EclipseExe + " " + projectname; // CB 2012 not necessary here redo wtp
		...
		// For the path to the restart file, ECLIPSE under LINUX wants "\" instead of "/"; replace for this section in
		// folder string
		...
		if (this->UsePrecalculatedFiles == false)
		{
			...
			else
			{
				...
				root_folder = Filename.substr(0, position);
				position = int(root_folder.find_last_of("/"));
				tempstring = EclipseExe + " ." + projectname.substr(position, position); // CB 1012
...
bool CECLIPSEData::CleanUpEclipseFiles(std::string folder, std::string projectname)
{
	...
	// systemcommand = "copy " + projectname + ".F* " + folder + "TemporaryResults.F*";
	if (this->Windows_System == true)
		systemcommand = "ren " + projectname + ".F* " + "TEMPORARYRESULTS.F*";
	else
	{
		// systemcommand = "rename " + projectname + ".  " + folder + "TEMPORARYRESULTS. " + projectname + ".F*";
		extension = ".F";
		temp.str("");
		temp.clear();
	...
	if (this->Windows_System == false)
	{
		...
		if (system(systemcommand.c_str()))
		{
			DisplayMsgLn("Could not delete the temporary result files basename.F*! ");
			// return 0;
		}
	}
...
int CECLIPSEData::WriteDataBackToEclipse(CRFProcess* m_pcs, std::string folder)
{
	...
	ostringstream temp;
	std::string tempstring;
	int variable_index_Gas_dissolved, variable_index_porevolume, variable_index_water_saturation,
	    variable_index_Moles_CO2, variable_index_Gas_saturation, variable_index_Gas_pressure;
	int idx = -1;
	...
	long nnodes = fem_msh_vector[0]->GetNodesNumber(false);
	// long nnodes = pcs_vector[this->ProcessIndex_CO2inLiquid]->nod_val_vector.size();
	...
	switch (int(this->Phases.size()))
	{
		case 2:
			...
			break;
		case 3:
			// Assumption that there are 3 phases but water is only used for the boundaries -> the oil and gas phase are
			// the relevant one for the exchange with OGS
			// WW	phase1 = 1;
			// WW	phase2 = 2;
			// phase1 = 1;
			phase2 = 2;
	...
	if (((variable_index_porevolume < 0) || (variable_index_water_saturation < 0) || (variable_index_Moles_CO2 < 0))
	    && (this->E100 == false))
	{
		cout << "Not all variables are existing in the eclipse output files that are necessary to write data back to "
		        "e300!"
	...
	// + 1; // +1: new timelevel
	indexConcentration = n_pcs->GetNodeValueIndex(n_pcs->pcs_primary_function_name[0]) + 1; // +1: new timelevel
	// WTP_CB: new way to get the total number of nodes in the mesh since nod_val_vector is not suitable anymore
	// for(unsigned long i = 0; i < pcs_vector[this->ProcessIndex_CO2inLiquid]->nod_val_vector.size(); i++){
	for (long i = 0; i < nnodes; i++)
	{
		...
		// recalculate dissolve gas: RS [m³gas/m³water] = c_CO2 [mol/m³water] * (Molweight_CO2 [g/mol] * 1e-3 [kg/g]) /
		// gas_density[kg/m³gas]
		// this->NodeData[i]->phase_density[phase2] = m_pcs->GetNodeValue(i, index_gas_density);
		// this->NodeData[i]->deltaDIC =  (pcs_vector[this->ProcessIndex_CO2inLiquid]->GetNodeValue(i,
		// indexConcentration) * (Molweight_CO2 * 1e-3) / this->SurfaceCO2Density)  - this->NodeData[i]->CO2inLiquid;
		// RS = (pcs_vector[this->ProcessIndex_CO2inLiquid]->GetNodeValue(i, indexConcentration) * (Molweight_CO2 *
		// 1e-3) / this->SurfaceCO2Density) ;
		RS = (n_pcs->GetNodeValue(i, indexConcentration) * (Molweight_CO2 * 1e-3) / this->SurfaceCO2Density);
		this->NodeData[i]->deltaDIC = RS - this->NodeData[i]->CO2inLiquid;

		// cout << "Node: " << i << " RS_alt: " << this->NodeData[i]->Gas_dissolved << " DIC: " <<
	...
	for (long i = 0; i < nnodes; i++)
	{
		...
		if ((value < 0) || (value > 1))
		{
			cout << "The CO2 Saturation after the reactions is beyond physical limts: "
			     << "\n";
		...
		if ((NodeData[i]->phase_saturation[phase2] < 0) || (NodeData[i]->phase_saturation[phase2] > 1))
		{
			cout << " Sgas_neu: " << NodeData[i]->phase_saturation[phase2] << " deltaSat: " << NodeData[i]->deltaSat
			     << "\n";
	...
	for (long i = 0; i < nnodes; i++)
	{
		...
		if (value < 0)
		{
			cout << "The CO2 Gas Pressure after the reactions is beyond physical limts: "
			     << "\n";
		...
		if (value_old < 0)
		{
			cout << "The CO2 Gas Pressure from Eclipse is beyond physical limts: "
			     << "\n";
			cout << "Node: " << i << " PRESSURE2_old: " << value_old << " PRESSURE2_new: " << value << "\n";
			return 0;
		}
	...
	for (long i = 0; i < nele; i++) // SB redo wtp
	{
		...
		{
			// cout << "     Node: " << vec_element_nodes[j]->GetIndex() << " RS: " <<
		...
		if (delta_gas_dissolved < 0)
			...
			{
				cout << "The amount of CO2 removed from water is larger than the available amount of CO2! The value is "
				        "corrected!"
				     << "\n";
				for (long j = 0; j < long(vec_element_nodes.Size()); j++)
					cout << "     Node: " << vec_element_nodes[j]->GetIndex()
					     << " RS: " << this->NodeData[vec_element_nodes[j]->GetIndex()]->deltaDIC << "\n";
				cout << "  Element: " << i << " deltaDIC: " << delta_gas_dissolved
				     << " RS_alt: " << this->Data[i][variable_index_Gas_dissolved];
				delta_gas_dissolved = -this->Data[i][variable_index_Gas_dissolved];
			}
		if (delta_gas_dissolved != 0)
		{
			// cout << "  Element: " << i << " deltaDIC: " << delta_gas_dissolved << " RS_alt: " <<
			...
			else
			{
				...
				if (this->E100 == false)
				{
					...
					// c_CO2inLiquid [mol/m�liquid] = RS [m�CO2 / m�liquid] * rho_CO2 [kgCO2 / m�CO2] * 1000 [gCO2 /
					// kgCO2] / M_CO2 [gCO2 / molCO2]
					...
					// deltaT_CO2 = deltaT_CO2 * this->Data[i][variable_index_water_saturation] *
					// this->Data[i][variable_index_porevolume];
					// cout << " water saturation: " << this->Data[i][variable_index_water_saturation] << " pore volume:
					// " << this->Data[i][variable_index_porevolume] << "\n";
					// T_CO2 [mol / L PV] = c_CO2inLiquid [molCO2 / m�liquid] * s_CO2 [m�liquid / m�PV] * 0.001 [m�PV /
					// L PV]
	...
	for (long i = 0; i < nele; i++)
	{
		...
		for (long j = 0; j < long(vec_element_nodes.Size()); j++)
		{
			delta_gas_dis = NodeData[vec_element_nodes[j]->GetIndex()]->deltaSat;
			delta_gas_dissolved += delta_gas_dis * weight;
		}
		if (delta_gas_dissolved < 0)
		{ // decrease in Sgas
			// check if delta_gas_dissolved is not larger than the available volume of CO2 gas phase
			if (fabs(delta_gas_dissolved) > (Data[i][variable_index_Gas_saturation] - epsilon))
			{
				cout << "The volume of CO2 gas phase (Sat) removed is larger than the available volume of CO2 gas! The "
				        "value is corrected!"
				     << "\n";
				for (long j = 0; j < long(vec_element_nodes.Size()); j++)
					cout << "     Node: " << vec_element_nodes[j]->GetIndex()
					     << " Sgas: " << NodeData[vec_element_nodes[j]->GetIndex()]->deltaSat << "\n";
				cout << "  Element: " << i << " deltaSat: " << delta_gas_dissolved
				     << " Sgas_old: " << Data[i][variable_index_Gas_saturation];
				delta_gas_dissolved = -Data[i][variable_index_Gas_saturation]; // limit the delta
			}
		}
		...
		{
			if ((fabs(delta_gas_dissolved) / Data[i][variable_index_Gas_saturation]) < epsilon)
				Sats.push_back(Data[i][variable_index_Gas_saturation]); // old satu
			else
				Sats.push_back(Data[i][variable_index_Gas_saturation] + delta_gas_dissolved);
		}
		else
			Sats.push_back(Data[i][variable_index_Gas_saturation]);
		if (Sats[Sats.size() - 1] < 0)
		{
			cout << "The new calculated Saturation of CO2 gas (SGAS) is negative!"
			     << "\n";
			return 0;
		}
	}
	...
	for (long i = 0; i < nele; i++)
	{
		...
		for (long j = 0; j < long(vec_element_nodes.Size()); j++)
		{
			delta_gas_dis = NodeData[vec_element_nodes[j]->GetIndex()]->deltaPress;
			delta_gas_dissolved += delta_gas_dis * weight;
		}
		if (delta_gas_dissolved < 0)
		{ // decrease in Pressure2
			// check if delta_gas_dissolved is not larger than the old Pressure of CO2 gas phase
			if (fabs(delta_gas_dissolved) > (Data[i][variable_index_Gas_pressure] - epsilon))
			{
				cout << "The pressure reduction of CO2 gas phase is larger than the old pressure of CO2 gas! The value "
				        "is corrected!"
				     << "\n";
				for (long j = 0; j < long(vec_element_nodes.Size()); j++)
					cout << "     Node: " << vec_element_nodes[j]->GetIndex()
					     << " Press: " << NodeData[vec_element_nodes[j]->GetIndex()]->deltaPress << "\n";
				cout << "  Element: " << i << " deltaPress: " << delta_gas_dissolved
				     << " Press_old: " << Data[i][variable_index_Gas_pressure];
				delta_gas_dissolved = -Data[i][variable_index_Gas_pressure]; // limit the delta
			}
		}
		// here set the delta ro element
		if (delta_gas_dissolved != 0)
		{
			if ((fabs(delta_gas_dissolved) / Data[i][variable_index_Gas_pressure]) < epsilon)
				Press.push_back(Data[i][variable_index_Gas_pressure]); // old satu
			else
				Press.push_back(Data[i][variable_index_Gas_pressure] + delta_gas_dissolved);
		}
		else
			Press.push_back(Data[i][variable_index_Gas_pressure]);
		if (Press[Press.size() - 1] < 0)
		{
			cout << "The new calculated Pressure of CO2 gas (PRESSURE) is negative!"
			     << "\n";
			return 0;
		}
		// Pa -> bar: /10000
	...
	if (this->E100 == true)
	{
		...
		{
			cout << "There was not the right number of RS data found during writing to Eclipse F-File!"
			     << "\n";
			// system("Pause"); // SB redo wtp
			return 0;
		}
		...
		{
			cout << "Replacing section " << Keyword << " in the file: " << Filename << " didn't work!"
			     << "\n";
			// system("Pause"); // SB redo wtp
			return 0;
		}
	...
	{
		cout << "There was not the right number of SGAS data found during writing to Eclipse F-File!"
		     << "\n";
		// system("Pause");
		return 0;
	}
	...
	{
		cout << "There was not the right number of PRESSURE data found during writing to Eclipse F-File!"
		     << "\n";
		// system("Pause");
		return 0;
	}
	...
	{ // WTP
		if (mmp_vector[i]->porosity_model == 13)
		{
			...
			if (idx_n < 0)
				cout << " Warning in Eclipse: No POROSITY ele index found with PCS " << m_pcs->getProcessType() << "."
				     << "\n";
		}
	...
	{ // write perm into the Eclipse restart file
		...
		else if (ReplaceASectionInFile(Filename, Keyword, vecString, false) == false)
		{
			cout << "Replacing section " << Keyword << " in the file: " << Filename << " didn't work!"
			     << "\n";
			// system("Pause");
		...
		else if (ReplaceASectionInFile(Filename, Keyword, vecString2, false) == false)
		{
			cout << "Replacing section " << Keyword << " in the file: " << Filename << " didn't work!"
			     << "\n";
			// system("Pause");
		...
		else if (ReplaceASectionInFile(Filename, Keyword, vecString3, false) == false)
		{
			cout << "Replacing section " << Keyword << " in the file: " << Filename << " didn't work!"
			     << "\n";
			// system("Pause");
	...
	/// WTP: Write temperature file for eclipse using the 'TEMPI' keyword in Eclipse
	/// if heattransport is used in OGS and the simulator is Eclipse.
	/// Only works one way (OGS->Eclipse).
	// should this be hard-coded or variable (heat transport in OGS but no coupling to Ecl)
	if (T_Process == true)
	{
		...
		for (long i = 0; i < long(eclipse_ele_active_flag.size()); i = i + j_max)
		{
			...
			for (int j = 0; j < j_max; j++)
			{
				...
				if (eclipse_ele_active_flag[i])
				{
					...
					ele_temp = 0.0;
					weight = float(1.0 / vec_element_nodes.Size()); // getting the correct weight for each nodal value
...
int CECLIPSEData::RunEclipse(long Timestep, CRFProcess* m_pcs)
{
	...
	{ // SB redo wtp
		...
		{
			cout << "The CO2 Density at surface conditions was not read properly (rho <= 0!)."
			     << "\n";
			// system("Pause"); //SB redo wtp
			return 0;
		}
	...
	// write dissolved amount of CO2 and and later maybe also water density back to Eclipse; ToDo: Write new Water
	// density back to Eclipse
	if ((Timestep > 1) && (this->GetVariableIndex("RS") >= 0))
	{
		if (this->UsePrecalculatedFiles == true)
			cout << "Attention! If you run a simulation with precalculated Files you have no interaction between "
			        "Multiphase Flow and Reactions thus you might get wrong Results for dissolved CO2 in liquid!"
			     << "\n";
		else if (this->WriteDataBackToEclipse(m_pcs, folder) == 0)
		{
			cout << "The WriteDataBackToEclipse funktion was not finished properly! The simulation is canceled!"
			     << "\n";
			// system("Pause");
			return 0;
		}
	...
	{
		cout << "The Eclipse execution does not work after " << number_loops << " trials!"
		     << "\n";
		// system("Pause");
		return 0;
	}
	...
	if (Timestep == 1)
	{
		...
		{
			cout << "The Eclipse grid is not identical to the Geosys grid! Abort Calculations!"
			     << "\n";
			// system("Pause");
			return 0;
		}
...
// interpolation of pressure to nodes-> volume is just devided by the number of nodes -> not exact if shape is not a
// quader
		...
		if ((this->eclgrid[i]->column == 40) && (this->eclgrid[i]->layer == 1))
		{
			for (int k = 0; k < this->rows; k++)
			{
				textfile << this->eclgrid[i]->column << " "
				         << " " << k << " " << this->eclgrid[i]->layer << " " << this->eclgrid[i]->x_barycentre << " "
				         << this->eclgrid[i]->y_barycentre << " " << this->eclgrid[i]->z_barycentre << " ";
				// textfile << pressure[i] << " " << soil[i] << " " sgas[i] << " " << oildens[i] << " " << gasdens[i] <<
				// " ";
