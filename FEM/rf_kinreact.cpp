...
#define DMIN(a, b) (dmaxarg1 = (a), dmaxarg2 = (b), (dmaxarg1) < (dmaxarg2) ? (dmaxarg1) : (dmaxarg2))
...
bool KRRead(const std::string& file_base_name, const GEOLIB::GEOObjects& geo_obj, const std::string& unique_name)
{
	...
	for (size_t i = 0; i < length; i++)
	{
		...
		{
			cout << " ERROR! Reaction " << m_kr->name << "  removed from set of reactions due to data inconsistency"
			     << "\n";
			KinReact_vector.erase(KinReact_vector.begin() + i);
		}
	...
	if (!KinReactData_vector.empty())
	{
		...
		for (int j = 0; j < m_krd->NumberReactions; j++)
		{
			...
			for (size_t i = 0; i < m_kr->ProdStochhelp.size(); i++)
			{
				...
				if (m_pcs == NULL)
				{
					cout << " Warning: Component " << m_kr->ProdStochhelp[i]->species
			...
			if (m_kr->type.compare("exchange") == 0)
				for (int i = 0; i < m_kr->number_reactionpartner; i++)
				{
					...
					if (m_pcs == NULL)
					{
						cout << " Warning: Component " << m_kr->reactionpartner[i]
			...
			{ // todo CB isotope fract
				...
				{ // todo CB isotope fract
					...
					if (m_pcs == NULL)
					{
						cout << " Warning: Component " << m_mech->mechSpeciesNames[k]
						     << " specified in KinReact in Mechanism term but not given as transport process "
						     << "\n";
						ok = false;
					}
				}
			...
			if (m_kr->type.compare("Mineralkinetics") == 0)
			{
				...
				if (m_kr->Km_uniform == false)
				{
					...
					for (size_t l = 1; l < m_msh->nod_vector.size();
					     l++) // start at idx 1, as first value has been set already in CKinReact::READ
		...
		for (int j = 0; j < m_krd->NumberReactions; j++)
		{
			...
			if (m_kr->type.compare("Mineralkinetics") == 0)
			{
				...
				for (size_t k = 0; k < length; k++)
				{
					if (m_name.compare(cp_vec[k]->compname) == 0)
					{
						...
						if (cp_vec[k]->transport_phase != 1)
							cout << "Warning: Mineral is not defined as solid phase species, phase # "
				...
				for (size_t k = 0; k < length; k++)
				{
					if ((cp_vec[k]->compname.compare("H2O") == 0) || (cp_vec[k]->compname.compare("H2O_liquid") == 0)
		...
		for (size_t j = 0; j < length; j++)
		{
			...
			//	cout << " PCS: " << j << ", " << sp_name << ", " << idummy << ", " << m_pcs->GetNodeValueIndex(sp_name)
			//+ 1 << "\n";
		}
		...
		int k = m_krd->NumberLinear + m_krd->NumberFreundlich + m_krd->NumberLangmuir + m_krd->NumberNAPLdissolution;
		if (k > 0)
			for (size_t j = 0; j < mmp_vector.size(); j++)
				if (mmp_vector[j]->vol_mat < MKleinsteZahl)
					std::cout << "Warning: Porosity of solid phase is 0.0 ! Change Settings in *.mmp file "
					          << "\n";
		...
		for (size_t j = 0; j < m_krd->AllowReactGeoName.size(); j++)
		{
			...
			if (s_geo_type.compare("SURFACE") == 0)
			{
				...
				if (m_surface)
				{
					m_msh->GetNODOnSFC(m_surface, nodes_vector); // kg44 if used together with PETSC one maybe needs to
			...
			if (!m_kr->NotThisReactGeoName.empty())
			{
				...
				for (size_t j = 0; j < m_kr->NotThisReactGeoName.size(); j++)
				{
					...
					if (s_geo_type.compare("SURFACE") == 0)
					{
						...
						if (m_surface)
						{
							m_msh->GetNODOnSFC(m_surface, nodes_vector); // kg44 if used together with PETSC one maybe
		...
		if (m_krd->ReactDeactFlag)
		{
			...
			//  cout << "\n";
			//}
			m_krd->concentrationmatrix = (double**)malloc(((long)mesh_node_vector_size) * sizeof(double*));
		...
		if (line_string.find("$THRESHHOLDTERMS") != string::npos)
			while (!new_subkeyword)
			{
				...
				if ((thresh_ord != -99.0) && (thresh_conc != -1.0e9))
				{
					...
					if (found == false)
					{
						cout << " WARNING: no matching MONOD SPECIES found in reaction ";
						cout << name << " for THRESHHOLD TERM SPECIES " << thresh_species << "\n";
					}
				}
				else
		...
		{ // subkeyword found
			...
			else if ((T_model == 1) || (T_model == 2))
			{
				if (type.compare("monod") == 0)
				{
					...
					{
						cout << " Unknown model for Temperature Dependence of Reaction " << name
						     << " in CKinReact::Read(). "
						     << "\n";
						cout << " Aborting now!"
						     << "\n";
				...
				{
					cout << " Warning in CKinReact::Read() - Arrhenius-Equation is used for Temperature correction of "
					        "Mineral Reaction "
					     << name << "\n";
				...
				{
					cout << " Warning in CKinReact::Read() - Temperature correction of " << type << " Reaction " << name
					     << " is not implemented. "
					     << "\n";
					cout << " Input values for $TEMPERATURE_DEPENDENCE keyword will be ignored. "
					     << "\n";
	...
	if (type.compare("NAPLdissolution") == 0)
	{
		...
		if (cp_vec[cp_name_2_idx[reactionpartner[0]]]->max_solubility < -MKleinsteZahl)
		{
			...
			cout << " Invalid maximum solubility Csat_pure: "
			     << cp_vec[cp_name_2_idx[reactionpartner[0]]]->max_solubility /*Csat_pure*/
			     << " in Reaction " << name << "\n";
		}
		// Density_NAPL >0
		...
		if (cp_vec[cp_name_2_idx[reactionpartner[0]]]->molar_density < MKleinsteZahl)
		{
			...
			cout << " Invalid NAPL density Density_NAPL: "
			     << cp_vec[cp_name_2_idx[reactionpartner[0]]]->molar_density /*Density_NAPL*/
			     << " in Reaction " << name << "\n";
		}
...
bool CKinBlob::Read(ifstream* rfd_file, const GEOLIB::GEOObjects& geo_obj, const std::string& unique_name)
{
	...
	while (!new_keyword)
	{
		...
		{ // subkeyword found
			in.str(GetLineFromFile1(rfd_file));
			in >> Pe_expo >> NContent_expo >> WContent_expo >> D50_expo >> Poro_expo >> Delta_expo >> UI_expo
			    >> NContent_ini_expo >> NContent_res_expo >> GSR_expo >> Tort_expo >> Length_expo >> Pfannkuch_constant;
			in.clear();
			modSherwood = true;
		}
		//....................................................................
		if (line_string.find("$SHERWOOD_MODEL")
		    != string::npos) // subkeyword found, SP: keyword for calculation of diffenrent Sherwood-models
		{
			in.str(GetLineFromFile1(rfd_file));
			in >> shidx; // Sh_factor >> Re_expo >> Sc_expo >> Pe_expo >> NContent_expo >> WContent_expo >> D50_expo >>
			// Poro_expo >> Delta_expo >> UI_expo >> NContent_ini_expo >> NContent_res_expo >> GSR_expo >>
			// Tort_expo >> Length_expo >> Pfannkuch_constant;
	...
	for (size_t i = 0; i < KinBlob_vector.size(); i++)
	{
		...
		for (size_t j = 0; j < m_kb->BlobGeoName.size(); j++)
		{
			...
			if (s_geo_type.compare("POINT") == 0)
			{
				...
				//				if (m_geo_point)
				//					l = m_msh->GetNODOnPNT(m_geo_point); // + ShiftInNodeVector; // find MSH point
				// number
				// stored
				// in l
...
void KBlobCheck(void)
{
	...
	for (i = 0; i < (int)KinBlob_vector.size(); i++)
	{
		...
		cout << "  Values for interfacial Surfaces have been defined for " << k << " nodes by user."
		     << "\n";
	...
	while (!new_keyword)
	{
		...
		if ((allow == true) || (allow_not == true))
		{
			while (OK)
			{
				...
				if (s_geo_type.find("POINT") != std::string::npos)
				{
					...
					if (!((geo_obj.getPointVecObj(unique_name))->getElementIDByName(s_geo_name, geo_obj_idx)))
					{
						std::cerr << "error in CKinReactData::Read: (type=" << s_geo_type << "): " << s_geo_name
		...
		{ // SB 01.2011
			...
			std::cout << " - RADIAL symmetric quasi 2D hex models with y = 0 as symmetry axis."
			          << "\n";
			std::cout
			    << " - 2DIN3D: vertical x-z cross sections in full 3D hex models with arbitrary y as symmetry axis."
			    << "\n";
	...
	if ((dt > 1.E-20) && (aktueller_zeitschritt > 0))
	{
		...
		if (NumberReactions > 0)
		{
			...
			for (it = 0; it < nnodes; it++)
			{
				...
				{
					if (sortnodes) // execute reactions at nodes in sequence according to no of substeps in last time
						// step
						node = noderanks[it];
					else
						node = it;
					...
					{
#if defined(USE_MPI) || defined(USE_MPI_KRC)
						Biodegradation(Concentration_buff + node * nComponents, node, eps, hmin, &usedtneu, &nok, &nbad,
						               tt, tsteps);
#else
#ifdef OGS_FEM_CAP
						if (tt > 0)
...
#endif
						Biodegradation(Concentration + node * nComponents, node, eps, hmin, &usedtneu, &nok, &nbad, tt,
						               tsteps);
						...
						if (tsteps > 1)
							usedt = usedtneu; // use last successfull timestep to continue integration over substeps
					}
				} // tt
				//
...
// Get MPI results
#if defined(USE_MPI) || defined(USE_MPI_KRC)
			MPI_Allreduce(Concentration_buff, Concentration, nnodes * nComponents, MPI_DOUBLE, MPI_SUM, comm_DDC);
#endif
			// post-processing
			...
			for (node_idx = 0; node_idx < nnodes; node_idx++)
			{
				...
				else
				{
					for (int sp = 0; sp < (nComponents - 1); sp++)
					{
						...
						// // CB HS update
						cp_vec[sp]->getProcess()->SetNodeValue(node_idx, sp_varind[sp],
			...
			if (ReactDeactFlag)
			{
				// cout << "    Kinetic reactions executed at " << count << " of " << nnodes << " nodes." << "\n";
				if (ReactDeactMode
				    == 2) // For mode 2 the C_new must be updated by C_old (C after eraction of last time step)
		...
		{
			cout << "\n"
			     << "Kinetics in node " << save_node << " limit integration step - nok: ";
...
void CKinReactData::Biodegradation(double* m_Conc, long node, double eps, double hmin, double* usedtneu, int* nok,
                                   int* nbad, int tt, int tsteps)
{
	...
	double Csat_max = 0, DensityNAPL = 0, DensityAQ = 0, DiffusionAQ = 0, ViscosityAQ = 0, PoreVelocity = 0.0, d50 = 0,
	       Reynolds = 0, Schmidt = 0, Sherwood = 0, NCont_Sh2 = 0; // NCont_Sh1=0,
	...
	if (Number_of_blobs > 0)
	{
		...
		if (gassdiss == false)
			DensityAQ
			    = mfp_vector[0]->Density(variables); // should be returned as temperature dependent value, if required
		if (Tflag)
		{
			if (REACTINT_vec.size() > 0) // Get the Temperature
			...
			if (m_kr->typeflag_napldissolution) // dissolution reaction identifie
			{
				if (m_kr->typeflag_gasdissolution == 1)
				{
					...
					m_kr->Current_Csat[node]
					    = Csat_max; // keine NAPL-Masse vorhanden, NAPL-Bildung möglich wenn c(singleSubstance) > Csat
				}
				else
				{
					Sp1 = m_kr->ex_species[0] + 1; // Sp1 = NAPL-species
					...
					if (oldMass[blob] > 0.)
						...
						// m_kr->current_Csat = Csat_max;              // keine NAPL-Masse vorhanden, NAPL-Bildung
						// möglich wenn c(singleSubstance) > Csat
						m_kr->Current_Csat[node] = Csat_max; // keine NAPL-Masse vorhanden, NAPL-Bildung möglich wenn
		...
		for (r = 0; r < nreactions; r++)
		{
			...
			if (m_kr->typeflag_napldissolution)
			{
				...
				DarcyVelocity
				    = m_kr->GetNodeDarcyVelocity(node); // SP: added for Sherwood calculation after Powers et al. 1992
		...
		for (r = 0; r < Number_of_blobs; r++)
		{
			...
			if (m_kb->gas_dissolution_flag == false)
			{
				DiffusionAQ = mfp_vector[0]->PhaseDiffusion(
				    variables); // CB Todo: this should be a component property => Sherwood is component dependent
				...
				switch (shidx)
				{
					...
					case 1: // Sherwood calculation after Powers et al. (1994)
						// Sherwood = m_kb->Sh_factor * pow(Reynolds, m_kb->Re_expo) * pow(Delta, m_kb->Delta_expo) *
						// pow(m_kb->UI, m_kb->UI_expo) * pow(m_kb->NCont_ini, m_kb->Geometry_expo) ;
					...
					case 2: // Sherwood calculation after Powers et al. (1992)
						Sherwood = m_kb->Sh_factor * pow(Reynolds, m_kb->Re_expo) * pow(d50, m_kb->D50_expo)
					...
					case 5: // Sherwood calculation after Miller et al. (1990)
						Sherwood = m_kb->Sh_factor * pow(mod_Reynolds, m_kb->Re_expo)
						           * pow(WATERcontent, m_kb->WContent_expo) * pow(Schmidt, m_kb->Sc_expo);
						break;
					...
					case 7: // Sherwood calculation after Geller&Hunt (1993)
						Sherwood = m_kb->Sh_factor * pow(mod_Reynolds, m_kb->Re_expo)
						           ...
						           * pow(m_kb->NCont_ini, m_kb->NContent_ini_expo) * pow(Poro, m_kb->Poro_expo)
	...
	{
		success = odeint(m_Conc, Number_of_Components, tstart, tend, eps, usedt, hmin, &nexth, nok, nbad, derivs,
		...
		{ // if odeint failed for some reason
			...
			success = odeint(m_Conc, Number_of_Components, tstart, tend, eps * 10, usedt, hmin, &nexth, nok, nbad,
			                 derivs, stifbs, rkqs, node, SolverType);
			if (success == 0)
			{
				cout << "  -Trying perturbation of initial time step usedt"
				     << "\n";
				success = odeint(m_Conc, Number_of_Components, tstart, tend, eps, usedt * 0.99, hmin, &nexth, nok, nbad,
				                 derivs, stifbs, rkqs, node, SolverType);
				if (success == 0)
				{
					...
					success = odeint(m_Conc /*_save*/, Number_of_Components, tstart, tend, eps, usedt, hmin, &nexth,
					                 nok, nbad, derivs, stifbs, rkqs, node, SolverType);
					...
					/*else*/ if (success == 0)
					{
						cout << "   -Trying perturbation of concentration vector by 1 per mil"
						     << "\n";
						for (sp = 0; sp < Number_of_Components; sp++)
							m_Conc[sp + 1] = tempstore[sp]; // restore original double value precision
						for (sp = 0; sp < Number_of_Components; sp++)
							m_Conc[sp + 1] *= 1.01; // pertube
						success = odeint(m_Conc, Number_of_Components, tstart, tend, eps, usedt, hmin, &nexth, nok,
						                 nbad, derivs, stifbs, rkqs, node, SolverType);
						for (sp = 0; sp < Number_of_Components; sp++)
							m_Conc[sp + 1] /= 1.01; // restore Masses
						if (success == 0)
						{
							cout << "   -Trying increased relative error " << eps * 100 << "\n";
							success = odeint(m_Conc, Number_of_Components, tstart, tend, eps * 100, usedt, hmin, &nexth,
							                 nok, nbad, derivs, stifbs, rkqs, node, SolverType);
							if (success == 0)
								cout << "  ODE integration finally failed. Skipping this node."
								     << "\n";
						}
					}
				}
...
void derivs(double t, double c[], double dcdt[], int n, long node, double /*steplength*/)
{
	...
	for (r = 0; r < nreactions; r++)
	{
		...
		if (m_kr->typeflag_monod)
		{
			...
			{
				m_kr->currentnode = node; // CB 19/10/09 This is eclusively for Brand model to allow porosity in
				...
				if (m_kr->grow)
					dcdt[BacteriaNumber]
					    += BacGrowth * m_kr->EffectiveYield(node); // CB 08/12 added a microbial yield coefficient
			...
			if (m_kr->typeflag_mineralkinetics)
			{
				...
				if (fabs(1 - pow(omega, m_kr->Theta)) > 0)
				{
					...
					if (MinGrowth != 0)
					{
						...
						// mineral species precipitation or dissolution (not for the actual mineral,
		...
		for (i = 0; i < n; i++)
		{
			...
			if ((c[i + 1] + dcdt[i + 1] * dt < 0) && (dcdt[i + 1] * dt != 0))
			{
				...
				{ // species by species
					dcdt[j + 1] = 0;
					for (r = 0; r < nreactions; r++)
						dcdt[j + 1] += ratematrix[j][r]; // sum up contributions of all reactions
				}
			}
		...
		if (m_kr->typeflag_exchange)
		{
			...
			if (m_kr->typeflag_exchange_linear)
			{
				...
				else
				{
					...
					/* Die Abfrage verringert die Desorptionsgeschwindigkeit, wenn absehbar ist, dass Csorbiert im
					 * Negativen landet */
			...
			if (m_kr->typeflag_exchange_freundlich)
			{
				...
				if (c[Sp2] > residual)
					...
					chochexp
					    = (1. - exponent) * pow(residual, exponent) + exponent * pow(residual, (exponent - 1)) * c[Sp2];

				exchange = exch * (parameter * chochexp - c[Sp1]);
				// slow down desorption rate by constant factor
				if (exchange < 0)
					exchange *= ratefact;
				/* Die Abfrage verringert die Desorptionsgeschwindigkeit, wenn absehbar ist, dass Csorbiert im Negativen
				 * landet */
				if (-exchange * dt > c[Sp1])
					exchange = -c[Sp1] / dt;
	...
	for (r = 0; r < nreactions; r++)
	{
		...
		if (m_kr->typeflag_napldissolution)
		{
			...
			exch = m_kb->Masstransfer_K[node]
			       * m_kb->Interfacial_area[node]; // k * A externally calculated in Function Biodegradation
...
void jacobn(double t, double c[], double dfdt[], double** dfdc, int n, long node)
{
	...
	double CMonodSpecies, MonodConcentration, CInhibitionSpecies, InhibitionConcentration, Yield;
	...
	for (r = 0; r < nreactions; r++)
	{
		...
		if (m_kr->typeflag_monod)
		{
			...
			if (BacteriaMass > 1.E-40)
			{
				...
				m_kr->currentnode = node; // CB This is eclusively for Brand model to allow porosity in Inhibition
				...
				// Berechnung der Bakterien Ableitungen für den Fall dass eine maximale Kapazität berücksichtigt werden
				// muss
				...
				// This is for the case that growth of a specific bacteria is limited by the SPECIFIC bacteria density
				// BacteriaMass & specif_cap
				...
				{ // bacteria specific capacity term defined
					...
					if (maxVelocity > 1.E-30)
					{
						...
						d2X_dtdS[BacteriaNumber] = BacGrowth * (speccap) / (BacteriaMass * (BacteriaMass + speccap))
						                           * Yieldcff; // CB 08/12 added yield coeff
				...
				// Berechnung der Bakterien Ableitungen für den Fall dass KEINE maximale Kapazität berücksichtigt werden
				// muss
				// without capacity term, this is identical for growth and decay term; but not if yield coeff is
				// considered (CB)
				else
				{ // maxkap = 0
					/* d2Xi / dt*dXi = BacGrowth / Xi */
					d2X_dtdS[BacteriaNumber]
					    = BacGrowth / BacteriaMass * Yieldcff; // CB 08/12 added yield coeff = 1 for decay;
				}
				...
				for (i = 0; i < NumberMonod; i++)
				{
					...
					if (CMonodSpecies > 1.E-20)
					{
						    ...
						    || (m_kr->monod[i]->species == m_kr->Isotope_light))
						{
							...
							d2X_dtdS[MonodSpecies] = BacGrowth * MonodOrder * (MonodConcentration + Ciso)
							                         / CMonodSpecies
							...
							d2X_dtdS[IsotopeSpecies]
							    = BacGrowth * MonodOrder * (-1) / (MonodConcentration + CMonodSpecies + Ciso);
							// If a threshhold term exists for the Monod species, the partial derivative is modified for
							// this species
							//   - the ODE with Monod and threshold term for Monod species C and Isotope fractionation
							//   is:
							...
							//   - The partial derivative with respect to Cl (taking into account Division by the Monod
							//   and Threshhold term) is
							//     d2X/dtdCl = my*R * [n*(Ch+K)/Cl/(Cl+Ch+K) + p*T/(Cl+Ch)/(Cl+Ch+T)]
							//   - The partial derivative with respect to Ch (taking into account Division by the Monod
							//   and Threshhold term) is
							//     d2X/dtdCh = my*R * [-n/(Cl+Ch+K) + p*T/(Cl+Ch)/(Cl+Ch+T)]
							//   - The latter term is identical for both derivs hence must be added to the previously
							//   calculated first terms
							if (m_kr->monod[i]->threshhold == true)
							{
								ThreshConc = m_kr->monod[i]->threshConc;
								ThreshOrder = m_kr->monod[i]->threshOrder;
								d2X_dtdS[MonodSpecies] += BacGrowth * ThreshOrder * ThreshConc / (CMonodSpecies + Ciso)
								                          / (ThreshConc + CMonodSpecies + Ciso); // isofrac
								d2X_dtdS[IsotopeSpecies] += BacGrowth * ThreshOrder * ThreshConc
								                            / (CMonodSpecies + Ciso)
								                            / (ThreshConc + CMonodSpecies + Ciso); // isofrac
							}
						}
						else // no isofrac standard case: d2X_dtdS[MonodSpecies] = BacGrowth * MonodConcentration /
						// CMonodSpecies / (MonodConcentration+CMonodSpecies); // no isofrac
						{
							d2X_dtdS[MonodSpecies] = BacGrowth * MonodOrder * MonodConcentration / CMonodSpecies
							...
							//      - with n and K the Order and Monod-concentration of the Monod term
							//      - with m and T the Order and Threshhold-concentration of the threshhold term
							//   - The partial derivative with respect to C (taking into account Division by the Monod
							//   and Threshhold term) is
					...
					else if (CMonodSpecies < -1.E-20)
					{
						...
						d2X_dtdS[MonodSpecies]
						    = BacGrowth * MonodOrder / CMonodSpecies / 1000; // Changed monod term with smaller slope CB
						if (m_kr->monod[i]->threshhold == true)
						{
							ThreshConc = m_kr->monod[i]->threshConc;
							ThreshOrder = m_kr->monod[i]->threshOrder;
					...
					else
					{
						...
						d2X_dtdS[MonodSpecies]
						    = m_kr->BacteriaGrowth(r, c, sumX, MonodSpecies, node) / MonodConcentration;
					}
				} // for NumberMonod
				...
				/* Berechnung der Ableitungen f�r die vom Bakteriellen Wachstum abh�ngigen Substanzen, unabh�ngig von
				 * maxkap */
		...
		if (m_kr->typeflag_mineralkinetics)
		{
			...
			if (Betrag > 0)
			{
				...
				for (i = 0; i < int(m_kr->minSpeciesIdx.size()); i++)
				{
					...
					if (BetragArg <= -1) // strong supersaturation
						d2X_dtdS[MineralSpecies]
						    *= 1 / eta2; //   --> slow down precipitation rate by root fct. (default = 1)
				...
				for (i = 0; i < no_mech; i++)
				{
					...
					if (BetragArg <= -1) // strong supersaturation
						dXdt_Mech = pow(dXdt_Mech,
						                1 / eta2); //   --> slow down precipitation rate by root function (default = 1)
					dXdt_Mech
					    *= m_mech->Mech(c, node) * area / porosity1; //   *K_mech*A0/(1-n)  ;  unit: --> mol/s/m³_solid
				...
				{ // these are the rows i
					Yield = m_kr->ProductionStoch[i]; // = 0 for Mineral (must), <0 for min species consumed, >0 for
					// species produced by precipitation
					if (fabs(Yield) > 1.E-30)
					{
						porosity2 = m_kr->GetReferenceVolume(i, node);
						for (j = 0; j < n; j++)
						{ // these are the j column entries of a row i, Yi is the same in 1 row
							if (fabs(d2X_dtdS[j + 1]) > 1.E-30)
								// unit conversion: --> mol/s/m³_solid * m³sol/m³Aq * (m³w/m³Aq)^-1 = mol/s/m³w
								dfdc[i + 1][j + 1] += d2X_dtdS[j + 1] * Yield * porosity1 / porosity2;
						}
					...
					if (m_kr1->getType().compare("exchange") == 0)
						if (m_kr1->typeflag_exchange_langmuir)
						{
							...
							if (surfaceID == surfaceID2)
							{
								...
								dfdc[Sp2][SpX] += kadsorb * c[Sp2] * porosity1 / porosity2 * density1;
							}
						}
				}
			} // end if langmuir
	...
	for (r = 0; r < nreactions; r++)
	{
		...
		if (m_kr->typeflag_napldissolution)
		{
			...
			// Remark: In function derivs the dissolution velocity can be reduced, if NAPL concentration falls below
			// zero.
			...
			if ((m_kb->Masstransfer_K[node] * m_kb->Interfacial_area[node]
			     * pow((m_kr->Current_Csat[node] - c[Sp2]), exponent)
			     * dt)
			    > c[Sp1])
			{
				...
				if ((m_kr->Current_Csat[node] < c[Sp2]) || (c[Sp1] > MKleinsteZahl))
			...
			else
			{
				...
				// if ( (m_kr->current_Csat < c[Sp2]) || (c[Sp1] > MKleinsteZahl) ) {         // no dissolution or NAPL
				// mass present
				if ((m_kr->Current_Csat[node] < c[Sp2])
				    || (c[Sp1] > MKleinsteZahl)) // no dissolution or NAPL mass present
				{
...
void CKinReact::TestWrite(void)
{
	...
	for (i = 0; i < (int)ProdStochhelp.size(); i++)
		cout << ProdStochhelp[i]->species << "  " << ProdStochhelp[i]->concentration << "\n";
	// exchange
	...
	for (i = 0; i < nnodes; i++)
	{
		...
		{ // gas_dissolution
			...
			if (psg) // PS_GLOBAL
				...
				satu_N_old
				    = 1.0
				      - m_pcs->GetNodeValue(i, idx2 + 1); // idx2+1 as in case of Eclipse coupling, Sats are transient.
...
void CalcNewPhasePressure()
{
	...
	for (i = 0; i < nnodes; i++)
	{
		...
		// if( delta > 1e-6)
		//  cout << "!!! " << Cgo << " " << Cgn << " " << Clo << " " << Cln << " " << ((Cgo - Cgn)+(Clo - Cln)) << " "
		//  << poro << " " << Satu << "\n";
...
void CKinReactData::ReactionDeactivation(long nonodes)
{
	...
	if (((aktueller_zeitschritt) % react_t)
	    == 0) // when evaluated before loop over nodes, i.e. prepare for this time step

		// if(((aktueller_zeitschritt+1) % react_t ) == 0) {  // when evaluated after  loop over nodes, i.e. prepare for
		// next time step
		for (node = 0; node < nonodes; node++)
		{
			ReactDeact[node] = false;
			if (ReactDeactMode != 3) // for all timesteps, anyway prepare the concentrationmatrix for the next time
		...
		if (ReactDeactMode == 1)
			for (node = 0; node < nonodes; node++)
			{
				...
				for (sp = 0; sp < Number_of_Components; sp++)
				{
					...
					{
						React_dCdT[node] += fabs((Concentration_old - Concentration) / maxi)
						                    / dt; // normalized by current local concentration
						// and now prepare concentrationmatrix for next time step, i.e. save current concentrations
						// after Transport
						concentrationmatrix[node][sp] = cp_vec[sp]->getProcess()->GetNodeValue(node, sp_varind[sp]);
					}
				}
			...
			switch (ReactDeactMode)
			{
				...
				case 2: // compare with C after transport of last timestep, loop over all components
					for (sp = 0; sp < Number_of_Components; sp++)
					{
						...
						// and now prepare the concentrationmatrix for the next time step, i.e. save the current
	...
	for (node = 0; node < nonodes; node++)
	{
		if (ReactDeact[node] == true)
			for (sp = 0; sp < Number_of_Components; sp++)
			{
				...
				cp_vec[sp]->getProcess()->SetNodeValue(node, sp_varind[sp], Concentration);
			}
	}
}
...
void CKinReactData::Aromaticum(long nonodes)
{
	...
	for (int sp = 0; sp < nospec; sp++)
		if (cp_vec[sp]->getProcess()->nod_val_name_vector[0].compare("Aromaticum") == 0)
		{
...
void CKinReactData::PreprocessMicrobe_drmc_(double steplength)
{
	...
	for (i = 0; i < NumberMicrobeData; i++)
	{
		...
		for (j = 0; j < (long)m_msh->nod_vector.size(); j++)
		{
			...
			S = m_md->_drmc_level[j]
			    * exp((m_md->k_incr * theta * (1 - theta) - m_md->k_decr * (1 - theta)) * m_md->dt);
			if (S > 1.0)
				S = 1.0;
	...
	if (activity_model == 1 || activity_model == 2)
	{
		...
		for (j = 0; j < nnode; j++)
		{
			...
			for (i = 0; i < ncomp; i++)
			{
				...
				if (cp_vec[i]->compname.compare("pH") == 0)
					conc = pow(
					    10,
					    -conc * unitfactor_l); // this is inacurate as it should be molality of H+, not the activity.
		...
		switch (activity_model)
		{
			...
			case 1: /* DH*/
				if (Imax > 0.1)
					DisplayMsgLn(
					    "Warning in CKinReactData::PreprocessMinkin(): I > 0.1 --> use of DH activity model is not "
					    "recomended!");
				break;
			case 2: /* Davies*/
				if (Imin < 0.1)
					DisplayMsgLn(
					    "Warning in CKinReactData::PreprocessMinkin(): I < 0.1 --> use of Davies activity model is not "
					    "recomended!");
				break;
			case 3: /* CHEMAPP*/
				break;
				...
				if (cp_vec[j]->transport_phase == 0)
				{
					...
					for (i = 0; i < nnode; i++)
					{
						...
						else
						{
							...
							if (i == 0 && j == 0)
								cout << "Warning in KinReactData: zero Ionic strength in PreprocessMinKin; using gamma "
								        "= 1.0"
								     << "\n";
						}
					}
						...
						if (I > 0)
						{
							if (fabs(val) > 0) // charged species
								ActivityCoefficients[i][j]
								    = pow(10, -A * val * val * (sqrt(I) / (1 + sqrt(I)) - 0.3 * I));
							else
							{ // uncharged species
								if (j == widx)
									ActivityCoefficients[i][j] = 1; // set water activity to 1
	...
	for (i = 0; i < NumberReactions; i++)
	{
		...
		if (m_kr->typeflag_mineralkinetics && m_kr->Km_uniform == false)
		{
			...
			{ // find the matching CAP Kinetic reaction
				...
				{
					if (m_kr->chemapp_name.compare(m_cap->Kin_Reactions[j].type) == 0)
					{ // found
						for (k = 0; k < nnode; k++)
						{
							...
							{ // what, if no data has been calculated by chemapp??
								cout << " Warning in CKinReactData::PreprocessMinkin:"
								     << "\n";
								cout << "    No Chemapp-log(K) data available for reaction " << m_kr->chemapp_name;
								cout << " at node " << k << "\n";
								if (k > 0)
									m_kr->Km[k] = m_kr->Km[k - 1]; // use neighbour node value
								else
									m_kr->Km[k] = m_kr->Km_default; // use default input value
							}
						}
						break;
					}
			...
			{ // find the matching HKF Kinetic reaction
				...
				{
					if (m_kr->chemapp_name.compare(m_cap->Kin_HKF_Reactions[j].type) == 0)
					{ // found
						for (k = 0; k < nnode; k++)
						{
	...
	for (i = 0; i < NumberReactions; i++)
	{
		...
		if (m_kr->typeflag_mineralkinetics && m_kr->Am_constant == false)
		{
			...
			for (k = 0; k < nnode; k++)
			{
				...
				// conc = pcs_vector[sp_pcsind[m_kr->mineral_number]]->GetNodeValue(k,sp_varind[m_kr->mineral_number]);
				// // CB HS update
				conc = cp_vec[m_kr->mineral_number]->getProcess()->GetNodeValue(k, sp_varind[m_kr->mineral_number]);
	...
	for (i = 0; i < NumberReactions; i++)
	{
		...
		if (m_kr->typeflag_mineralkinetics && m_kr->Am_constant == false)
		{
			...
			for (k = 0; k < nnode; k++)
			{
				...
				conc0
				    = cp_vec[m_kr->mineral_number]->getProcess()->GetNodeValue(k, sp_varind[m_kr->mineral_number] - 1);
...
double CKinReact::MinRate(double* c, int node, double dt, double Om)
{
		...
		else
		{
			...
			if ((1 - H1) <= 0)
				rate = -1.0 * precipfactor * ktot * A
				       / vol; // precipfactor used to slow down precipitation by constant factor
			else
				rate = ktot * A / vol;
		...
		{ // dissolution
			...
			for (i = 0; i < int(ProductionStoch.size()); i++)
			{
				if (ProductionStoch[i] > 0 || i == mineral_number)
				{
					...
					{ // this is other species consumed by dissolution, Stcoef > 0
	...
	//        poro2	= GetReferenceVolume(minSpeciesIdx[i],node);         // dissolved phase volume
	//        maxrate = -conc / dt * poro2 / poro1 / ProductionStoch[minSpeciesIdx[i]]; // - before conc is necessary
	//        here
	//      }
	//      // update maxrate, the larger, the more positive, so find minimum here
	//      rate = DMIN(rate,maxrate);
...
double CKinReact::MinRateConstant(double* c, int node, double disso)
{
	...
	for (i = 0; i < number_Mech; i++)
	{
		...
		{ // for precipitation, consider only base mechanism, if set in inputfile
...
double MinkinMech::Mech(double* c, int node)
{
	...
	for (i = 0; i < no_mechSpec; i++)
	{
		...
		{ // pH is -log(gamma*H+)
			if (actmodel > 0)
				kmech *= pow(pow(10, m_krd->ActivityCoefficients[node][idx] * (-conc * unitfactor_l)),
				             mechSpeciesExpo[i]);
			else
	...
	if (copy_nodes.size() < 1)
	{
		...
		else if ((radial) || (TwoDinThreeD))
		{
			...
			if (radial)
			{
				...
				if ((coord0[1] != -1.0 * coord3[1]) || (coord1[1] != -1.0 * coord2[1]))
			...
			if (radial)
			{
				...
				{
					cout << "\n"
					     << " Copy Cons - Please switch off one model side for reaction calculation ! - Option "
					        "switched off "
					     << "\n";
					radial = TwoDinThreeD = copy_concentrations = false;
				}
