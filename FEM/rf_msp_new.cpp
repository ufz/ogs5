...
double CSolidProperties::Enthalpy(double temperature, const double latent_factor )
{
	        ...
	        (temperature < ((*data_Capacity)(2) + (*data_Capacity)(3))))
	{
		...
		val +=  (*data_Capacity)(0) * dT
		       + 0.5 * dT * dT *
		       ((*data_Capacity)(1) - (*data_Capacity)(0)) / (*data_Capacity)(3);
	}
	else
	{
		dT =   (*data_Capacity)(3);
...
void CSolidProperties::HeatConductivityTensor(const int dim, double* tensor, int group)
{
	...
	case 1: //Ortho
		...
		break;
	case 2: //Aniso
		for(i = 0; i < thermal_conductivity_tensor_dim * thermal_conductivity_tensor_dim;
		    i++)
...
void CSolidProperties::ElasticConstitutiveTransverseIsotropic(const int Dimension)
{
	...
	if(Time_Dependent_E_nv_mode>0)
	{
		...
		{
		case 1:
			cout<<"WARNING: for transversely isotropic elasticity, please use TIME_DEPENDENT_YOUNG_POISSON mode=2"<<endl;
			break;
		case 2:
...
void CSolidProperties::CalculateCoefficent_MOHR(double ep, double scalar_comp, double scalar_tens)	//WX:11.2010, 09.2011
{
	int valid = 1;
                                           ...
                                           double& dPhi, const int Update)
{
	...
	if(F > 0.0 && (!PreLoad))             // in yield status
	{
		...
		err_corner = 4.5 * Xi * K * normXi / G + BetaN *
		             (0.5 * Hard *
		              sqrt(1.0 + 3.0 * Xi * Xi) * normXi / G + Y0 + Hard * ep ) / Al;
		// Multi-surface
		if(p3 > err_corner)
		{
			// RF0 = F;
			...
			while(isLoop)
			{
				...
				Jac = 9.0 * Xi * K + 3.0 * Xi * Xi * BetaN * Hard *
				      (dPhi + dl2) / (fac * Al);
				F = 9.0 * Xi * K *
				    (dPhi + dl2) + BetaN * (Y0 + Hard * (ep0 + fac)) / Al - p3;
				dl2 -= F / Jac;
...
//WX
int CSolidProperties::DirectStressIntegrationDPwithTension(const int GPiGPj,
                                                           Matrix* De,
                                                           const ElementValue_DM* ele_val,
                                                           double* TryStress,
                                                           const int Update,
		...
		else
		{
			...
			else
			{
				...
				for(i = 0; i < Size; i++)
				{
					dTempStr[i] = 0.0;
					if(i < 3)
						dTempStr2[i] = TryStress[i] + dstrs[i] - yy / Al /
						               3.0;
					else
						dTempStr2[i] = TryStress[i] + dstrs[i];
				}
				...
				for(i = 0; i < Size; i++)
				{
					if(i < 3)
						tmpStr[i] =
						        (TryStress[i] + I1 /
						         3.0) - tmpvalue * (dTempStr[i] + dI1 / 3.0);
					else
			...
			mm =
			        (3 * tension -
			         ((*ele_val->Stress)(0,
			          ...
			          (*ele_val->Stress)(1,
			                             GPiGPj) +
			          (*ele_val->Stress)(2, GPiGPj))) / (dstrs[0] + dstrs[1] + dstrs[2]);
		else
			mm = 1.;
		failurestate = 2; //tensile failure
	}
	...
	if(shearsurf>1e-5 || tensionsurf>1e-5)
	{
		...
		if(tensionsurf>MKleinsteZahl)//if tension
		{
			...
			while (1)
			{
				...
				{
					dlode_dsig[0]=sqrt3/(2.*MathLib::fastpow(sqrtJ2,3)*cos(3.*LodeAngle))*(3.*J3/2./J2*devStr[0]
					-(devStr[0]*devStr[0]+devStr[3]*devStr[3]+devStr[4]*devStr[4])+2.*J2*Kronecker[0]/3.0);

					dlode_dsig[1]=sqrt3/(2.*MathLib::fastpow(sqrtJ2,3)*cos(3.*LodeAngle))*(3.*J3/2./J2*devStr[1]
					-(devStr[3]*devStr[3]+devStr[1]*devStr[1]+devStr[5]*devStr[5])+2.*J2*Kronecker[1]/3.0);
					...
					if(Dim==3)
					{
						dlode_dsig[4]=sqrt3/(2.*MathLib::fastpow(sqrtJ2,3)*cos(3.*LodeAngle))*(3.*J3/2./J2*devStr[4]
						-(devStr[0]*devStr[4]+devStr[3]*devStr[5]+devStr[4]*devStr[2])+2.*J2*Kronecker[4]/3.0);

						dlode_dsig[5]=sqrt3/(2.*MathLib::fastpow(sqrtJ2,3)*cos(3.*LodeAngle))*(3.*J3/2./J2*devStr[5]
						-(devStr[3]*devStr[4]+devStr[1]*devStr[5]+devStr[5]*devStr[2])+2.*J2*Kronecker[5]/3.0);
					}
				}
				...
				if(first_step)
				{
					for(i=0;i<Size;i++)
					{
						dft_dsig[i]=1/3.*Kronecker[i]+2/sqrt3*sin(LodeAngle+2/3.*PI)
							*dsqrtJ2_dsig[i]+2/sqrt3*sqrtJ2*dlode_dsig[i]*cos(LodeAngle+2/3.*PI)
							-dtens_dsig[i];
						dgt_dsig[i]=1/3.*Kronecker[i]+2/sqrt3*sin(LodeAngle+2/3.*PI)
							*dsqrtJ2_dsig[i]+2/sqrt3*sqrtJ2*dlode_dsig[i]*cos(LodeAngle+2/3.*PI);
						//-dcsn_dsig[i];
					}
				}
				...
				else
				{
					...
					else if(tensionsurf_k1<0)
					{
						...
						lamda_neg = dlamda;
						dlamda = lamda_pos-tensionsurf_pos*(lamda_neg-lamda_pos)/(tensionsurf_neg-tensionsurf_pos);
		...
		if(shearsurf>1e-5)
		{
			...
			while (1)
			{
				...
				if(first_step)
				{
					for(i=0;i<Size;i++)
					{
						dfs_dsig[i]=(Ntheta-1)/3.*Kronecker[i]+2/sqrt3*(Ntheta*sin(LodeAngle+2/3.*PI)-sin(LodeAngle-2/3.*PI))
							*dsqrtJ2_dsig[i]+2/sqrt3*sqrtJ2*dlode_dsig[i]*(Ntheta*cos(LodeAngle+2/3.*PI)-cos(LodeAngle-2/3.*PI))
							-dcsn_dsig[i];
						dgs_dsig[i]=(Nphi-1)/3.*Kronecker[i]+2/sqrt3*(Nphi*sin(LodeAngle+2/3.*PI)-sin(LodeAngle-2/3.*PI))
							*dsqrtJ2_dsig[i]+2/sqrt3*sqrtJ2*dlode_dsig[i]*(Nphi*cos(LodeAngle+2/3.*PI)-cos(LodeAngle-2/3.*PI));
						//-dcsn_dsig[i];
					}
				}
				//
	...
	*TmpDe = (0.);
	...
	if(Plasticity_Bedding)//WX:09.2011
	{
		if(itesteps==2||Update>0)
		{
			...
			//tmp_load_dir_major = acos(fabs(tmp_load_dir_major))*180/PI; //because ||bedding_norm|| and ||prin_dir|| is 1
	...
	if((shearsurf > 0) || (tensionsurf > 0))
	{
		...
		else
		{
			...
			cout<<"error"<<"csn="<<csn<<"||"<<"Ntheta="<<Ntheta<<"||"<<"tension="<<tension<<endl;
		}
...
{
	return (vec1[1] * vec2[2] - vec1[2] *
	        vec2[1]) *
	       (sigma_B[0] -
	        sigma_l[0]) + (vec1[2] * vec2[0] - vec1[0] * vec2[2]) * (sigma_B[1] - sigma_l[1])
...
void CSolidProperties::ConsistentTangentialDP(Matrix* Dep, const double dPhi, const int Dim)
{
	...
	if(dl2 > 0.0)                         // Multi-surface
	{
		...
		c3 = K * c1 /
		     (c1 + 3.0 * Al * K *
		      sqrt(dPhi * dPhi + 3.0 * Xi * Xi * (dPhi + dl2) * (dPhi + dl2)));
	...
	else
	{
		...
		case 2:                   // 2D
			...
			if(axisymmetry)
			{
				(*Dep)(0,2)
				        = -c1 / 3.0 + c2 + c3 *
				          (Xi * s33 + Al * s11) + c4 * s11 * s33;
				// Row 2
				(*Dep)(1,2)
				        = -c1 / 3.0 + c2 + c3 *
				          (Xi * s33 + Al * s22) + c4 * s33 * s22;
				// Row 3
				(*Dep)(2,0)
				        = -c1 / 3.0 + c2 + c3 *
				          (Xi * s11 + Al * s33) + c4 * s11 * s33;
				(*Dep)(2,1)
				        = -c1 / 3.0 + c2 + c3 *
				          (Xi * s22 + Al * s33) + c4 * s22 * s33;
				(*Dep)(2,2)
				        =  2.0 * c1 / 3.0 + c2 + c3 *
				          (Al + Xi) * s33 + c4 * s33 * s33;
				(*Dep)(2,3)
        ...
        const int Update)
{
	...
	if(F > TolF && !PreLoad)              /* In Yield Status */
	{
		...
		if(Co > 0.0)
			preSub = 1 +
			         (int)(acos(TensorMutiplication2(Stress_n, nStress,
			                                         dim) / Co) / den);
		else
			preSub = minSub;
		if(preSub > maxSub)
		...
		for(iSub = 0; iSub < preSub; iSub++)
		{
			...
			I_p4 = I_p2 * I_p2;
			ang = Stress_Inv[2] / MathLib::fastpow(sqrt(Stress_Inv[1]),3);
			PHI = sqrt(Stress_Inv[1] * pow(1 + Mat_n1[5] * ang,
			                               Mat_n1[6]) + 0.5 * Mat_n1[0] * I_p2
		...
		for(iSub = elasticSub; iSub < preSub; iSub++)
		{
			...
			while(ErrLoc > Tolerance_Local_Newton)
			{
				...
				PHI = sqrt(Stress_Inv[1] * pow(1 + Mat_n1[5] * ang,
				           ...
				           + Mat_n1[2] * Mat_n1[2] * I_p4);
				PSI = sqrt(
				        Stress_Inv[1] / psi1 + 0.5 * Mat_n1[0] * I_p2 + Mat_n1[2] *
				        Mat_n1[2] * I_p4);
					...
					for(i = 0; i < LengthMat; i++)
					{
						...
						for(j = 0; j < LengthStrs; j++)
						{
							...
							for(k = 0; k < LengthStrs; k++)
							{
								...
								if(k > 2)
									c2 = 2.0;
								t1 += c1 * c2 * Stress_n1[k] *
								      (*d2G_dSdS)(k,j);
							}
							t2 = -dlmd * Co *
							     (supMat[i] - Mat_n1[i]) * t1;
			...
			if(subPLASTIC > 0)
				ep += dlmd * sqrt(2.0 * TensorMutiplication2(dG_dS,
				                                             dG_dS,
				                                             dim) / 3.0);
                                   ...
                                   const int LengthStrs)
{
	...
	for(i = 0; i < LengthStrs; i++)
	{
		dFdS[i] *= MatN1[5] * MatN1[6] * S_Invariants[1] * pow(1 + MatN1[5] * ang,
		                                                       MatN1[6] - 1.0);
...
void CSolidProperties::dF_dMat(double* dFdM, const double* S_Invariants, const double* MatN1)
{
	...
	dFdM[4] = -1.0;
	/*dF_dGamma*/
	dFdM[5] = 0.5 * S_Invariants[1] *
	          MatN1[6]* ang* pow(1.0 + MatN1[5] * ang, MatN1[6] - 1.0) / PHI;
	/*dF_dM*/
		...
		for(j = 0; j < LengthStrs; j++)
		{
			...
			(*d2G_dSdS)(i,j) =
			        ...
			        ((Kronecker(ii,kk) * Kronecker(jj,ll) - delta_ij_kl / 3.0) / psi1
			         + (MatN1[0] + 12.0 * MatN1[2] * MatN1[2] *
			            I_p2) * delta_ij_kl) / PSI
			        - 0.25 *
			        (DevS[i] / psi1 +
			         (MatN1[0] * In1 + 4.0 * MatN1[2] * MatN1[2] *
			          I_p3) * Kronecker(ii,jj))
			        * (DevS[j] / psi1 +
			           (MatN1[0] * In1 + 4.0 * MatN1[2] * MatN1[2] *
			            I_p3) * Kronecker(kk,ll)) / PSI_p3
                                           ...
                                           const int LengthStrs)
{
	...
	for(i = 0; i < LengthStrs; i++)
	{
		...
		for(j = 0; j < LengthStrs; j++)
			(*d2G_dSdS)(i,j) -=
			        0.5 * (MatN1[0] + 12.0 * MatN1[2] * MatN1[2] * I_p2) * Kronecker(ii,
			                                                                         jj)
			        * RotV[j] / PSI
			        - 0.25 *
			        (DevS[i] / psi1 +
	...
	for(i = 0; i < LengthStrs; i++)
	{
		...
		(*d2G_dSdM)(i,0)          //dG_dS_dAlpha
		        =   0.5 * In1 * delta_ij / PSI
		          - 0.125 *
		          (DevS[i] / psi1 +
		           (MatN1[0] * In1 + 4.0 * MatN1[2] * MatN1[2] * I_p3) * delta_ij)
		          * I_p2 / PSI_p3;
	...
	for(i = 0; i < LengthStrs; i++)
	{
		...
		for(j = 0; j < LengthStrs; j++)
		{
			        ...
			        * (DevS[j] / psi1 +
			        ...
			        + var * (RotV[i] * Kronecker(kk,ll)
			                 ...
			                 (Kronecker(ii,
			                  ...
			                  Kronecker(jj,
			                            ll) -
			                  Kronecker(ii,jj) * Kronecker(kk,ll) / 3.0)) / PSI);

			(*d2G_dSdS)(l,j) /= In1;
			(*d2G_dSdS)(l,
			            j) -= 0.5 *
			                  (In1 * RotV[i] - mr *
			                   DevS[i]) * Kronecker(kk,ll) / (PSI * In1 * In1);

			if(j < 4)
				(*d2G_dSdM)(l,j) = -In1 * (*d2G_dSdS)(i,j) / 3.0
				                   - var* In1* Kronecker(ii,
				                                         kk) *
			...
			(*d2G_dSdS)(l,j) +=
			        -0.5 * var * (In1 * RotV[i] - mr * DevS[i])
			        * (MatN1[0] * In1 + 4.0 * MatN1[2] * MatN1[2] *
			           I_p3) * RotV[j] / PSI_p3 + var * RotV[i] * RotV[j] / PSI
                                                         ...
                                                         const int Update)
{
	...
	if(F > 0.0 && !PreLoad)               // in yield status
	{
		...
		if(Update < 1)
		{
			...
			alpha3 = -q_tr *
			         (1.0 + 0.5 * vartheta * p_c * var1 / K) / (p_tr - 0.5 * p_c);
                                         ...
                                         double* dstrain, double temperature)
{
	...
	case 2:
		...
		//TN: BGRb
	case 3:
		fac = 1.5*dt*((*data_Creep)(0)*exp(-(*data_Creep)(2)/(8.314472*(temperature+273.15)))*pow(norn_S, (*data_Creep)(1))+
			(*data_Creep)(3)*exp(-(*data_Creep)(5)/(8.314472*(temperature+273.15)))*pow(norn_S, (*data_Creep)(4)));
		break;
		//TN: BGRsf
	case 4:
		fac = 1.5*dt*((*data_Creep)(0)*exp(-(*data_Creep)(2)/(8.314472*(temperature+273.15)))*
			pow(norn_S, (*data_Creep)(1))+(*data_Creep)(4)*pow(norn_S, 2));
		break;
	}
	for(i = 0; i < ns; i++)
