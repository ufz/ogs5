...
namespace SolidProp
{
		...
		while (!new_keyword)
		{
			...
			if (line_string.find("$PLASTICITY") != string::npos)
			{
				...
				else if (sub_line.find("MOHR-COULOMB") != string::npos) // WX
				{
					...
					Plasticity_type = 4;
					Size = 7;  // LU 10.2018 added flag for locking tensile behavior
							   /*
							   i	parameter
							   0	cohesion
							   1	friction angle
							   ...
							   6   tensile lock flag (if 1, set tensile strength to *data_plasticity(3) even with beddings and microplasticity)
							   */
				}
	...
	*************************************************************************/
																									   ...
																									   // 09.2011
	{
		...
		if ((scalar_comp_tmp > 0) || (scalar_tens_tmp > 0))
        {
			;
        }
		...
		if ((*data_Plasticity)(6) > 0)       // LU 10.2018 to overwrite limit on tensile strength if flag is activated
		{                                     // LU 10.2018 to overwrite limit on tensile strength if flag is activated
			tension = (*data_Plasticity)(3);  // LU 10.2018 to overwrite limit on tensile strength if flag is activated
		}                                     // LU 10.2018 to overwrite limit on tensile strength if flag is activated
	}

	void CSolidProperties::CalculateCoefficent_MOHRjoint(double ep, double scalar_comp, double scalar_tens) // WX:11.2010,
																											// 09.2011
	{
		int valid = 1;
		...
		tensionj = (*data_Plasticity_joint)(3);  // LU 10.2018 to overwrite limit on tensile strength if flag is activated
	...
	*******************************************************/
	...
	// LU WW: calculte prin. stress and direc. with Eigen library
	void CSolidProperties::CalPrinStrDir(double* stress, double* prin_str,
		double* prin_dir, int Dim)
	{
		double  eigv[9] = { 0. };
		int  j, ii, TmpValue1, TmpValue2, Tmp[3] = { 0 };
		...
		for (int i = 0; i < 3; i++)
		{
			Eigen::VectorXd const& v = eigensolver.eigenvectors().row(i);
			for (int j = 0; j < 3; j++)
				eigv[i * 3 + j] = v[j];
		}
		for (ii = 0; ii < 3; ii++)  // LU to reorder principal stresses
		{
			for (j = ii + 1; j < 3; j++)
			{
				if (prin_str[ii] < prin_str[j])
				{
					TmpValue1 = prin_str[ii];
					prin_str[ii] = prin_str[j];
		...
		for (int i = 0; i < 3; i++)  // LU to reorder eigenvectors
		{

			for (int j = 0; j < 3; j++)
				prin_dir[j * 3 + i] = eigv[j * 3 + Tmp[i]];
			//prin_dir[i * 3 + j] = v[j];
		}
	}
	// WX: calculate transform matrix between normal stress and principal stress
	// LU: calculation based on Reuter's matrix and pseudovector computations ?
