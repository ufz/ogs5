...
int REACT_PRQ::isKey(std::string pies0)
{
	...
	const std::string PHREEQC_KEYWORD[8]
	    = {"SOLUTION", "EQUILIBRIUM_PHASES", "KINETICS", "PRINT", "SELECTED_OUTPUT", "USER_PUNCH", "END", "RATES"};
	for (i = 0; i < 8; i++)
	{
...
void REACT_PRQ::SetInterface(void)
{
	...
	{
		cout << right << setw(7) << id_key[i] << setw(8) << idx_key[i] << setw(3) << phrq_id[i] << setw(7)
		     << phrq_id_pos[i] << setw(7) << phrq_id_pcs[i] << "\n";
	}
}
	...
	{ // ii==0 as boundary point without reaction calc
		...
		if (m_rei)
		{
			if (m_rei->unitconversion)
			{
				...
				// unitfactor_s = (1 - m_rei->node_porosity[ii]) * MOLH2OPERKG / (m_rei->water_conc[ii] *
				// m_rei->node_porosity[ii] * m_rei->GetWaterSaturation(ii));
				// if(unitfactor_s ==0) unitfactor_s = (1 - m_rei->node_porosity[ii]) * MOLH2OPERKG /
