...
CInitialCondition::CInitialCondition(const InitialCondition* ic)
    : ProcessInfo(ic->getProcessType(), ic->getProcessPrimaryVariable(), NULL),
      GeoInfo(ic->getGeoType(), ic->getGeoObj()), DistributionInfo(ic->getProcessDistributionType())
{
	...
	while (!new_keyword)
	{
		...
		if (line_string.find("$PRIMARY_VARIABLE") != string::npos)
		{
			...
			if (this->getProcessType() == FiniteElement::MASS_TRANSPORT)
			{
				...
				{
					DisplayErrorMsg(
					    "Error: In reading IC file, the input component names are not found in MCP file!!!");
					exit(1);
				}
			}
			else
		...
		if (line_string.find("$GEO_TYPE") != string::npos)
		{
			...
			if (geo_type_name.find("POINT") != string::npos)
			{
				...
				if (pnt == NULL)
				{
					std::cerr << "error in CInitialCondition::Read: point name \"" << geo_name << "\" not found!"
					          << "\n";
					exit(1);
				}
			...
			if (geo_type_name.find("POLYLINE") != string::npos)
			{
				...
				if (ply == NULL)
				{
					std::cerr << "error in CInitialCondition::Read: polyline name \"" << geo_name << "\" not found!"
					          << "\n";
					exit(1);
				}
			...
			if (geo_type_name.find("SURFACE") != string::npos)
			{
				...
				in >> geo_name;
				GEOLIB::Surface const* sfc(geo_obj.getSurfaceVecObj(unique_geo_name)->getElementByName(geo_name));
				setGeoObj(sfc);
				if (sfc == NULL)
				{
					std::cerr << "ERROR: CInitialCondition::Read: surface \"" << geo_name << "\" not found!"
					          << "\n";
					exit(1);
				}
			}
...
void CInitialCondition::SetPoint(int nidx)
{
	if (m_msh && this->getProcessDistributionType() == FiniteElement::CONSTANT)
		this->getProcess()->SetNodeValue(
		    this->getProcess()->m_msh->GetNODOnPNT(static_cast<const GEOLIB::Point*>(getGeoObj())),
		    nidx,
		    geo_node_value);
	else
		std::cerr << "Error in CInitialCondition::SetPoint - point: "
		          << *(static_cast<const GEOLIB::Point*>(getGeoObj())) << " not found"
		          << "\n";
}
...
void CInitialCondition::SetPolyline(int nidx)
{
	if (this->getProcessDistributionType() == FiniteElement::CONSTANT)
	{
		...
		if (!m_polyline)
		{
			std::cout << "Error in CInitialCondition::SetPolyline - CGLPolyline: " << geo_name << " not found"
			          << "\n";
		}
		if (!getGeoObj())
		{
			std::cout << "Error in CInitialCondition::SetPolyline - Polyline: " << geo_name << " not found"
			          << "\n";
			return;
		}
		...
		m_msh->GetNODOnPLY(static_cast<const GEOLIB::Polyline*>(getGeoObj()), nodes_vector, automatic,
		                   m_polyline->epsilon);
...
#ifdef DEBUGMESHNODESEARCH
		{
			...
			{
				debug_out << k << " " << GEOLIB::Point((m_msh->getNodeVector())[nodes_vector[k]]->getData())
				          << " $NAME " << nodes_vector[k] << "\n";
			}
			debug_out << "#STOP"
			          << "\n";
...
void CInitialCondition::SetSurface(int nidx)
{
	...
	if (m_sfc && m_msh)
	{
		...
		GEOLIB::Surface const* sfc(static_cast<const GEOLIB::Surface*>(getGeoObj()));
		if (sfc == NULL)
		{
...
#ifdef DEBUGMESHNODESEARCH
		{
			...
			for (size_t k(0); k < msh_nod_vec.size(); k++)
			{
				debug_out << k << " " << GEOLIB::Point((m_msh->getNodeVector())[msh_nod_vec[k]]->getData()) << " $NAME "
				          << msh_nod_vec[k] << "\n";
			}
			debug_out << "#STOP"
			          << "\n";
		...
		if (this->getProcessDistributionType() == FiniteElement::CONSTANT)
			for (size_t i = 0; i < sfc_nod_vector.size(); i++)
				this->getProcess()->SetNodeValue(sfc_nod_vector[i], nidx, geo_node_value);
		// end surface nodes
		...
		else if (this->getProcessDistributionType() == FiniteElement::GRADIENT)
		{
			...
			for (size_t i = 0; i < sfc_nod_vector.size(); i++)
			{
				...
				else
				{
					cout << "Error in CInitialCondition::SetSurface - dis_type: "
					     << convertDisTypeToString(this->getProcessDistributionType()) << "don't know If 2D or 3D"
					     << "\n";
					node_depth = 0;
				}
			...
			cout << "Error in CInitialCondition::SetSurface - dis_type: "
			     << convertDisTypeToString(this->getProcessDistributionType()) << " not found"
			     << "\n";
	} // end m_sfc
	else
...
void CInitialCondition::SetDomain(int nidx)
{
	...
	if (SubNumber == 0)
	{
		...
		if (this->getProcessDistributionType() == FiniteElement::GRADIENT)
			...
			for (i = 0; i < m_msh->GetNodesNumber(true); i++)
			{
				...
				node_val = gradient_ref_depth_gradient * (gradient_ref_depth - node_depth) + gradient_ref_depth_value;
				this->getProcess()->SetNodeValue(m_msh->nod_vector[i]->GetIndex(), nidx, node_val);
			}
	...
	else // WW
	{
		...
		{
			if (convertPrimaryVariableToString(this->getProcessPrimaryVariable()).find("DISPLACEMENT")
			    != string::npos) // Deform
				quadratic = true;
			else
				quadratic = false;
			...
			if (this->getProcessDistributionType() == FiniteElement::GRADIENT)
			{
				if (k == 0) // TEST for DECOVALEX
					...
					for (i = 0; i < nodes_vector.size(); i++)
						this->getProcess()->SetNodeValue(nodes_vector[i], nidx, subdom_ic[k]);
				else
					for (i = 0; i < nodes_vector.size(); i++)
					{
						if (onZ == 1) // 2D
						...
						this->getProcess()->SetNodeValue(nodes_vector[i], nidx, node_val);
					}
			}
				...
				for (i = 0; i < nodes_vector.size(); i++)
				{
					...
					//                thisNode->X(),thisNode->Y(), thisNode->Z()));
					double const* const pnt(m_msh->nod_vector[nodes_vector[i]]->getData());
					getProcess()->SetNodeValue(nodes_vector[i], nidx,
...
{
	string variable_name = "DELTA_" + convertPrimaryVariableToString(this->getProcessPrimaryVariable());
