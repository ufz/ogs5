	...
	while (!new_keyword)
	{
		...
		if (line_string.find("$GEO_TYPE") != string::npos)
		{
			...
			if (geo_type_name.find("DOMAIN") != string::npos)
			{
				...
				{
					*ic_file >> SubNumber;
					if (convertPrimaryVariableToString(this->
					...
					{
						// 24.08.2011. WW
						dis_linear_f = new LinearFunctionData(*ic_file,
						                                      SubNumber);
...
void CInitialCondition::SetPoint(int nidx)
{
	if (m_msh && this->getProcessDistributionType() == FiniteElement::CONSTANT)
		this->getProcess()->SetNodeValue(this->getProcess()->m_msh->GetNODOnPNT(static_cast
		                                                                        <const
		                                                                         GEOLIB::
		                                                                         Point*>(
		                                                                                getGeoObj(
	...
	if(m_sfc && m_msh)
	{
		...
		if(this->getProcessDistributionType() == FiniteElement::CONSTANT)
			for(size_t i = 0; i < sfc_nod_vector.size(); i++)
				this->getProcess()->SetNodeValue(sfc_nod_vector[i],
				                                 nidx,
				                                 geo_node_value);
		// end surface nodes
