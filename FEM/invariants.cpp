...
double CalEffectiveStress(const KVec& dev_stress)
{
    double s(0.);
    s = 3. *
        CalJ2(dev_stress);  // Kelvin mapped deviatoric stress has to be used
    return sqrt(s);
}
...
{
    Eigen::Matrix<double, 3, 3> tens;
    tens = KelvinVectorToTensor(
        dev_vec);  // TN: Not strictly necessary. Can be written explicitly for
                   // vector coordinates
...
double CalLodeAngle(const KVec& dev_vec)
{
    ...
    thetaR = std::min(std::max(theta, -1. + DBL_EPSILON),
                      1. - DBL_EPSILON);  // strict limits for trigonometric
                                          // functions
