...
//#define CountParticleNumber                            //YS: to count the
//number of particles leave the domain
...
RandomWalk::RandomWalk(int srand_seed)
{
    ...
    numOfParticles = 0;
    leavingParticles =
        0;                // YS: to count the number of particles in the outflow
...
void RandomWalk::InterpolateVelocity(Particle* A)
{
    ...
    //Fluid_Momentum 		if(
    //m_pcs->pcs_type_name.find("RICHARDS_FLOW")!=string::npos) TF 			m_msh =
    //FEMGet("RICHARDS_FLOW"); 		else if( m_pcs->getProcessType () == LIQUID_FLOW)
    ...
    */
    ...
    if (nnode == 4)
    {
        ...
        for (int i = 0; i < nnode; ++i)
        {
            ...
            // vx[i] =
            // m_pcs->GetNodeValue(theEle->GetNodeIndex(i),idx)/porosity;
            // //FM_TEST vy[i] =
            // m_pcs->GetNodeValue(theEle->GetNodeIndex(i),idy)/porosity; vz[i]
            // = m_pcs->GetNodeValue(theEle->GetNodeIndex(i),idz)/porosity;
        }
        ...
        for (int i = 0; i < 4; ++i)
        {
            ...
            // theNodesOfThisEdge[0]->Y(); TF        node2[0] =
            // theNodesOfThisEdge[1]->X(); node2[1] =
            // theNodesOfThisEdge[1]->Y(); Get the referece position of these
...
void RandomWalk::TracePathlineInThisElement(Particle* A)
{
    ...
    //		// Select the mesh whose process name has the mesh for
    //Fluid_Momentum 		if(
    //m_pcs->pcs_type_name.find("RICHARDS_FLOW")!=string::npos) 			m_msh =
    //FEMGet("RICHARDS_FLOW"); 		else if(
    //m_pcs->pcs_type_name.find("LIQUID_FLOW")!=string::npos) 			m_msh =
    //FEMGet("LIQUID_FLOW"); 		else if(
    //m_pcs->pcs_type_name.find("GROUNDWATER_FLOW")!=string::npos) 			m_msh =
    //FEMGet("GROUNDWATER_FLOW");
    //	}
    ...
    /*   //WW
       ...
       porosity = MediaProp->porosity;             // This is for simple one.
                                                   ...
                                                   // This will get you
       porosity. porosity = MediaProp->porosity_model_values[0];
     */
...
void RandomWalk::InterpolateVelocityOfTheParticleByInverseDistance(Particle* A)
{
    ...
    for (int i = 0; i < nnodes; ++i)
    {
        ...
        else
        {
            ...
            // Flow. Thus, I only divide Darcy velocity by porosity only to get
            // pore velocity.
            CMediumProperties* MediaProp = mmp_vector[m_ele->GetPatchIndex()];
            double porosity = 0.0;
            // This will get you porosity.
                                                            ...
                                                            Particle* A)
{
    ...
    else if (option == 1)  // miniFEM Way
    {
        ...
        // at the corner vertix Do not do anything. If not, then proceed. Check
        // if the particle is on the edge or node.
        ...
        if (eleIndex != -10 && IsOnTheEdge == 0)
        {
            ...
            */
            ...
            for(int i=0; i<nnode; ++i)
            {
                ...
                vx[i] =
            m_pcs->GetNodeValue(theEle->GetNodeIndex(i),idx)/porosity; //FM_TEST
                vy[i] =
            m_pcs->GetNodeValue(theEle->GetNodeIndex(i),idy)/porosity; vz[i] =
            m_pcs->GetNodeValue(theEle->GetNodeIndex(i),idz)/porosity;
            }
...
// Todo
                    ...
                    x = new double[size];
                    m_pcs->EQSSolver(
                        x);  // an option added to tell FLUID_MOMENTUM for
        ...
        else if (IsOnTheEdge != 0)  // The particle is on the edge.
        {
            ...
            // edge. Even for this, I will use 1d Galerkin method.
        }
        // else
        //	;	// Think later for out of boundary particles.
    }
        ...
        if (eleIndex != -10)
        {
            ...
            */
            ...
            for (int i = 0; i < 4; ++i)
            {
                ...
                // = theNodesOfThisEdge[0]->Y(); TF           node2[0] =
                // theNodesOfThisEdge[1]->X(); node2[1] =
                // theNodesOfThisEdge[1]->Y(); Get the referece position of
                // these two ending points of the edge
                IsoparametricMappingQuadfromPtoR(A->elementIndex, node1);
                IsoparametricMappingQuadfromPtoR(A->elementIndex, node2);
                                                     ...
                                                     double* P2, int axis)
{
    ...
    for (int i = 0; i < nnodes; ++i)
    {
        ...
        for (int j = 0; j < 2; ++j)
        {
            ...
            int status = G_intersect_line_segments(p1[0], p1[1], p2[0], p2[1],
                                                   p3[0], p3[1], p4[0], p4[1],
                                                   &ra, &rb, &x, &y);
                                                             ...
                                                             double* dD)
{
    ...
    TensorOfdD[0] = V[0] * A->dVxdx *
                    (alphaL * (2.0 / U - V[0] * V[0] / (U * U * U)) -
    ...
    TensorOfdD[4] = V[1] * A->dVydy *
                    (alphaL * (2.0 / U - V[1] * V[1] / (U * U * U)) -
    ...
    TensorOfdD[8] = V[2] * A->dVzdz *
                    (alphaL * (2.0 / U - V[2] * V[2] / (U * U * U)) -
...
int RandomWalk::SolveForDerivativeOfVelocity(Particle* A)
{
    ...
    else if (m_ele->GetDimension() == 2)
    {
        ...
        // treated in true 3D. Yes. Velocity should be on the XY plane
        double Vx[3];
        Vx[0] = XR.Vx - XL.Vx;
        Vx[1] = XR.Vy - XL.Vy;
        Vx[2] = XR.Vz - XL.Vz;
...
void RandomWalk::TraceStreamline(void)
{
    ...
    // step. Should combine with the picking...
    CWnd* pWnd = NULL;
...
#ifdef _OPENMP
    std::cout
        << "[RandomWalk::AdvanceToNextTimeStep] OpenMP parallelized loop ... "
        << std::flush;
#else
...
#pragma omp parallel for
    ...
    for (int i = 0; i < numOfParticles; ++i)
    {
        ...
        if (m_cp->mobil && TimeMobility > 0)
        {
            ...
            if (X[i].Now.elementIndex != -10 && X[i].Now.identity != 2)
            {
                ...
                // outpus and Paraview RecordPath(i, &Y);
                ...
                do
                {
                    ...
                    if (Astatus == -1)
                        Y.t = dt;
                    // YS: absorbed and filtered particles don't move, but
                    // absorbed particles can be desorpted
        ...
        if (m_pcs->rwpt_app == 1)  // Is the application is Cell Dynamics?
        {
            /*
               ...
               particles double ChanceOfMobile = randomZeroToOne(); double
               ChanceOfImmobile = randomZeroToOne();
               ...
               {
               if(numOfComps > 2)	// If the number of components is bigger than
               2, X[i].Now.identity = 2;	// Make another kind of immobile
               else
               ;
               }
               else
                 ...
                 2)  // Is the application Cryptosporidium oocysts?
        {
            if (X[i].Now.elementIndex != -10 && X[i].Now.identity != 2)
            {
                ...
                // chemicals decayed C/C0 = exp(-kt) For oocyst irreversible
                // filtration, k = vp * lambda
    ...
    stop = clock();
    std::cout << " took " << (stop - start) / (double)(CLOCKS_PER_SEC)
              << " seconds" << std::endl;
}
...
void RandomWalk::RandomWalkOutput(double dbl_time, int current_time_step)
{
    ...
    for (size_t i = 0; i < rwpt_out_strings.size(); i++)
    {
        ...
        if (outputornot)
        {
            for (size_t j = 0; j < out->_rwpt_string_vector.size(); j++)
            {
                ...
                if (out->_rwpt_string_vector[j].compare(
                        "CONTROL_PLANE_NORMAL_X") == 0)
                {
                    for (size_t k = 0;
                         k < out->_control_plane_x_normal_vector.size(); k++)
                    {
                        DATWriteParticleControlPlaneFile(
                            current_time_step, "CONTROL_PLANE_NORMAL_X",
                            out->_control_plane_x_normal_vector[k]);
                    }
                }
                if (out->_rwpt_string_vector[j].compare(
                        "CONTROL_PLANE_NORMAL_Y") == 0)
                {
                    for (size_t k = 0;
                         k < out->_control_plane_y_normal_vector.size(); k++)
                    {
                        DATWriteParticleControlPlaneFile(
                            current_time_step, "CONTROL_PLANE_NORMAL_Y",
                            out->_control_plane_y_normal_vector[k]);
                    }
                }
                if (out->_rwpt_string_vector[j].compare(
                        "CONTROL_PLANE_NORMAL_Z") == 0)
                {
                    for (size_t k = 0;
                         k < out->_control_plane_z_normal_vector.size(); k++)
                    {
                        DATWriteParticleControlPlaneFile(
                            current_time_step, "CONTROL_PLANE_NORMAL_Z",
                            out->_control_plane_z_normal_vector[k]);
                    }
                }
            }
        ...
        /*
                ...
                // DATWriteParticleConcFile(current_time_step); // routine not
           yet configured
                {
                    for (size_t j = 0; j < no_times; j++)
                        if (CurrentTime >= out->getRWPTTimeVector()[j])
                        {
                            ...
                            // else
           if(current_name.compare("PARTICLE_CONCENTRATION")==0)
                            // DATWriteParticleConcFile(current_time_step); //
           routine not yet configured (see commented text
                            // below)

                            out->getRWPTTimeVector().erase(out->getRWPTTimeVector().begin()
           + j); break;
                        }
                }*/
    }
    ...
    /*
       ...
       (double)CountInThisElement/numOfParticles; SetElementValue(i,
       GetElementValueIndex("CONCENTRATION0")+1, NormConcentrationOfTheElement);
       }
     */
...
void RandomWalk::SolveDispersionCoefficient(Particle* A)
{
    ...
    // Later....
    if (m_ele->GetDimension() < 3 &&
        (m_ele->GetDimension() != m_msh->GetMaxElementDim()))
...
void RandomWalk::CheckBoundary2D(Particle* A, Particle* B)
{
    ...
    // If the particle is out of domain, find the intersection point between the
    // path line and the boundary. But check the old position first, in case it
    ...
    else
    {
        ...
        for (size_t j = 0; j < m_msh->face_vector.size(); j++)
        {
            ...
            xx = yy = ua = ub = 0.;
            if (G_intersect_line_segments(p1[0], p1[1], p2[0], p2[1], p3[0],
                                          p3[1], p4[0], p4[1], &ua, &ub, &xx,
                                          &yy) == 1)
            {
                if (ua >= 0.0 && ua <= 1.0)
                {
...
the intersection point is in the surface inside the triangle spanned by the
three points if and only if 0<miu,ita,lambda<1 and miu+ita<=1
...
void RandomWalk::CheckBoundary3D(Particle* A, Particle* B)
{
    ...
    else
    {
        ...
        for (int j = 0; j < (long)m_msh->face_vector.size(); j++)
        {
            ...
            ntr = 1;
            // if the element facet is quadrilateral, divide it into two
            // triangles
            ...
            for (itr = 0; itr < ntr; itr++)
            {
                ...
                {
                    a_node =
                        n_ele->GetNode(0);  // point r on the boundary plane
                    x1[0] = a_node->X();
                    x1[1] = a_node->Y();
                    x1[2] = a_node->Z();
                    a_node =
                        n_ele->GetNode(1);  // point s on the boundary plane
                    x2[0] = a_node->X();
                    x2[1] = a_node->Y();
                    x2[2] = a_node->Z();
                    a_node =
                        n_ele->GetNode(2);  // point t on the boundary plane
                    x3[0] = a_node->X();
                    x3[1] = a_node->Y();
                    x3[2] = a_node->Z();
                }
                else
                {
                    ...
                    x1[2] = a_node->Z();
                    a_node =
                        n_ele->GetNode(2);  // point s on the boundary plane
                    x2[0] = a_node->X();
                    x2[1] = a_node->Y();
                    x2[2] = a_node->Z();
                    a_node =
                        n_ele->GetNode(3);  // point t on the boundary plane
                    x3[0] = a_node->X();
                    x3[1] = a_node->Y();
                    x3[2] = a_node->Z();
                }
            ...
            for (size_t j = 0; j < theNeighbor->GetFacesNumber(); ++j)
            {
                ...
                if (theNNeighbor->GetDimension() == theNeighbor->GetDimension())
                {
                    ...
                    for (size_t k = 0; k < theNNeighbor->GetFacesNumber(); ++k)
                    {
                        ...
                        if (theNNNeighbor->GetDimension() ==
                            theNNeighbor->GetDimension())
                        {
                            ...
                            // Try one more time, the neighbor's neighbor's
                            // neighbor's neighbor
                            for (size_t l = 0;
                                 l < theNNNeighbor->GetFacesNumber(); ++l)
                            {
                                CElem* theNNNNeighbor =
                                    theNNNeighbor->GetNeighbor(l);

                                if (theNNNNeighbor->GetDimension() ==
                                    theNNNeighbor->GetDimension())
                                {
                                    A->elementIndex =
                                        theNNNNeighbor->GetIndex();
...
//---------------------------------------------------------------------------
/*
  Task: find out if the particle position (*xyz) belongs to current element
  (indicated by A->elementIndex) Programing: 11/2011 YS/WW Implementation
*/
...
int RandomWalk::SolveForNextPosition(Particle* A, Particle* B)
{
    ...
    if (ele_dim < 3)
    {
        ...
        for (int i = 0; i < nEdges; ++i)
        {
            ...
            double X1_4XZ[3] = {theNodes[0]->getData()[0],
                                theNodes[0]->getData()[2],
                                theNodes[0]->getData()[1]};
            double X2_4XZ[3] = {theNodes[1]->getData()[0],
                                theNodes[1]->getData()[2],
                                theNodes[1]->getData()[1]};
            //         X1[0] = theNodes[0]->X(); X1[1] = theNodes[0]->Y(); X1[2]
            //         = theNodes[0]->Z(); X2[0] = theNodes[1]->X(); X2[1] =
            //         theNodes[1]->Y(); X2[2] = theNodes[1]->Z();
            ...
            if (dDStatus == 1)
            {
                if (ele_dim == 2)
                {
                    ...
                    // transformed to be on the XY plane. dD[] should be fine
                    // because it is handled in the
                    ...
                    // SolveDispersionCoefficient function. OK. Just velocity
                    // left. In fact, V[2] gotta be zero.
                    V[0] = B->Vx;
                    V[1] = B->Vy;
                    V[2] = B->Vz;
                    // If 2D-XZ Model
        ...
        if (countNoIntersection == nEdges)
        {
            if (ele_dim == 2)
            {
                ...
                // element in this if condition should be one of the connected
                // planes randomly chosen. Now just solve the real plane
                // coordinates for the particle at the next position.
                double P[3];
                P[0] = B->x;
                P[1] = B->y;
                P[2] = B->z;
...
//         theNodesOfThisEdge[0]->Y(); X1[2] = theNodesOfThisEdge[0]->Z(); X2[0]
//         = theNodesOfThisEdge[1]->X(); X2[1] = theNodesOfThisEdge[1]->Y();
            ...
            if (thisNeighbor->GetElementType() != MshElemType::LINE)
            {
                ...
                for (size_t j = 0; j < thisNeighbor->GetFacesNumber(); ++j)
                {
                        ...
                        MshElemType::LINE)
                    {
                        ...
                        // Third, search the neighbor's neighbor's neighbor
                        for (size_t k = 0;
                             k < theNeighborsNeighbor->GetFacesNumber(); ++k)
                        {
...
physical coordinates. For bilinear elements such as quadrilateral elements,
...
reference coordinates. For bilinear elements such as quadrilateral elements,
    ...
    for (k = 0; k < nz; k++)  // loop over the dummy element set
    {
        for (j = 0; j < ny; j++)
            for (i = 0; i < nx; i++)
            {
                ...
                for (iel = 0; iel < nels;
                     iel++)  // loop over mesh elements, assign them to dummy
                             // elements
                {
                    double const* center =
...
void PCTRead(string file_base_name)
{
    ...
    while (End)
    {
        ...
        for (int i = 0; i < RW->numOfParticles; ++i)
        {
            ...
            double x = 0.0, y = 0.0, z = 0.0, Starting = 0, vx = 0.0, vy = 0.0,
                   vz = 0.0, K = 0.0;
...
void DATWriteParticleControlPlaneFile(int current_time_step,
                                      string control_plane_name,
                                      double normal_value)
{
    CFEMesh* m_msh = NULL;
    RandomWalk* RW = NULL;
    ...
    string vtk_file_name =
        pathJoin(defaultOutputPath, pathBasename(FileName)) + "_RWPT_";
    ...
    if (!vtk_file.good())
        return;
    int pos = -1;
    vtk_file.seekg(0, vtk_file.end);
    pos = vtk_file.tellg();

    // Write Header
    if (pos == 0)
        vtk_file << "id, starting_time,  arrival_time_step_time, "
                    "arrival-time-cp , start_coor_x, start_coor_y, "
                    "start_coor_z, control_plane_x, control_plane_y, "
                    "control_plane_z, time_correction_factor, x_back_shift "
                 << endl;

    double starting_time = 0.0;
    double time_correction = 0.0;
    double arrival_time_step_time = 0.0;
    double arrival_time_control_plane = 0.0;
    double back_shift = 0.0;
    double start_coor[3];

    // Write particle locations on plane
    for (int i = 0; i < np; ++i)
    {
        starting_time = RW->X[i].Now.StartingTime;
        arrival_time_step_time = RW->X[i].Now.t;
        start_coor[0] = RW->X[i].Now.x_start;
        start_coor[1] = RW->X[i].Now.y_start;
        start_coor[2] = RW->X[i].Now.z_start;

        // Cross X-Normal-Plane
        if (control_plane_name.compare("CONTROL_PLANE_NORMAL_X") == 0)
        {
            if ((RW->X[i].Past.x < normal_value &&
                 RW->X[i].Now.x > normal_value) ||
                (RW->X[i].Past.x > normal_value &&
                 RW->X[i].Now.x < normal_value))
            {
                back_shift = RW->X[i].Now.x - normal_value;
                if (RW->X[i].Now.Vx > 0.0)
                    time_correction = back_shift / (RW->X[i].Now.Vx);
                else
                    time_correction = 0.0;
                arrival_time_control_plane =
                    arrival_time_step_time - time_correction;
                vtk_file << i << ", " << starting_time << ", "
                         << arrival_time_step_time << ", "
                         << arrival_time_control_plane << ", " << start_coor[0]
                         << ", " << start_coor[1] << ", " << start_coor[1]
                         << ", " << normal_value << ", " << RW->X[i].Now.y
                         << ", " << RW->X[i].Now.z << ", " << time_correction
                         << ", " << back_shift << "\n";
            }
        }
        if (control_plane_name.compare("CONTROL_PLANE_NORMAL_Y") == 0)
        {
            if ((RW->X[i].Past.y < normal_value &&
                 RW->X[i].Now.y > normal_value) ||
                (RW->X[i].Past.y > normal_value &&
                 RW->X[i].Now.y < normal_value))
            {
                back_shift = RW->X[i].Now.y - normal_value;
                if (RW->X[i].Now.Vy > 0.0)
                    time_correction = back_shift / (RW->X[i].Now.Vy);
                else
                    time_correction = 0.0;
                arrival_time_control_plane =
                    arrival_time_step_time - time_correction;
        ...
        {
            if ((RW->X[i].Past.z < normal_value &&
                 RW->X[i].Now.z > normal_value) ||
                (RW->X[i].Past.z > normal_value &&
                 RW->X[i].Now.z < normal_value))
            {
                back_shift = RW->X[i].Now.z - normal_value;
                if (RW->X[i].Now.Vz > 0.0)
                    time_correction = back_shift / (RW->X[i].Now.Vz);
                else
                    time_correction = 0.0;
                arrival_time_control_plane =
                    arrival_time_step_time - time_correction;
...
bool WriteDatasetOf_RWPT_PVD(std::fstream& fin, double timestep,
...
void DATWriteParticleVTPFile(int current_time_step)
{
        ...
        for (std::size_t t = 0; t < time_vector.size(); ++t)
        {
            ...
            if (m_tim->pcs_type_name.compare("RANDOM_WALK") == 0)
            {
                if (current_time_step > 0.0 && i > 0)
                    ...
                    time_past = 0.0;
                WriteDatasetOf_RWPT_PVD(
                    fin, time_past,
                    pathJoin(defaultOutputPath, pathBasename(FileName)) +
                        "_RWPT_particles" + istr.data() + ".vtp");
            }
        }
    }
    ...
    fin.close();
    // End writing pvd-file. PVD needs to be rewritten for each time step. Same
    // procedure as for the VTU output

    // Write VTP Header
    vtk_file << "<!--Time step: " << current_time_step << " -->" << endl;
    vtk_file << "<VTKFile type=\"PolyData\" version=\"1.0\" "
    ...
    // Write PointData
    vtk_file << "   <PointData Scalars =\"identity\" Vectors =\"velocity\">"
             << endl;
    ...
    // Write particle identities
    vtk_file << "    <DataArray type = \"Int32\" Name = \"identity\" format = "
                "\"ascii \" ";
    ...
    // Write particle on_boundary or not
    vtk_file << "    <DataArray type = \"Int32\" Name = \"on_boundary\" format "
                "= \"ascii \" ";
    ...
    vtk_file << "    <DataArray type = \"Float64\" Name = \"velocity\" "
    ...
    vtk_file << "    <DataArray type = \"Float64\" Name = \"Points\" "
                "NumberOfComponents=\"3\" format = \"ascii \" ";
    vtk_file << "RangeMin = \"" << xyz_coord_min << "\" ";
    vtk_file << "RangeMax = \"" << xyz_coord_max << "\"> " << endl;
    ...
    // Write offset
    vtk_file << "    <DataArray type = \"Int64\" Name = \"offsets\" format = "
                "\"ascii \" ";
    vtk_file << "RangeMin =\"1\" ";
