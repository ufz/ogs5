...
/* New class KinReaction: contains the kinetic reactions and all necessary data
 * structures for them */
...
public:
    ...
    int isotopecouplenumber;  // CB isotope fractionation : specis number of
                              // isotope partner
...
private:
    ...
    friend void KRConfig(const GEOLIB::GEOObjects& geo_obj,
...
public:
    ...
    int number;                 /* counter */
    int number_reactionpartner; /* Number of chemical species involved in
                                   reaction */
    std::vector<std::string>
        reactionpartner; /* all names of reaction partners stored here */
    std::vector<double> stochmet; /* stochiometric coefficients for each
                                     reactionpartner stored here */
    ...
    std::vector<MonodSubstruct*>
        monod; /* saves monod concentrations and names of species */
    std::vector<MonodSubstruct*>
        inhibit; /* saves inhibit concentrations and names of species */
    std::vector<MonodSubstruct*> production; /* saves production concentrations,
                                                orders and names of species */
    ...
    long currentnode;  // CB 19/10/09 This is eclusively for Brand model to
                       // allow porosity in Inhibition constant
    // calculation
    ...
    double current_Csat;   /* current solubility after considering Roult's law,
    ...
    int typeflag_exchange_linear;   /* set to 1 if reaction is linear exchange
                                       type */
    int typeflag_exchange_langmuir; /* set to 1 if reaction is langmuir exchange
                                       type */
    int typeflag_exchange_freundlich; /* set to 1 if reaction is freundlich
                                         exchange type */
    ...
    std::vector<double> Current_Csat; /* initial interfacial area, interfacial
                                         area of last iteration */
    // Mineral kinetics data
    std::vector<MinkinMech*>
        mechvec;  // stores individual mechanisms of this reaction
    std::vector<int>
        minSpeciesIdx;  // stores cp vector index of mineral species
    std::vector<double>
        Am;  // initial specific reactive mineral surface area m²/m³ aquifer =
             // Am * rho_b = m²/kg * kg/m³aquifer
     ...
     */
    bool Read(std::ifstream* in, const GEOLIB::GEOObjects& geo_obj,
              const std::string& unique_name);

    void Write(std::ofstream*); /* Class Write Function */
    void ReadReactionEquation(
        std::string); /* Read function for chemical equations */
...
public:
    ...
    std::vector<double>
        Masstransfer_K; /* CB: current Mass transfer coefficient, now saved in
                           node vector*/
...
public:
    ...
    // output flag
    bool testoutput;
    // index vector for shortening vectors c in kinetic calculations (omitting
    // nonreacting species)
    //	vector <int> sp_index;
    ...
    std::vector<double>
        React_dCdT;  // Sum of reaction rates for individual nodes
    ...
    bool copy_concentrations;  // SB 01.2011 Flag for copying concentrations in
                               // radial models to save reaction simulation
    // time
    ...
    bool scale_dcdt;  // stabilization for ODE solver in derivs, sclae dcdt
                      // vector
    bool lagneau;  // special configuration of minrate and porosity update for
                   // Lagneaubenchmark
    bool sortnodes;
    long* substeps;
    long* noderanks;
    ...
    std::vector<std::vector<double> >
        ActivityCoefficients;  //  For Mineral Dissolution, store gammas in 1
                               //  data object
    int activity_model;
    ...
    void Biodegradation(double* m_Conc, long node, double eps, double hmin,
                        double* usedtneu, int* nok, int* nbad, int tt,
                        int tsteps);
    ...
    void Calc_linearized_rates(double* m_Conc, long Number_of_Components,
...
extern std::vector<CKinReact*>
    KinReact_vector;  // declare extern instance of class CKinReact
// declare extern instance of class CKinReact
extern std::vector<CKinReactData*> KinReactData_vector;
extern std::vector<CKinBlob*>
    KinBlob_vector;  // declare extern instance of class Blob

// CB _drmc_
extern std::vector<MicrobeData*>
 ...
 * @param unique_name unique name to access the geometric entities in geo_obj
 */
void KRConfig(const GEOLIB::GEOObjects& geo_obj,
              const std::string& unique_name);
...
extern void KNaplCalcDensity(
    void); /* CB check if NAPL dissolution is modeled */
...
extern bool stifbs(double y[], double dydx[], int nv, double* xx, double htry,
                   double eps, double yscal[], double* hdid, double* hnext,
                   void (*derivs)(double, double[], double[], int, long,
                                  double),
                   long);

extern bool rkqs(double y[], double dydx[], int n, double* x, double htry,
                 double eps, double yscal[], double* hdid, double* hnext,
                 void (*derivs)(double, double[], double[], int, long, double),
                 long);
