    ...
    switch (method)
    {
        ...
        case 1:
            ...
            for (ii = 0; ii < pcs_number_of_primary_nvals; ii++)
            {
                ...
                {
                    val = eqs_x[pcs_number_of_primary_nvals *
            ...
            for (ii = 0; ii < pcs_number_of_primary_nvals; ii++)
            {
                ...
                for (i = 0l; i < g_nnodes; i++)
                {
                    val = eqs_x[pcs_number_of_primary_nvals *
                                ...
                                ii];
                    error_l = max(
                        error_l, fabs(val - GetNodeValue(i, nidx1)) /
                                     (fabs(val) + fabs(GetNodeValue(i, nidx1)) +
                                      MKleinsteZahl));
                }
            }
            MPI_Allreduce(&error_l, &error, 1, MPI_DOUBLE, MPI_MAX,
        ...
        case 3:
            for (ii = 0; ii < pcs_number_of_primary_nvals; ii++)
            {
                ...
                for (i = 0l; i < g_nnodes; i++)
                {
                    ...
                    max_cl = max(max(max_cl, fabs(fabs(val))),
                                 fabs(GetNodeValue(i, nidx1)));
                }
            }
            MPI_Allreduce(&error_l, &error, 1, MPI_DOUBLE, MPI_MAX,
                          MPI_COMM_WORLD);
            MPI_Allreduce(&max_cl, &max_c, 1, MPI_DOUBLE, MPI_MAX,
                          MPI_COMM_WORLD);
        ...
        case 4:
            ...
            MPI_Allreduce(&max_cl, &max_c, 1, MPI_DOUBLE, MPI_MAX,
                          MPI_COMM_WORLD);
            MPI_Allreduce(&min_cl, &min_c, 1, MPI_DOUBLE, MPI_MIN,
                          MPI_COMM_WORLD);
            ...
            for (ii = 0; ii < pcs_number_of_primary_nvals; ii++)
            {
                ...
                for (i = 0l; i < g_nnodes; i++)
                {
                    val = eqs_x[pcs_number_of_primary_nvals *
                                ...
                                ii];
                    error_l = max(error_l,
                                  fabs(val - GetNodeValue(i, nidx1)) /
                                      (fabs(val - GetNodeValue(i, nidx1 - 1)) +
                                       MKleinsteZahl));
                }
            }
            MPI_Allreduce(&error_l, &error, 1, MPI_DOUBLE, MPI_MAX,
        ...
        case 6:
            for (ii = 0; ii < pcs_number_of_primary_nvals; ii++)
            {
                ...
                for (i = 0l; i < g_nnodes; i++)
                {
                    ...
                    error_l = max(error_l, fabs(val - GetNodeValue(i, nidx1)));
                    change_l =
                        max(change_l, fabs(val - GetNodeValue(i, nidx1 - 1)));
                }
            }
            MPI_Allreduce(&error_l, &error, 1, MPI_DOUBLE, MPI_MAX,
                          MPI_COMM_WORLD);
            MPI_Allreduce(&change_l, &change, 1, MPI_DOUBLE, MPI_MAX,
                          MPI_COMM_WORLD);
