...
public:
    ...
    // Drucker-Prager
    bool StressIntegrationDP(const int GPiGPj,
                             const FiniteElement::ElementValue_DM* ele_val,
                             double* TryStress, double& dPhi, const int Update);
    void ConsistentTangentialDP(Math_Group::Matrix* Dep, const double dPhi,
                                const int Dim);
    bool DirectStressIntegrationDP(
        const int GPiGPj,
        const FiniteElement::ElementValue_DM* ele_val,
        double* TryStress,
        const int Update);
    int DirectStressIntegrationDPwithTension(
        const int GPiGPj,
    ...
    void TangentialDPwithTensionCorner(Math_Group::Matrix* Dep,
                                       double mm);  // WX

    // Single yield surface model
    int CalStress_and_TangentialMatrix_SYS(
        ...
        const int Update);
    // Cam-clay model
    void CalStress_and_TangentialMatrix_CC(
        const int GPiGPj,
        const FiniteElement::ElementValue_DM* ele_val,
        double* dStrain,
    ...
    void setSolidReactiveSystem(
        FiniteElement::SolidReactiveSystem reactive_system);

    // general routine to get consistent tangent from local Newton iteration of
    // material functionals
    void ExtractConsistentTangent(const Eigen::MatrixXd& Jac,
                                  const Eigen::MatrixXd& dGdE,
                                  const bool pivoting,
                                  Eigen::Matrix<double, 6, 6>& dsigdE);
    // general local Newton routines to integrate inelastic material models
    void LocalNewtonBurgers(const double dt,
                            const std::vector<double>& strain_curr,
                            std::vector<double>& stress_curr,
                            std::vector<double>& strain_K_curr,
                            std::vector<double>& strain_M_curr,
                            Math_Group::Matrix& Consistent_Tangent,
                            double Temperature, double& local_res);
    void LocalNewtonMinkley(const double dt,
                            const std::vector<double>& strain_curr,
                            std::vector<double>& stress_curr,
                            std::vector<double>& eps_K_curr,
                            std::vector<double>& eps_M_curr,
                            std::vector<double>& eps_pl_curr, double& e_pl_v,
                            double& e_pl_eff, double& lam,
                            Math_Group::Matrix& Consistent_Tangent,
                            double Temperature, double& local_res);
...
private:
    ...
    Math_Group::Matrix*
        Crotm;  // If this is needed by permaebility calculation, we keep it.
                // Otherwise remove it. (To do, UJG/WW)
    ...
    Math_Group::Matrix*
        TransMicroStru;  // WX: 11.2011 micro structure tensor transform matrix
    ...
    // WX:09.2011
    double* Bedding_Norm;  // Norm vector of bedding plane, used for anisotropic
                           // elasto-plasticity
    ...
    int StressIntegrationHoekBrown(
        const int GPiGPj, const FiniteElement::ElementValue_DM* ele_val,
    ...
    void AccumulateEtr_HL_ODS(const FiniteElement::ElementValue_DM* ele_val,
                              const int nGS);
    ...
    int DirectStressIntegrationMOHR(const int GPiGPj,
                                    FiniteElement::ElementValue_DM* ele_val,
                                    double* TryStress, const int Update,
                                    Math_Group::Matrix* Dep, int itesteps);
    int StressIntegrationMOHR_Aniso(
        const int GPiGPj, const FiniteElement::ElementValue_DM* ele_val,
        double* TryStress, const int Update,
        Math_Group::Matrix* Dep);  // WX:12.2011 aniso mohr
    int StressIntegrationMOHR_Joint(const int GPiGPj, const FiniteElement::ElementValue_DM* ele_val, double* TryStress, 
                                    const int Update, Math_Group::Matrix* Dep); // LU:04.2019 shear on weakness plane
     double CalAnisoPara(
        double* Stress,
    ...
    void CalDep_l(double* vecl, double* veclg, Math_Group::Matrix* D,
                  Math_Group::Matrix* Dep_l, double fkt);
...
extern double TensorMutiplication2(const double* s1, const double* s2,
                                   const int Dim);
extern double TensorMutiplication3(const double* s1, const double* s2,
