...
BoundaryCondition::BoundaryCondition(const CBoundaryCondition& bc,
    ...
    : FEMCondition(geometry_name, bc.getProcessType(),
                   bc.getProcessPrimaryVariable(), bc.getGeoType(),
                   bc.getGeoName(), bc.getProcessDistributionType(),
                   FEMCondition::BOUNDARY_CONDITION)
{
                 ...
                 FiniteElement::LINEAR_NEUMANN)
    {
        ...
        for (size_t i = 0; i < dis_nodes.size(); i++)
            dis_nodes[i] = static_cast<size_t>(bc_nodes[i]);
