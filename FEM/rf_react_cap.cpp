...
**************************************************************************/
 ios::pos_type REACT_CAP::Read(ifstream *rfd_file, const GEOLIB::GEOObjects& geo_obj, const std::string& unique_name){
  ...
  while (!new_keyword) {
	...
	/* read keywords */
	  //....................................................................
    if(line_string.find("$CAP_MODE")!=string::npos) { // subkeyword found
	  in.str(GetLineFromFile1(rfd_file));
      in >> CAP_MODE; //sub_line
	  in.clear();
    }
    //....................................................................
    if(line_string.find("$DATA_FILE")!=string::npos) { // subkeyword found
	  in.str(GetLineFromFile1(rfd_file));
      in >> data_file; //sub_line
	  in.clear();
    }
    //....................................................................
    if(line_string.find("$DATA_FORMAT")!=string::npos) { // subkeyword found
	  in.str(GetLineFromFile1(rfd_file));
	...
	//....................................................................
    if(line_string.find("$MASS_NUM")!=string::npos) { // subkeyword found
	  in.str(GetLineFromFile1(rfd_file));
      in >> mass_num; //sub_line
	  in.clear();
    }
	//....................................................................
    if(line_string.find("$MASS_TYPE")!=string::npos) { // subkeyword found
	  in.str(GetLineFromFile1(rfd_file));
      in >> mass_type; //sub_line
	  in.clear();
    }
	//....................................................................
	if(line_string.find("$PHASE_LIST")!=string::npos){ // subkeyword found
	...
	//....................................................................
    if(line_string.find("$SPECIES_DEFINE")!=string::npos){ // subkeyword found
	  in.str(GetLineFromFile1(rfd_file));
      in >> species_define; //sub_line
	  in.clear();
    }
	//....................................................................
	if(line_string.find("$SPECIES_LIST")!=string::npos){ // subkeyword found
	...
	//....................................................................
	if(line_string.find("$KINETIC_SPECIES")!=string::npos){ // subkeyword found
	...
	//....................................................................
	if(line_string.find("$KINETIC_REACTIONS")!=string::npos){ // subkeyword found
    ...
    //....................................................................
	if(line_string.find("$KINETIC_HKF_REACTIONS")!=string::npos){ // subkeyword found
	...
	if(line_string.find("$PCS_RENAME")!=string::npos){ // subkeyword found
	...
	if(line_string.find("$PCS_PRE_RENAME")!=string::npos){ // subkeyword found
	...
	if(line_string.find("$RELATIVE_ACTIVITY")!=string::npos){ // subkeyword found
		this->species_relative_activity_name.clear();
		this->species_relative_activity.clear();
		while(1){
			in.str(GetLineFromFile1(rfd_file));
	...
	if(line_string.find("$NODE_ALL_SI")!=string::npos){ // subkeyword found
		this->Node_All_SI.clear();
		while(1){
			in.str(GetLineFromFile1(rfd_file));
			line_str2=in.str();
	...
	//....................................................................
	if(line_string.find("$REDOX_EH")!=string::npos){ // subkeyword found
	...
	//....................................................................
	if(line_string.find("$SET_BC_EQ_GEO")!=string::npos){ // subkeyword found
		...
		while(1){
			...
			if((line_str1.find("$")!=string::npos)||(line_str1.find("#STOP")!=string::npos)||(line_str1.find("END")!=string::npos)) {
    		  break;
	    	}
			in.str(line_str1);
            ...
            if (geometry_type.find("POINT") != std::string::npos)
            {
               ...
               if (!((geo_obj.getPointVecObj(unique_name))->getElementIDByName(
                  geometry_name, geo_obj_idx)))
               {
                  std::cerr
                     << "error in CKinReactData::Read: (type=" << geometry_type << "): " << geometry_name << " point name not found!"
                     << "\n";
                  exit(1);
               }
            }
            if (geometry_type.find("POLYLINE") != std::string::npos)
            {
               ...
               {
                  std::cerr
                     << "error in CKinReactData::Read: polyline name " << geometry_name << " not found!"
                     << "\n";
                  exit(1);
               }
            }
	...
	//....................................................................
	if(line_string.find("$SET_BC_EQ_SPECIES")!=std::string::npos){ // subkeyword found
	...
	if(aktueller_zeitschritt < 2){ //do not in the very first calculation before first time step and in the first time step
		...
		if(m_krd == NULL){
			...
			if(m_krd->is_a_CCBC.size() > 0){ // reaction nodes specified in krc input file
				...
				for(l=0; l< (long)m_msh->nod_vector.size();l++) { // node 1 needed for phreeqc-input
					this->rateflag[l] = 1;
...
if(CAP_MODE==2){
  ...
  //  cap_datafile = new char [m_gsp->path.length() + this->data_file.length()];
	...
	//	if(strcmp(m_pcs->pcs_primary_function_name[0], "pH")==0 || strcmp(m_pcs->pcs_primary_function_name[0], "Eh")==0){
...
void REACT_CAP::ShowDataFile(void){
	...
	printf("--> %li elements\n", nscom);
	for (i = 1; i <= nscom; i++) { //elements list
...
void REACT_CAP::LoadMassPCS(void){
	...
	for(i=0;i<no_pcs;i++){
		m_pcs = pcs_vector[i];
		cout << left << " Process "<< setw(6) << i << setw(18) << m_pcs->getProcessType() << " " << m_pcs->pcs_primary_function_name[0] << "\n";
...
bool REACT_CAP::CmpSpeciesListPCS(void){
	...
	for(i=0;i<ns;i++){
		if(strcmp(species_name[i],"EA")==0) ipc=0;
		else{
			noerr=0;
...
bool REACT_CAP::CmpSpeciesListDAT(void){
	...
	for(i=0;i<ns;i++){
		...
		if(species_list_phase[i]==2)
			for(ii=this->mass_num; ii<(int)pcs_mass_idx.size(); ii++){
				m_pcs = pcs_vector[pcs_mass_idx[ii]];
				if(/*this->species_mobil[ii]==0 &&*/ strcmp(m_pcs->pcs_primary_function_name[0], species_list_name[i])==0){
...
bool REACT_CAP::CmpSpeciesList(void){
	...
	for(i=0;i<ns;i++){
		...
		for(j=0;j<nls;j++)
			if(strcmp(species_name[i], species_list_name[j])==0)  sf=1;
		if(sf!=1){
...
void REACT_CAP::ResetElement(void){
	...
	if(nscom>=ipw){
		...
		if(newscv.size()+nm+1==(size_t)nscom) newscv.push_back("EA");// add electron as first element
  ...
  if(rateflag[ii]!=0)
  {
    ...
    if(m_rei)
	  for(iv=0;iv<m_rei->VLE_pressure.size();iv++){
		...
		m_rei->VLE_pressure[iv].vp_value = m_pcs->GetNodeValue(ii,f); //vp_value is the partial pressure of the gas, from current node value (IC or BC ??)
		for( i=0;i<(size_t)ns;i++)
		  if(this->pcs_mass_idx[i]==m_rei->VLE_pressure[iv].aq_idx){
			...
			//m_rei->VLE_pressure[iv].aq_value=   m_rei->VLE_pressure[iv].vp_value; // species_value[i]; //should get value from geochemcalc
          }
      }
    ...
    while(!is_equilibrium){
	  ...
	  for(i_np=-1;i_np<4;i_np++){ //ChemApp eq calc recovering
      ...
      for( i=0;i<(size_t)ns;i++){
        precision_type=0;
		if(species_dormant[i]==0){  // dormant = 1 --> kinetic species, does not take part in eq reactions
            ...
            if(species_phase[i]==0){
              if(ff != -1) // for full system, ff==-1 for the liquid system
                if(species_value[i]>-50){
                  CAP_tqsetc((char *)"ia", species_idx[i], 0, species_value[i], &numcon, &noerr);	// single comp phase (solid)
			    }
            }
            else{
  		      if(i_np==-1)
              ...
              residual_idx.push_back(i);
	          CAP_tqsetc((char *)"ia", species_phase[i], species_idx[i], sp_value, &numcon, &noerr); // any mixture phase
	          CAP_tqstpc(species_phase[i], species_idx[i], stoi, &wmass, &noerr);
		  ...
		  //pre setting
			for(i=0;i<species_relative_activity_name.size();i++){
			  if(species_value[species_relative_activity_idx_pcs[i]]>-50){
				if(species_value[species_relative_activity_idx_pcs[i]] > 1.0e-16 ) {//precision
				  if(species_relative_activity_state[i]==1 || species_relative_activity_state[i]==0){
					CAP_tqcsp(species_relative_activity_idx[i], (char *)"entered", &noerr);//DL
					CAP_tqsetc((char *)"ac",species_relative_activity_idx[i],0,species_relative_activity[i],&numcon,&noerr); //DL
				  ...
				  if(this->species_relative_activity_state[i]==3 || this->species_relative_activity_state[i]==0){
                    ...
                    CAP_tqsetc((char *)"ia",this->species_relative_activity_idx[i],0,0.0,&numcon,&noerr);
                    this->species_relative_activity_state[i]=3;
                  }
                  if(this->species_relative_activity_state[i]==4){
                    CAP_tqcsp(this->species_relative_activity_idx[i], (char *)"entered", &noerr);//DL
                    CAP_tqsetc((char *)"ac",this->species_relative_activity_idx[i],0,this->species_relative_activity[i],&numcon,&noerr); //DL
                  }
                }
              }
            }
			...
			if(first_time && fg==0){ // do when the first node is called
				...
				if(ii==1 || ii==6 /*|| ii==8 || ii==12 || ii==16 || ii==20 */)
					CAP_tqcen((char *)" ", 0, 0, vals, &noerr); // this is faster, using previuos result as start for iteration
				else
					CAP_tqcen((char *)" ", 0, 0, vals, &noerr); // this is faster, using previuos result as start for iteration
			}
			...
			if(noerr)
			  ...
			  for(i=0;i<species_relative_activity_name.size();i++)
              {
				if(species_value[this->species_relative_activity_idx_pcs[i]]>-50){ //DL====
				  CAP_tqgetr((char *)"ia", this->species_relative_activity_idx[i], 0, &value, &noerr); //get income amount, "ia"
				  this->species_relative_activity_ia.push_back(value);
				  CAP_tqgetr((char *)"ac", this->species_relative_activity_idx[i], 0, &value, &noerr); //get relative activity after calc, "ac"
				  ...
				  if(this->species_relative_activity_state[i]==1 || this->species_relative_activity_state[i]==4){ // ra case
				    if(this->species_relative_activity_ia[i]>0.0
					  ...
					  this->species_relative_activity_ia[i]=
					      species_value[this->species_relative_activity_idx_pcs[i]]-this->species_relative_activity_ia[i];
                    }
				  }
				  else if(this->species_relative_activity_state[i]==2 || this->species_relative_activity_state[i]==3){ // dormant case
				    if(this->species_relative_activity_calc[i]>this->species_relative_activity[i]){
    ...
    if(noerr) {
	  ...
	  for( i=0;i<(size_t)ns;i++){
		  if(species_dormant[i]==0)
			  cout << setprecision(8) << setw(14) << species_value[i] << " " << species_phase[i] << " " << setw(4) << species_idx[i] << " " << species_name[i] << "\n";
	  }
          ...
          if(species_phase[i]==0)
            ...
            if(species_phase[i]==2){  // aqueous
		      if(this->mass_type.compare("ELEMENT")) 						  // element  //DL 2013
			    CAP_tqgetr((char *)"a", 2, species_idx[i], &value, &noerr); // A Equilibrium amount
		      else {                                              // species
			    isc++;
			    CAP_tqgetr((char *)"ap", 2, isc, &value, &noerr);	// AP Equilibrium amount / Equilibrium amount of system component in a phase , isc counter
              }
            }
            else                                                  // other phases
	...
	//		delta= m_rei->VLE_conditions[i].aq_value - species_value[m_rei->VLE_conditions[i].idx_aq_species];
	//		delta *= 1000.0;
	//		if(abs(delta)>1.0e-8){
	//			if(delta>0)
	...
	//					species_value[m_rei->VLE_conditions[i].idx_aq_species] += m_rei->VLE_conditions[i].vp_value;
	//				}
	//			else{
	//				m_rei->VLE_conditions[i].vp_value -= delta;
	//				species_value[m_rei->VLE_conditions[i].idx_aq_species] += delta;
      ...
      if(m_rei->VLE_conditions.size()>0 )
      {
        ...
        is_equilibrium=true;
		  ...
		  if(!is_equilibrium){
			...
			for(i=0;i<m_rei->VLE_conditions.size();i++)
			  species_value[m_rei->VLE_conditions[i].idx_aq_species] += m_rei->VLE_conditions[i].delta;
		  }
      }
    ...
    if(!noerr) {
      if(m_rei)
		...
		//if(MinKinReact && ff != -1){
		  ...
		  for(i=0;i<species_value.size();i++)
			  species_value_d.push_back(species_value[i]);
		  this->KinParamUpdata(ii, noerr, species_value_d);    // in case of  no error, a filled vector will be stored
		  this->KinParamUpdataHKF(ii,TT,PP,noerr);		           // CB include noerr
		  //-----------------------------------
          ...
          if( species_dormant[ns-i-1]==0){ // return only values of non-dormant species
            ...
            if(this->species_phase[ns-i-1]==0){
              if(ff==1) // full geochemistry calculation
                m_pcs->SetNodeValue(ii,f,species_value.back()/unitfactor_s);  // old f==0 or new f==1 time step
            }
...
void REACT_CAP::KinParamUpdata(int /*ii*/, int err, vector <double> & spvc){
  ...
  if(err == 0){ // value was calculated
	  ...
	  for(i_re=0;i_re<(int)this->Kin_Reactions.size();i_re++){ // loop over reactions
		  logK=0;
		  for(i_resp=0;i_resp<(int)this->Kin_Reactions[i_re].species_name.size();i_resp++){  // loop over species of reaction
...
vector<double> REACT_CAP::KineticReact(vector<double> species_value0){
	...
	//for(i=0;i<(int)species_value0.size();i++){
	//	cout << setw(32) << species_name[i] << setw(16) << species_value0[i] /*<< setw(10) << species_value1[i] */<< " dcdt " << dcdt0[i] << "\n";
	//}
	...
	while(j<iter_max){
		...
		for(i=0;i<(int)c0.size();i++){
			if(h[i]>0){
				...
				cout << " dcdt0 " << setw(16) << dcdt0[i] << " c0 " << setw(16) << c0[i] << " h " <<  h[i] << " t1 " << setw(16) << t1 /*<< "\n"*/;
				cout << " i " << i << " k " << k << " t0 " << t0 << "\n";
				if(t1<t0)
					t0=t1;
			}
		...
		for(i_re=0;i_re<(int)this->Kin_Reactions.size();i_re++){
			for(i_resp=0;i_resp<(int)this->Kin_Reactions[i_re].species_name.size();i_resp++){
				if(this->Kin_Reactions[i_re].is_Kin){
					...
					if(Kin_Reactions[i_re].species_phase[i_resp]==2){
						CAP_tqstpc(Kin_Reactions[i_re].species_phase[i_resp],Kin_Reactions[i_re].species_idx[i_resp],stoi,&wmass,&noerr);
						ii=0;
						for(i_sp=0;i_sp<(int)this->species_name.size();i_sp++){
							if(species_phase[i_sp]==2){
								dcdt[i_sp]+= stoi[ii]*Kin_Reactions[i_re].species_stoi[i_resp]*Kin_Reactions[i_re].rate;
								//cout << species_name[i_sp] << " " << Kin_Reactions[i_re].type << " " << stoi[ii] << " " << Kin_Reactions[i_re].species_stoi[i_resp] << " " << Kin_Reactions[i_re].species_name[i_resp] << "\n";
								ii++;
							}
						}
					}
		...
		if(Kin_Reactions[i].is_Kin){
			...
			for(ix=0;ix<(int)Kin_Reactions[i].EA.size();ix++){
				if(Kin_Reactions[i].ref_species[ix]=="")
					Kin_rate += Kin_Reactions[i].K25[ix]*exp(-Kin_Reactions[i].EA[ix]/8.314*(1.0/T-1.0/298.15));
				else
					Kin_rate += Kin_Reactions[i].K25[ix]*exp(-Kin_Reactions[i].EA[ix]/8.314*(1.0/T-1.0/298.15)) * pow(node_ac[ii][Kin_Reactions[i].ref_idx[ix]], Kin_Reactions[i].ref_expo[ix]);
			}
			Kin_rate *= 1.0;
			//cout << " " << Kin_rate << "\n";
...
void REACT_CAP::KinInit(void){
	...
	for (i=0; i<(int)KinReact_vector.size(); i++){
		...
		for(ix=0;ix<n_r;ix++){
			if(m_kr->chemapp_name.compare(Kin_Reactions[ix].type)==0){ // found
		...
		for(i_resp=0;i_resp<(int)this->Kin_HKF_Reactions[i_re].species_name.size();i_resp++){
			...
			for(i_sp=0;i_sp<(int)Kin_HKF_species.size();i_sp++){
				if(strcmp(this->Kin_HKF_species[i_sp].name0.c_str(),
					this->Kin_HKF_Reactions[i_re].species_name[i_resp])==0){
						is_new=0;
...
bool REACT_CAP::LoadHKFparam(){
	...
	for(i=0;i<(int)this->Kin_HKF_species.size();i++){
		...
		if(Kin_HKF_species[i].name0=="H2O" || Kin_HKF_species[i].name0=="H2O_liquid"  || Kin_HKF_species[i].name0=="water_liquid")
...
bool REACT_CAP::HKFcalc(double T, double P){
	...
	for(i=0;i<(int)this->Kin_HKF_species.size();i++){
		if(Kin_HKF_species[i].type !=-1){
			HKF::HKF_OGS_calc(T, P, G, H, S, Kin_HKF_species[i].type, Kin_HKF_species[i].charge, Kin_HKF_species[i].param);
			//cout << Kin_HKF_species[i].name0 << " " << G << " " << H << " " << S << "\n";
			Kin_HKF_species[i].G=G;
			Kin_HKF_species[i].H=H;
			Kin_HKF_species[i].S=S;
		}
		else if(Kin_HKF_species[i].name0=="H2O" || Kin_HKF_species[i].name0=="H2O_liquid"  || Kin_HKF_species[i].name0=="water_liquid"){
...
/********************************************************/
  CFEMesh* m_msh = fem_msh_vector[0]; //SB: ToDo hart gesetzt
  if(m_msh == NULL) {cout << "ConvertIC2BC" << "\n"; exit(0);}
				...
				if(replace){
					...
					for(j=0;j< (int) m_pcs->bc_node_value.size();j++){
						if(m_pcs->bc_node_value[j]->msh_node_number == node_nr[ii]){
							cout << "      Replaced bc value  " << m_pcs->bc_node_value[j]->node_value << "  by  " << species_value << "\n";
