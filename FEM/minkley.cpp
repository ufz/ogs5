...
void SolidMinkley::NumericalJacobian(const double dt, const Eigen::Matrix<double,6,1> &dstrain_curr, const double e_curr,
                                     const Eigen::Matrix<double,6,1> &stress_curr, const Eigen::Matrix<double,6,1> &dstrain_Kel_curr,
                                     const Eigen::Matrix<double,6,1> &dstrain_Kel_t, const Eigen::Matrix<double,6,1> &dstrain_Max_curr,
                                     const Eigen::Matrix<double,6,1> &dstrain_Max_t, const Eigen::Matrix<double,6,1> &dstrain_pl_curr,
                                     const Eigen::Matrix<double,6,1> &dstrain_pl_t, const double e_pl_vol_curr, const double e_pl_vol_t,
    ...
    for (size_t i=0; i<27; i++)
        for (size_t j=0; j<27; j++)
        {
            if (j < 6)
            {
                ...
                UpdateMinkleyProperties(smath->CalEffectiveStress(smath->P_dev*upper * GM0),e_pl_eff_curr,0.);
                CalViscoplasticResidual(dt,dstrain_curr,e_curr,upper,dstrain_Kel_curr,dstrain_Kel_t,dstrain_Max_curr,dstrain_Max_t,dstrain_pl_curr,dstrain_pl_t,
                                                 e_pl_vol_curr,e_pl_vol_t,e_pl_eff_curr,e_pl_eff_t,lam_curr,residual);
                up = residual(i);
                //Update Material parameters
                UpdateMinkleyProperties(smath->CalEffectiveStress(smath->P_dev*lower * GM0),e_pl_eff_curr,0.);
                CalViscoplasticResidual(dt,dstrain_curr,e_curr,lower,dstrain_Kel_curr,dstrain_Kel_t,dstrain_Max_curr,dstrain_Max_t,dstrain_pl_curr,dstrain_pl_t,
                                                 e_pl_vol_curr,e_pl_vol_t,e_pl_eff_curr,e_pl_eff_t,lam_curr,residual);
                low = residual(i);
            }
            else if (j < 12)
            {
                upper = dstrain_Kel_curr;
                lower = dstrain_Kel_curr;
                upper(j-6) += pertub;
                lower(j-6) -= pertub;
                //Update Material parameters
                UpdateMinkleyProperties(smath->CalEffectiveStress(smath->P_dev*stress_curr * GM0),e_pl_eff_curr,0.);
                CalViscoplasticResidual(dt,dstrain_curr,e_curr,stress_curr,upper,dstrain_Kel_t,dstrain_Max_curr,dstrain_Max_t,dstrain_pl_curr,dstrain_pl_t,
                                                 e_pl_vol_curr,e_pl_vol_t,e_pl_eff_curr,e_pl_eff_t,lam_curr,residual);
                up = residual(i);
                CalViscoplasticResidual(dt,dstrain_curr,e_curr,stress_curr,lower,dstrain_Kel_t,dstrain_Max_curr,dstrain_Max_t,dstrain_pl_curr,dstrain_pl_t,
                                                 e_pl_vol_curr,e_pl_vol_t,e_pl_eff_curr,e_pl_eff_t,lam_curr,residual);
                low = residual(i);
            }
            else if (j < 18)
            {
                upper = dstrain_Max_curr;
                lower = dstrain_Max_curr;
                upper(j-12) += pertub;
                lower(j-12) -= pertub;
                //Update Material parameters
                UpdateMinkleyProperties(smath->CalEffectiveStress(smath->P_dev*stress_curr * GM0),e_pl_eff_curr,0.);
                CalViscoplasticResidual(dt,dstrain_curr,e_curr,stress_curr,dstrain_Kel_curr,dstrain_Kel_t,upper,dstrain_Max_t,dstrain_pl_curr,dstrain_pl_t,
                                                 e_pl_vol_curr,e_pl_vol_t,e_pl_eff_curr,e_pl_eff_t,lam_curr,residual);
                up = residual(i);
                CalViscoplasticResidual(dt,dstrain_curr,e_curr,stress_curr,dstrain_Kel_curr,dstrain_Kel_t,lower,dstrain_Max_t,dstrain_pl_curr,dstrain_pl_t,
                                                 e_pl_vol_curr,e_pl_vol_t,e_pl_eff_curr,e_pl_eff_t,lam_curr,residual);
                low = residual(i);
            }
            else if (j < 24)
            {
                upper = dstrain_pl_curr;
                lower = dstrain_pl_curr;
                upper(j-18) += pertub;
                lower(j-18) -= pertub;
                //Update Material parameters
                UpdateMinkleyProperties(smath->CalEffectiveStress(smath->P_dev*stress_curr * GM0),e_pl_eff_curr,0.);
                CalViscoplasticResidual(dt,dstrain_curr,e_curr,stress_curr,dstrain_Kel_curr,dstrain_Kel_t,dstrain_Max_curr,dstrain_Max_t,upper,dstrain_pl_t,
                                                 e_pl_vol_curr,e_pl_vol_t,e_pl_eff_curr,e_pl_eff_t,lam_curr,residual);
                up = residual(i);
                CalViscoplasticResidual(dt,dstrain_curr,e_curr,stress_curr,dstrain_Kel_curr,dstrain_Kel_t,dstrain_Max_curr,dstrain_Max_t,lower,dstrain_pl_t,
                                                 e_pl_vol_curr,e_pl_vol_t,e_pl_eff_curr,e_pl_eff_t,lam_curr,residual);
                low = residual(i);
            }
            else if (j < 25)
            {
                //Update Material parameters
                UpdateMinkleyProperties(smath->CalEffectiveStress(smath->P_dev*stress_curr * GM0),e_pl_eff_curr,0.);
                CalViscoplasticResidual(dt,dstrain_curr,e_curr,stress_curr,dstrain_Kel_curr,dstrain_Kel_t,dstrain_Max_curr,dstrain_Max_t,dstrain_pl_curr,dstrain_pl_t,
                                                  e_pl_vol_curr+pertub,e_pl_vol_t,e_pl_eff_curr,e_pl_eff_t,lam_curr,residual);
                up = residual(i);
                CalViscoplasticResidual(dt,dstrain_curr,e_curr,stress_curr,dstrain_Kel_curr,dstrain_Kel_t,dstrain_Max_curr,dstrain_Max_t,dstrain_pl_curr,dstrain_pl_t,
                                                  e_pl_vol_curr-pertub,e_pl_vol_t,e_pl_eff_curr,e_pl_eff_t,lam_curr,residual);
                low = residual(i);
            ...
            {
                //Update Material parameters
                UpdateMinkleyProperties(smath->CalEffectiveStress(smath->P_dev*stress_curr * GM0),e_pl_eff_curr+pertub,0.);
                CalViscoplasticResidual(dt,dstrain_curr,e_curr,stress_curr,dstrain_Kel_curr,dstrain_Kel_t,dstrain_Max_curr,dstrain_Max_t,dstrain_pl_curr,dstrain_pl_t,
                                                  e_pl_vol_curr,e_pl_vol_t,e_pl_eff_curr+pertub,e_pl_eff_t,lam_curr,residual);
                up = residual(i);
                //Update Material parameters
                UpdateMinkleyProperties(smath->CalEffectiveStress(smath->P_dev*stress_curr * GM0),e_pl_eff_curr-pertub,0.);
                CalViscoplasticResidual(dt,dstrain_curr,e_curr,stress_curr,dstrain_Kel_curr,dstrain_Kel_t,dstrain_Max_curr,dstrain_Max_t,dstrain_pl_curr,dstrain_pl_t,
            ...
            else if (j < 27)
            {
                ...
                UpdateMinkleyProperties(smath->CalEffectiveStress(smath->P_dev*stress_curr * GM0),e_pl_eff_curr,0.);
                CalViscoplasticResidual(dt,dstrain_curr,e_curr,stress_curr,dstrain_Kel_curr,dstrain_Kel_t,dstrain_Max_curr,dstrain_Max_t,dstrain_pl_curr,dstrain_pl_t,
                                                  e_pl_vol_curr,e_pl_vol_t,e_pl_eff_curr,e_pl_eff_t,lam_curr+pertub,residual);
                up = residual(i);
                CalViscoplasticResidual(dt,dstrain_curr,e_curr,stress_curr,dstrain_Kel_curr,dstrain_Kel_t,dstrain_Max_curr,dstrain_Max_t,dstrain_pl_curr,dstrain_pl_t,
                                                  e_pl_vol_curr,e_pl_vol_t,e_pl_eff_curr,e_pl_eff_t,lam_curr-pertub,residual);
                low = residual(i);
