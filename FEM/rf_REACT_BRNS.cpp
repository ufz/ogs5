...
#ifdef TESTTIME
    cout << "Total time spent in BRNS Coupling, including Solver: "
         << timeSpentInBrnsCoupling << "s" << endl;
#endif
}
...
void REACT_BRNS::InitBRNS(Problem* myProblem)
{
        ...
        for (k = 0; k < num_Comp; k++)
        {
            ...
            if (this_pcs)
                for (p = 0; p < this_pcs->pcs_number_of_primary_nvals; ++p)
                {
                    ...
                    const std::string tmp_pcs_type(
                        convertProcessTypeToString(this_pcs->getProcessType()));
                    m_bc_group = BCGetGroup(
                        tmp_pcs_type, this_pcs->pcs_primary_function_name[p]);

                    // BC printing
                    if (IsThisPointBCIfYesStoreValue(i, this_pcs, BCValue))
                    {
                        // If this node is on the fixed boudnary for this
                        // component
                        boundary_flag[i * num_Comp + k] = 1;
                        cout << "Node " << i << ", Comp " << k << ",Value "
                             << BCValue << endl;
                    }
                    else
                        // If this node is NOT on the fixed boudnary for this
                        // component
    ...
    for (long i = 0; i < nNodes; i++)
    {
        if (m_flow_pcs->m_msh->nod_vector[i]->getConnectedElementIDs().size() >
            0)
        {
            total_volume = 0.0;
            for (int j = 0; j < (int)m_flow_pcs->m_msh->nod_vector[i]
                                    ->getConnectedElementIDs()
                                    .size();
                 j++)
            {
                idx_elem = m_flow_pcs->m_msh->nod_vector[i]
                               ->getConnectedElementIDs()[j];
                elem_volume =
                    m_flow_pcs->m_msh->ele_vector[idx_elem]->GetVolume();
...
void REACT_BRNS::GSRF2Buffer(long i)
{
    ...
    // for this node, loop over all the chemical components and update the
    // values
    for (int k = 0; k < num_Comp; k++, i_times_num_Comp_plus_k++)
    {
        ...
        if (this_pcs)
        {
            ...
            cur_ts_Conc[i_times_num_Comp_plus_k] = this_pcs->GetNodeValue(
                i,
                this_pcs->GetNodeValueIndex(
                    this_pcs->pcs_primary_function_name[0]) +
                    1);
            pre_ts_Conc[i_times_num_Comp_plus_k] = this_pcs->GetNodeValue(
                i,
                this_pcs->GetNodeValueIndex(
                    this_pcs->pcs_primary_function_name[0]) +
                    0);
...
void REACT_BRNS::Buffer2GSRF(long i)
{
    ...
    for (int k = 0; k < num_Comp; k++, i_times_num_Comp_plus_k++)
    {
        ...
        if (this_pcs)
            ...
            this_pcs->SetNodeValue(i,
                                   this_pcs->GetNodeValueIndex(
                                       this_pcs->pcs_primary_function_name[0]) +
                                       1 /*1-current time step*/,
...
void REACT_BRNS::RUN(double time_step)
{
    ...
    {
        rt_BRNS[0] = -1;  // reaction rates should be printed by BRNS.dll for
                          // the whole domain
        for (i = 1; i < nNodes;
             i++)  // -1: delete a potentially existing rate file
            rt_BRNS[i] = -2;
    }
#ifdef USE_MPI_BRNS
    if (myrank == 1)
        startTime = clock();  // in MPI: make statistics only when running on
                              // one processor
#else
    startTime = clock();
#endif
...
#pragma omp parallel for shared( \
    ...
    cur_ts_Conc_temp,            \
    pre_ts_Conc_temp,            \
    boundary_flag_temp) private(i, pos_x, pos_y, pos_z)
    for (i = 0; i < nNodes_temp; i++)
    {
        ...
        //------------------------
        // Prepare porosity and waterSaturation values (weighted sums?), or move
        // to Buffer<->GSFR methods
        ...
        cout << "#" << num << "thread reporting. My i is: "
             << i /*cur_ts_Conc_temp[i*num_Comp_temp]*/ << endl;
...
#endif
    {
        // Check out if this node is on the boundary - A fixed boundary
        // condition
    ...
    for (i = 0; i < nNodes; i++)
    {
        ...
        {
            cout << "In Node " << i
                 << ": BRNS exceeded max newton iterations, aborting." << endl;
            exit(10);
        }
        ...
        switch (rt_BRNS[i])
        {
            case 1:
                cout << "In Node " << i
                     << ": BRNS calculated negative concentration!" << endl;
                break;
            case 2:
                cout << "In Node " << i
                     << ": BRNS exceeded max newton iterations!" << endl;
                break;
            case 3:
                cout << "In Node " << i
                     << ": BRNS calculated negative concentration, "
        ...
        {
            cout << "Still negative concentrations after chemical simulation "
                    "for:";
            for (int j = 0; j < num_Comp; j++)
                if (pre_ts_Conc[i * num_Comp + j] < 0.0)
                    cout << " Species " << j + 1 << ": "
                         << pre_ts_Conc[i * num_Comp + j];
            cout << endl;
        }
#endif
    }
    // do not forget to write the porosity value into the Node data structure
    // here. 1 stands for new time step.
    ConvPorosityNodeValue2Elem(0);
    ConvPorosityNodeValue2Elem(1);

#ifdef USE_MPI_BRNS
    if (myrank == 1)
        timeSpentInBrnsCoupling +=
...
#ifdef USE_MPI_BRNS
    if (myrank == 1)
        cout << "Total time spent in BRNS Coupling and Solver: "
             << timeSpentInBrnsCoupling << "s, " << timeSpentInBrnsSolver
             << "s " << endl;

#else
    cout << "Total time spent in BRNS Coupling and Solver: "
         << timeSpentInBrnsCoupling << "s, " << timeSpentInBrnsSolver << "s "
         << endl;
#endif
#endif
}

int REACT_BRNS::IsThisPointBCIfYesStoreValue(int index,
                                             CRFProcess* m_pcs,
                                             double& value)
{
...
int REACT_BRNS::SetPorosityValue_MT(long ele_Index,
        ...
        switch (flowflag)
        {
            case 1:
                ...
                m_pcs->SetElementValue(
                    ele_Index, idx + i_timestep, m_porosity_Elem);
                break;
            case 2:
            ...
            case 3:
                ...
                if (m_pcs->GetElementValue(ele_Index, idx + 1) == 0)
                    m_pcs->SetElementValue(ele_Index, idx + 1, m_porosity_Elem);
                else if (m_porosity_Elem !=
                         m_pcs->GetElementValue(ele_Index, idx + 1))
                {
                    cout << "Richards flow not supported - quitting program"
                         << endl;
                    exit(1);
                }
                break;
...
void REACT_BRNS::ConvPorosityNodeValue2Elem(int i_timestep)
{
    ...
    for (i = 0; i < nElems; i++)
    {
        ...
        for (j = 0; j < m_Elem->GetNodesNumber(false); j++)
        {
            ...
            // this is arithmetric mean here we use harmonic mean, as porosity
            // is used for permeability/diffusivity changes....flux in the
            // element is strongly influenced by the minimum values
            // this is for harmonic mean
            m_porosity_Elem[i] +=
                1.0 / m_porosity_Node[idx_Node] / number_of_nodes;
        }
...
void REACT_BRNS::GetBRNSResult_MPI(void)
{
    ...
    MPI_Allreduce(
        rt_BRNS_buf, rt_BRNS, nNodes, MPI_INT, MPI_SUM, MPI_COMM_WORLD);
    MPI_Allreduce(pre_ts_Conc_buf,
                  pre_ts_Conc,
                  num_Comp * nNodes,
                  MPI_DOUBLE,
                  MPI_SUM,
