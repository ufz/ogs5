...
void REACT_BRNS::InitBRNS(Problem* myProblem)
{
		...
		for ( k = 0; k < num_Comp; k++ )
		{
			...
			if (this_pcs)
				for(p = 0; p < this_pcs->pcs_number_of_primary_nvals; ++p)
				{
					...
					const std::string tmp_pcs_type (convertProcessTypeToString(
					                                        this_pcs->
					                                        getProcessType()));
					m_bc_group = BCGetGroup(
					        tmp_pcs_type,
					        this_pcs->pcs_primary_function_name
					        [p]);
	...
	for ( long i = 0; i < nNodes; i++ )
	{
		if ( m_flow_pcs->m_msh->nod_vector[i]->getConnectedElementIDs().size() > 0 )
		{
			...
			for ( int j = 0;
			      j <
			      (int)m_flow_pcs->m_msh->nod_vector[i]->getConnectedElementIDs().size();
			      j++ )
			{
				idx_elem =
	...
	for ( int k = 0; k < num_Comp; k++, i_times_num_Comp_plus_k++ )
	{
		...
		if (this_pcs)
		{
			...
			cur_ts_Conc[i_times_num_Comp_plus_k] = this_pcs->GetNodeValue(
			        i,
			        this_pcs->
			        GetNodeValueIndex( this_pcs->pcs_primary_function_name[0] ) + 1 );
			pre_ts_Conc[i_times_num_Comp_plus_k] = this_pcs->GetNodeValue(
			        i,
			        this_pcs->
			        GetNodeValueIndex( this_pcs->pcs_primary_function_name[0] ) + 0 );
		}
		else                      // not getting the pointer to the proper PCS.
		{
#ifdef MFC
			AfxMessageBox(
			        "!!! In Data transfer for BRNS, can not find corresponding PCS!");
#endif
			DisplayErrorMsg(
			        "!!! In Data transfer for BRNS, can not find corresponding PCS!");
			abort();
		...
		if (this_pcs)
			...
			this_pcs->SetNodeValue( i,
			                        this_pcs->GetNodeValueIndex(
			                                this_pcs->pcs_primary_function_name[0]) + 1 /*1-current time step*/,
...
#pragma \
	omp parallel for shared(nNodes_temp, num_Comp_temp, time_step, cur_ts_Conc_temp, pre_ts_Conc_temp, boundary_flag_temp) private(i,pos_x,pos_y,pos_z)
	...
	if (myrank == 1)
		...
		timeSpentInBrnsCoupling << "s, " << timeSpentInBrnsSolver << "s " << endl;

#else
	cout << "Total time spent in BRNS Coupling and Solver: " << timeSpentInBrnsCoupling <<
	"s, " << timeSpentInBrnsSolver << "s " << endl;
#endif
#endif
}
