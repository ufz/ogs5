...
static void InitialiseProjectionTensors()
{
...
static Eigen::Matrix<double,3,3>  KelvinVectorToTensor(const Eigen::Matrix<double,6,1> &vec)
{
...
static Eigen::Matrix<double,6,1>  TensorToKelvinVector(const Eigen::Matrix<double,3,3> &tens)
{
...
static double CalJ2(const Eigen::Matrix<double,6,1> &dev_vec)
{
	double s(0.);
...
static double CalEffectiveStress(const Eigen::Matrix<double,6,1> &dev_stress)
{
	double s(0.);
...
//Kelvin mapped DEVIATORIC vector
//A deviatoric mapping was not done here in order to avoid unnecessary calculations
static double CalJ3(const Eigen::Matrix<double,6,1> &dev_vec)
{
	Eigen::Matrix<double,3,3> tens;
...
static Eigen::Matrix<double,6,1> InvertVector(const Eigen::Matrix<double,6,1> &vec)
{
...
//Kelvin mapped DEVIATORIC vector
//A deviatoric mapping was not done here in order to avoid unnecessary calculations
static double CalLodeAngle(const Eigen::Matrix<double,6,1> &dev_vec)
{
...
static double CalI1(const Eigen::Matrix<double,6,1> &vec)
{
