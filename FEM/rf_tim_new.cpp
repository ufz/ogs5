...
/**************************************************************************
   ...
   08/2004 OK Implementation
**************************************************************************/
CTimeDiscretization::CTimeDiscretization(void) : Write_tim_discrete(false), tim_discrete(NULL) // YD
{
	step_current = 0;
	time_start = 0.0;
	time_end = 1.0;
...
std::ios::pos_type GetNextSubKeyword(std::ifstream* file, std::string* line, bool* keyword)
{
	...
	*keyword = false;
	...
	while (!(line_complete.find("$") != std::string::npos) && (!file->eof()))
	{
		...
		j = (int)line_complete.find(
		    ";", i); // Nach Kommentarzeichen ; suchen. j = Position des Kommentarzeichens, j=-1 wenn es keines gibt.
		if (j < 0)
			j = (int)line_complete.length();
		// if(j!=i) break;						 //Wenn das erste nicht-leerzeichen ein Kommentarzeichen ist, zeile
		// �berlesen.
		// Sonst
		// ist
		// das eine Datenzeile
		if (i != -1)
			*line = line_complete.substr(i, j - i); // Ab erstem nicht-Leerzeichen bis Kommentarzeichen rauskopieren in
		// neuen substring, falls Zeile nicht leer ist
	}
	return position;
}
		...
		if (line_string.find("$TIME_CONTROL") != std::string::npos)
			while ((!new_keyword) || (!new_subkeyword) || (!tim_file->eof()))
			{
				...
				else if (time_control_type == TimeControlType::DYNAMIC_VARIABLE) // JT2012
				{
					...
					//
					line >> dynamic_control_error_method; // Corresponds to FiniteElement::ErrorMethod. Defines how
					// tolerance is applied.
					line >> time_step_length; // initial_dt
					line >> min_time_step; // min_dt
					line >> dynamic_failure_threshold; // threshold to force a time failure (recommend 1.1-2.0. If
					// tolerance is exceeeded on first iteration by this factor, dt
					// will be decreased and a failure forced)
					line >> dynamic_control_tolerance[DOF_NUMBER_MAX]; // max_increase_factor (dt never allowed to
					// increase faster than this factor (i.e. 1.5)
					//
					// tolerances[1:dof]: One tolerance for each degree of freedom in process (or only one tolerance for
					// certain methods)
					switch (FiniteElement::convertErrorMethod(dynamic_control_error_method))
					{
						case FiniteElement::ENORM: // only 1 tolerance required
							...
							line >> include_third_variable; // if >0.0, include a 3rd variable in the global norm (PwSnw
							// model: Pc -- PcPnw model: Snw -- PwPnw model: Snw)
							break;
						//
						case FiniteElement::ERNORM: // only 1 tolerance required
							...
							break;
						//
						case FiniteElement::EVNORM: // 1 tolerance for each primary variable (for Deformation, only 1
							// tolerance required. Applies to x,y,z)
							for (int i = 0; i < num_tolerances; i++)
								line >> dynamic_control_tolerance[i];
							//
							// Next entry is OPTIONAL (may be left blank)!
							line >> third_variable_tolerance; // tolerance of a third variable (PwSnw model: Pc -- PcPnw
							// model: Snw -- PwPnw model: Snw)
							break;
						//
						case FiniteElement::LMAX: // 1 tolerance for each primary variable (for Deformation, only 1
							// tolerance required. Applies to x,y,z)
							for (int i = 0; i < num_tolerances; i++)
								line >> dynamic_control_tolerance[i];
							//
						...
						default:
							ScreenMessage(
							    "ERROR in TIMRead. Invalid error method selected for dynamic time control.\n");
							exit(1);
							break;
					}
					//
				...
				{
					std::cout << "Josh apologizes (especially to Marc), but promises DYNAMIC_COURANT will be merged "
					...
					if (time_control_name.find("CONCENTRATION") != std::string::npos)
					{ // DYNAMIC_COURANT_CONCENTRATION
						line >> dynamic_control_tolerance[1]; // Concentration threshold (elements with concentration
						// beneath this value are not included in Courant
						// restriction)
					}
					else if (time_control_name.find("TEMPERATURE") != std::string::npos)
					{ // DYNAMIC_COURANT_TEMPERATURE
						line >> dynamic_control_tolerance[1]; // Temperature threshold (elements with temperature
						// BENEATH this value are not included in Courant
						// restriction)
						line >> dynamic_control_tolerance[2]; // Temperature threshold (elements with temperature ABOVE
						// this value are not included in Courant restriction)
					}
					//
					line.clear();
				}
				         ...
				         || time_control_type == TimeControlType::STABLE_ERROR_ADAPTIVE)
				{
					...
					while ((!new_keyword) || (!new_subkeyword) || (!tim_file->eof()))
					{
						...
						else if (line_string.find("MIN_TIME_STEP") != std::string::npos)
						{
							*tim_file >> line_string;
						 ...
						 */
						else if (line_string.find("INITIAL_STEP_SIZE") != std::string::npos)
						{
							*tim_file >> line_string;
							initial_step_size = strtod(line_string.data(), NULL);
							line.clear();
						}
						else if (line_string.find("ITERATIVE_TYPE") != std::string::npos)
						{
							*tim_file >> line_string;
							adapt_itr_type = convertIterationType(line_string);
							if (line_string == "COUPLED_STABLE_ERROR") // convertIterationType() finds
								// IterationType::COUPLED if that is part of the
								// name... very unpractical.
								adapt_itr_type = IterationType::COUPLED_STABLE_ERROR;
							line.clear();
						}
						else if (line_string.find("STAY") != std::string::npos)
						{
							*tim_file >> line_string;
						         ...
						         || (!line_string.empty() && isdigit(line_string[0])))
						{
							...
							{
								std::cout << "ERROR: at least two multipliers should be provided for SELF_ADAPTIVE "
								             "time stepping"
								          << std::endl;
								exit(1);
							}
						}
						else if (line_string.find("DESIRED_ERROR") != std::string::npos)
						{
							*tim_file >> line_string;
							desired_error = strtod(line_string.data(), NULL);
							line.clear();
						}
						else if (line_string.find("MAX_INCREASE") != std::string::npos)
						{
							*tim_file >> line_string;
							max_increase = strtod(line_string.data(), NULL);
							line.clear();
						}
						else if (line_string.find("MIN_INCREASE") != std::string::npos)
						{
							*tim_file >> line_string;
							min_increase = strtod(line_string.data(), NULL);
							line.clear();
						}
						else if (line_string.find("DAMPENING") != std::string::npos)
						{
							*tim_file >> line_string;
	...
	*tim_file << "  " << time_end << "\n";
	...
	else
	{
		...
		*tim_file << "  " << convertTimeControlTypeToString(time_control_type) << std::endl;
		...
		{
			*tim_file << "   " << PI_tsize_ctrl_type << " " << relative_error << " " << absolute_error << " "
			          << this_stepsize << "\n";
		}
...
double CTimeDiscretization::CalcTimeStep(double current_time)
{
	...
	for (int i = 0; i < (int)critical_time.size(); i++)
	{
		if (current_time < critical_time[i])
		{
			...
			tval = next
			       + time_step_length
			             / 1.0e3; // JT2012. A tiny increase in dt is better than a miniscule dt on the next step
...
double CTimeDiscretization::FirstTimeStepEstimate(void)
{
	...
	for (size_t n_p = 0; n_p < pcs_vector.size(); n_p++)
	{
		...
		{
			//		case 'G': // kg44 groudnwater flow ---if steady state, time step should be greater zero...transient
			// flow does not work with adaptive stepping
			case FiniteElement::GROUNDWATER_FLOW: // TF, if steady state, time step should be greater zero...transient
			// flow does not work with adaptive stepping
			case FiniteElement::LIQUID_FLOW: // TF, if steady state, time step should be greater zero...transient flow
				// does not work with adaptive stepping
			...
			case FiniteElement::HEAT_TRANSPORT: // MW copied from MASS_TRANSPORT // TF, if steady state, time step
...
double CTimeDiscretization::DynamicVariableTimeControl(void)
{
	...
	if (dynamic_control_tolerance[DOF_NUMBER_MAX] > 0.0 && m_pcs->isPCSMultiFlow)
	{
		...
		dynamic_control_tolerance[num_variables]
		    = dynamic_control_tolerance[DOF_NUMBER_MAX]; // shift to 3rd variable slot
		num_variables++;
	}
	//
	// Get error
...
double CTimeDiscretization::DynamicTimeSmoothing(double suggested_time_step_change)
{
	...
	{
		time_step = recommended_time_step; // recommended_time_step is the m_tim variable of last suggested time step
		// before any critical changes
	}
	else
	{
...
double CTimeDiscretization::StableErrorAdaptive(void)
{
	...
	for (size_t n_p = 0; n_p < pcs_vector.size(); n_p++)
	{
		if (pcs_vector[n_p]->getProcessType() == pcs_type)
		{
			...
			{ // does this ever trigger?
				...
				// ScreenMessage("-> ERROR in " + convertTimeControlTypeToString(time_control_type).c_str() + ": PCS not
	...
	// screen output
	std::cout << "\n"
	          << pcs_type_name << " " << convertTimeControlTypeToString(time_control_type) << " suggest "
	          << ((time_step_length / last_time_step_length > 1) ? "increasing" : "decreasing")
...
double CTimeDiscretization::SEA_zbrent(const double tol)
{
	...
	if ((fa > 0.0 && fb > 0.0) || (fa < 0.0 && fb < 0.0)) // cout << "Error in zbrent, fluid " << fluid << " T: " << TT
		// << " P: " << PP << " b: " << b << "\n";
		std::cout << ".";
	fc = fb;
...
// First calculate maximum time step according to Neumann and Courant criteria
	...
	for (size_t n_p = 0; n_p < pcs_vector.size(); n_p++)
	{
		if (pcs_vector[n_p]->getProcessType() == pcs_type)
		{
			...
			else if (adapt_itr_type == IterationType::COUPLED || adapt_itr_type == IterationType::COUPLED_STABLE_ERROR)
			{
	...
	if (adapt_itr_type == IterationType::COUPLED_STABLE_ERROR)
	{
		...
		if (inverse_error < 2 * multiplier)
		{
			...
			std::cout << "Adapting multiplier to " << multiplier << " instead of " << old_multiplier << std::endl;
		}
	}
...
double CTimeDiscretization::CheckCourant(void)
{
	...
	std::cout << "Courant time step control, critical element = " << critical_element_no << " Recomended time step "
	...
	for (size_t n_p = 0; n_p < pcs_vector.size(); n_p++)
	{
		...
		switch (m_pcs->getProcessType()) // TF
		{
			...
			case FiniteElement::MASS_TRANSPORT: // TF
				...
				break;
			//		case 'G': // kg44 groudnwater flow ---if steady state, time step should be greater zeor...transient
			// flow does not work with adaptive stepping
			case FiniteElement::GROUNDWATER_FLOW: // if steady state, time step should be greater zero ... transient
				// flow does not work with adaptive stepping
				time_step_length = min_time_step; // take min time step as conservative best guess for testing
				break;
			default:
			...
			case FiniteElement::RICHARDS_FLOW: // accepted and refused time step
				...
				if (repeat)
					time_step_length *= MMax(
					    safty_coe * sqrt(m_pcs->m_num->nls_error_tolerance[0] / nonlinear_iteration_error), rmin);
				else
					time_step_length *= MMin(
					    safty_coe * sqrt(m_pcs->m_num->nls_error_tolerance[0] / nonlinear_iteration_error), rmax);
...
bool CTimeDiscretization::isDynamicTimeFailureSuggested(CRFProcess* m_pcs)
{
	...
	{ // Alright, this is annoying. Unfortunately we have to recalculate the node errors.
		...
		for (int ii = 0; ii < m_pcs->temporary_num_dof_errors; ii++)
		{
			if (this->dynamic_failure_threshold
			    > m_pcs->temporary_absolute_error[ii] / this->dynamic_control_tolerance[ii])
			{
				return true;
			}
		}
