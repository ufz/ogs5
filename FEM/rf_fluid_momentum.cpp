...
void CFluidMomentum::SolveDarcyVelocityOnNode()
{
	...
	for (int phase = 0; phase < GetRFProcessNumPhases(); phase++)
	{
		for(int d = 0; d < dimension; ++d)
		{
			...
			if(dimension == 1)
				nidx1 =
				        m_pcs->GetNodeValueIndex(m_pcs->pcs_primary_function_name[
				                                         axis]) + 1;
			else if(dimension == 2)
			{
				if(axis == 1) // x,y only
					nidx1 = m_pcs->GetNodeValueIndex(
					        m_pcs->pcs_primary_function_name[(axis - d +
					                                          1) %
					                                         dimension]) + 1;
				...
				nidx1 = m_pcs->GetNodeValueIndex(
				        m_pcs->pcs_primary_function_name[d]) + 1;
			else
				abort();  // Just stop something's wrong.
		...
		for (i = 0; i < (long)m_msh->ele_vector.size(); i++)
		{
			...
			{
				vx += m_pcs->GetNodeValue(elem->GetNodeIndex(
				                                  j),
				                          m_pcs->GetNodeValueIndex(
				                                  "VELOCITY1_X") + 1);
				vy += m_pcs->GetNodeValue(elem->GetNodeIndex(
				                                  j),
				                          m_pcs->GetNodeValueIndex(
				                                  "VELOCITY1_Y") + 1);
				vz += m_pcs->GetNodeValue(elem->GetNodeIndex(
				                                  j),
				                          m_pcs->GetNodeValueIndex(
				                                  "VELOCITY1_Z") + 1);
			}
			...
			m_pcs->SetElementValue(i, m_pcs->GetElementValueIndex(
			                               "VELOCITY1_X") + 1, vx);
			m_pcs->SetElementValue(i, m_pcs->GetElementValueIndex(
			                               "VELOCITY1_Y") + 1, vy);
			m_pcs->SetElementValue(i, m_pcs->GetElementValueIndex(
...
void CFluidMomentum::ConstructFractureNetworkTopology()
{
   ...
   if(!m_msh)
		...
		if(thisNode->crossroad)
		{
			...
			for(int j = 0; j < numOfPlanesAtCrossroad; ++j)
			{
				...
				for(int l = j + 1; l < numOfPlanesAtCrossroad; ++l)
				{
					...
					{
						// Two elements stay on the same plane
						for(int m = l; m < (numOfPlanesAtCrossroad - 1);
						    ++m)
	...
	for(int i = 0; i < (int) m_msh->edge_vector.size(); ++i)
	{
		...
		V0[0] = m_pcs->GetNodeValue(
		        theNodesOfThisEdge[0]->GetIndex(), m_pcs->GetNodeValueIndex(
		                "VELOCITY1_X") + 1);
		V0[1] = m_pcs->GetNodeValue(
		        theNodesOfThisEdge[0]->GetIndex(), m_pcs->GetNodeValueIndex(
		                "VELOCITY1_Y") + 1);
		V0[2] = m_pcs->GetNodeValue(
		        theNodesOfThisEdge[0]->GetIndex(), m_pcs->GetNodeValueIndex(
		                "VELOCITY1_Z") + 1);
		V1[0] = m_pcs->GetNodeValue(
		        theNodesOfThisEdge[1]->GetIndex(), m_pcs->GetNodeValueIndex(
		                "VELOCITY1_X") + 1);
		V1[1] = m_pcs->GetNodeValue(
		        theNodesOfThisEdge[1]->GetIndex(), m_pcs->GetNodeValueIndex(
		                "VELOCITY1_Y") + 1);
		V1[2] = m_pcs->GetNodeValue(
		        theNodesOfThisEdge[1]->GetIndex(), m_pcs->GetNodeValueIndex(
		                "VELOCITY1_Z") + 1);
		V[0] = (V0[0] + V1[0]) / 2.0;
		V[1] = (V0[1] + V1[1]) / 2.0;
...
void CFluidMomentum::SolveForEdgeVelocity(void)
{
	...
	for(int i = 0; i < (int) m_msh->edge_vector.size(); ++i)
	{
		...
		double MagOfVector = sqrt(
		        VectorOfEdge[0] * VectorOfEdge[0] + VectorOfEdge[1] * VectorOfEdge[1]);
		...
		else
		{
			// For Ve0 first
			cos =
			        (VectorOfEdge[0] * V0[0] + VectorOfEdge[1] *
			         V0[1]) / (MagOfVector * MagOfV0);
		...
		else
		{
			// For Ve1 second
			cos =
			        (VectorOfEdge[0] * V1[0] + VectorOfEdge[1] *
			         V1[1]) / (MagOfVector * MagOfV1);
			sin = sqrt(1.0 - cos * cos);
		...
		double angle =
		        acos((V[0] * Vc[0] + V[1] *
		             ...
		             (sqrt(V[0] * V[0] + V[1] * V[1] + V[2] *
		                   V[2]) * sqrt(Vc[0] * Vc[0] + Vc[1] * Vc[1] + Vc[2] * Vc[2])));
