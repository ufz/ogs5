...
void REACT::ExecutePQCString(void)
{
	...
	for (std::size_t j = 0; j < rankrankliststore[myrank].size(); j++)
	{
		...
		for (std::size_t i = 0; i < ranknodeliststore[idy].size(); i++)
		{
			...
			{
				pqc_file.seekg(0L, ios_base::beg);
				ok = WriteInputPQCString(idx, rankliststringstore[myrank][j], firstinput);
				firstinput = false;
				ii++;
			}
		}
	...
	// int signal;
	int strlength;
	if (mysize_all > mysize) // if group_IPQC exists, send the input strings to relevant ranks
	...
	if (ok) // call to Ipqc with myrank
		ok = Call_IPQC(rankliststringstore[myrank][0], ranklistoutstringstore[myrank][0]);
	...
	int num_signal = 1;
	if (mysize_all > mysize) // if group_IPQC exists, receive the output strings from relevant ranks
	...
	if (ok)
	{
		...
		for (std::size_t j = 0; j < rankrankliststore[myrank].size(); j++)
		{
			...
			ok = ReadOutputIPQC(ranknodeliststore[idy], ranklistoutstringstore[myrank][j], Concentration_buff);
		}
		MPI_Barrier(comm_DDC);
		MPI_Allreduce(Concentration_buff, Concentration, ntot * nodenumber, MPI_DOUBLE, MPI_SUM, comm_DDC);
		MPI_Barrier(comm_DDC);
#else
		ok = ReadOutputIPQC(ranknodeliststore[myrank], ranklistoutstringstore[myrank][0], Concentration);
#endif
	...
	for (std::size_t kk = 0; kk < ranknodeliststore.size(); kk++)
	{
		...
		for (std::size_t jj = 0; jj < ranknodeliststore[kk].size(); jj++)
		{
			...
			{
				// kinetic phases
				for (int i = n1 + 3 + n2 + n3 + n4; i < n1 + 3 + n2 + n3 + n4 + n5; i++)
					pcs_vector[pqc_process[i]]->SetNodeValue(nidx, pqc_index[i] + 0, Concentration[position + i]);
				// additional species
				for (int i = n1 + 3 + n2 + n3 + n4 + n5; i < n1 + 3 + n2 + n3 + n4 + n5 + n6; i++)
					pcs_vector[pqc_process[i]]->SetNodeValue(nidx, pqc_index[i] + 0, Concentration[position + i]);
			}
...
int REACT::WriteInputPQCString(long index, /*ifstream *pqc_iinfile,*/ stringstream* out_buff, bool firstinput)
{
	char line[MAX_ZEILE];
	std::stringstream in;
	...
	while (!pqc_infile.eof())
	{
			...
			*out_buff << "#GRID " << index + 1 << "\n";
			while (line_string.find("#ende") == string::npos)
			{
				...
				if (line_string.find("# comp") != string::npos)
				{
					if (line_string.find("pH") == string::npos && line_string.find("pe") == string::npos)
					{
						...
						// CB 19.1.2011
						// based on porosity, calculate molality mi,w before coputing equilirium chemistry
						if (m_rei)
						{
							if (m_rei->unitconversion)
							{
								idx = pcs_vector[pqc_process[count]]->GetProcessComponentNumber();
			...
			if (this->gamma_Hplus > 0) // pH and H+ in GeoSys species, calculate pH from H+
			{
				dval1 = fabs(pcs_vector[pqc_process[n1 + 1]]->GetNodeValue(
				    index, pqc_index[n1 + 1])); // CB 01/11 probably safer taking abs before calculating log(H+)
				// CB 19.1.2011
				// based on porosity, calculate molality mi,w before coputing equilirium chemistry
				if (m_rei)
			...
			while (line_string.find("#ende") == string::npos)
			{
				...
				if (line_string.find("# comp") != string::npos)
				{
					...
					if (dval < 1.0e-19)
						dval = 0.0;
					// CB 19.1.2011
					// based on porosity, calculate molality mj before coputing equilirium chemistry
					if (m_rei)
					{
						if (m_rei->unitconversion)
						{
		...
		if (line_string.find("GAS_PHASE") != string::npos) // keyword found
		{
			*out_buff << "\n"
			...
			// get necessary values for conversion of molar concentrations to partial pressures, and to calculate total
			...
			// cout << " Pressure: " << press << " = " << dens << " * " << gravity_constant << " * ( " << h << " - " <<
			// z << " ) " << "\n";
		...
		if (line_string.find("USER_PUNCH") != string::npos)
		{
			if (firstinput /*index < 1*/)
			{
				...
				*out_buff << "  -LA(\"e-\")"
				...
				{
					for (i = 0; i < n6; i++)
						*out_buff << " " << 90 + i * 10 << " " << additional_punches[i] << "\n";
				}
			} // end if index < 1
		...
		if (line_string.find("KINETICS") != string::npos)
		{
			...
			*out_buff << "\n"
			...
			while (line_string.find("#ende") == string::npos)
			{
				pqc_infile.getline(line, MAX_ZEILE);
				line_string = line;
				// Check if this is a new kinetic formulation, i.e. kinetics name = speciesname (phase)
				if (line_string.find("# comp") != string::npos)
				{
					in.str(line_string);
					in >> speciesname;
				...
				{ // keyword found
					dval = pcs_vector[pqc_process[found]]->GetNodeValue(index, pqc_index[found]);
					if (dval < 1.0e-19)
						dval = 0.0;
					// CB 19.1.2011
					// based on porosity, calculate molality mj before coputing equilirium chemistry
					if (m_rei)
					{
						if (m_rei->unitconversion)
						{
							idx = pcs_vector[pqc_process[found]]->GetProcessComponentNumber();
							// mi,w = Ci,w * n *55.5 / CH2O
							// mi,s = Ci,w * (1-n) *55.5 / CH2O
							if (cp_vec[idx]->transport_phase == 0) // liquid phase
								dval *= unitfactor_l;
...
int Call_IPQC(stringstream* out_buff, stringstream* outIPQC)
{
	...
	// Load phreeqc database, the old one used in the benchmark for calcite dissolution
...
int REACT::ReadOutputIPQC(std::vector<int> ranknodelist, stringstream* ein, double* m_Conc)
{
	int ok = 0;
	int ntot;
	...
	for (size_t j = 0; j < np; j++) // for all processes
	{
		...
		if (m_pcs->getProcessType() == FiniteElement::MASS_TRANSPORT)
		{
			...
			for (i = 0; i < this->nodenumber; i++)
			{
				...
				this->val_in[comp][i]
				    = m_pcs->GetNodeValue(i, m_pcs->GetNodeValueIndex(m_pcs->pcs_primary_function_name[0]) + timelevel);
	...
	for (size_t j = 0; j < np; j++) // for all processes
	{
		...
		if (pcs->getProcessType() == FiniteElement::MASS_TRANSPORT)
		{
			...
			if (comp >= 0 && (cp_vec[comp]->mobil == 0))
			{
				ic = ic_vector[j];
				ic->setProcess(pcs);
				if (ic->getProcessPrimaryVariable() == FiniteElement::convertPrimaryVariable(this->name[comp]))
				{
			...
			for (i = 0; i < this->elenumber; i++)
			{
				// get concentration values
				this->val_in[comp][i]
				    = pcs->GetElementValue(i, pcs->GetElementValueIndex(pcs->pcs_primary_function_name[0]) + timelevel);
				if ((this->val_in[comp][i] < 0.0) && (strcmp(this->name[comp], "pe") != 0)) // MX, 01.2005
				{
					if (abs(this->val_in[comp][i]) > MKleinsteZahl)
					{
...
int REACT::ReadReactionModelNew(ifstream* pqc_infile)
{
				...
				if (line_string.find("# comp") != string::npos)
				{
					if (line_string.find("pH") == string::npos && line_string.find("pe") == string::npos)
					{
						...
						/*                    if(speciesname.compare("H+") == 0){
						                        ...
						                        this->pqc_Hplus_index = n_master_species-1; // Store index for H+ in
						   vectors
						                    } */
					}
					else if (line_string.find("pH") != string::npos)
						// WW pH_found = 1;
						//                    cout << " pH found in GeoSys species" << "\n";
	...
	if ((this->gamma_Hplus > 0))
		cout << " pH found and H+ found, pH for PHREEQC input is calculated using gamma_H+ and [H+] " << gamma_Hplus
		     << "\n";

	// WW nj=rcml_number_of_master_species + rcml_number_of_equi_phases + rcml_number_of_ion_exchanges +
	// rcml_number_of_gas_species;
	cout << " Found in *.pqc file: " << rcml_number_of_master_species << " master species (excluding pH, H+ and pe), ";
	cout << rcml_number_of_equi_phases << " equilibrium phases,  " << rcml_number_of_ion_exchanges
	     << " ion exchangers, ";
	cout << rcml_number_of_kinetics << " (exclusively) kinetic species, " << rcml_number_of_gas_species
	     << " gas species and ";
		...
		while ((!strstr(str, "END")) && (!strstr(str, "#ende")) && StrOnlyReadStr(sub, str, f, /*TFString,*/ &p))
		{
							...
							else if (strcmp(s, "pH") == 0) /* if pH will change with the reaction! */
							{
								for (i = 0; i < np; i++)
								{
									if (strcmp(s, name[i]) == 0)
										...
										{
											pH_flag = 0;
											//                           printf("Component %s: concentration %lf", s,
											//                           val_in[i][index]);
			...
			else if (!strcmp(sub, "EQUILIBRIUM_PHASES"))
			{
				...
				if (nep != rcml_number_of_equi_phases)
					FilePrintString(
					    f, "Warning: One or more solid phase(s) in EQUILIBRIUM_PHASES not found in *.rfd file!!!");
					...
					if (strstr(str, "-steps"))
					{
						...
						if (index > -1)
						{
							...
							if (time_vector.size() > 0)
								...
								cout << "Error in MPCCalcCharacteristicNumbers: no time discretization data !"
								     << "\n";
			...
			while (line_string.find("#ende") == string::npos)
			{
				...
				if (line_string.find("# comp") != string::npos)
				{
					if (line_string.find("pH") == string::npos && line_string.find("pe") == string::npos)
					{
						...
						speciesname = pqc_names[count];
						dval = pcs_vector[pqc_process[count]]->GetNodeValue(index, pqc_index[count]);
						/*                cout << "Testing index vectors: " << speciesname << ", " << m_pcs->pcs_number
						   << ", "<< idx <<",  With vectors: " << pqc_names[count] << ", " <<
						   pcs_vector[pqc_process[count]]->pcs_number << ", " << pqc_index[count];
						...
						*out_file << speciesname << "       " << dval << "     # comp "
						...
						//                else
						//                    *out_file << speciesname << "       " << dval << " charge " << "       #
						//                    comp " <<"\n";
						//                in.clear();
					}
				}
			...
			if (this->gamma_Hplus > 0) // pH and H+ in GeoSys species, calculate pH from H+
			{
				...
				if (m_rei)
					if (m_rei->unitconversion) // mi,w = Ci,w * n *55.5 / CH2O
						dval1 *= unitfactor_l;
				dval = -log10(dval1 * gamma_Hplus);
				//            if(index<2) cout << " .  Resetting pH to: " << dval << "; MOL[H+]= " << dval1 << ",
				//            gamma_H+ = " << gamma_Hplus;
			}
			if (this->rcml_pH_charge > 0)
				*out_file << "pH"
				...
				if (line_string.find("# comp") != string::npos)
				{
					...
					/*                //m_pcs = PCSGet("MASS_TRANSPORT",speciesname);// CB HS update
					                  m_pcs = cp_vec[cp_name_2_idx[speciesname]]->getProcess();
					                        ...
					                        dval = m_pcs->GetNodeValue(index,idx);
					         */
					//                if(count != (n1+3)) cout << " Error in index pqc " << "\n";
					speciesname = pqc_names[count];
					dval = pcs_vector[pqc_process[count]]->GetNodeValue(index, pqc_index[count]);
					if (dval < 1.0e-19)
						dval = 0.0;
					...
					//                ", "<< idx <<",  With vectors: " << pqc_names[count] << ", " <<
					//                pcs_vector[pqc_process[count]]->pcs_number << ", " << pqc_index[count];
					//                if(index <2) cout << " EQ-Species " << speciesname << " " << dval << "\n";
					// CB 19.1.2011
					// based on porosity, calculate molality mj before coputing equilirium chemistry
					if (m_rei)
					{
					...
					*out_file << speciesname << " " << sat_index << "  " << dval << "       # comp "
					          << "\n";
					in.clear();
				}
				else
			...
			*out_file << "\n"
			...
			while (line_string.find("#ende") == string::npos)
			{
				...
				else if (line_string.find("# comp") != string::npos)
				{
					...
					if (dval < 1.0e-19)
						dval = 0.0;
					// cout << "Testing index vectors: " << speciesname << " , with vectors: " << pqc_names[count] << ",
					// " << pcs_vector[pqc_process[count]]->pcs_number << ", " << pqc_index[count];
					// cout << " Molar mass: " << dval  << ", component / total mass: " << dval/mm << "\n";
					// dval is molar mass of gas component, convert to partial pressure [Pa]
					if (mm > 0.0)
						partial_press = press * dval / mm;
					else
						partial_press = 0.0;
					// cout << " partial pressure: " << partial_press  << ", partial/total press: " <<
					// (partial_press/press) << "\n";
		...
		if (line_string.find("USER_PUNCH") != string::npos)
		{
			if (index < 1)
			{
				...
				*out_file << "  -LA(\"e-\")"
				...
				{
					for (i = 0; i < n6; i++)
						*out_file << " " << 90 + i * 10 << " " << additional_punches[i] << "\n";
					//*out_file << " 90 PUNCH ";
					// for(i=n1+3+n2+n3+n4+n5; i<n1+3+n2+n3+n4+n5+n6; i++) *out_file << " MOL(\"" << pqc_names[i] <<
					// "\"),";
					//*out_file << "\n";
				}
			} // end if index < 1
		...
		if (line_string.find("-steps") != string::npos) // keyword found
		{
			...
			if (time_vector.size() > 0)
				...
				cout << "Error in WriteInputPhreeqc: no time discretization data !"
				     << "\n";
			dval = m_tim->CalcTimeStep();
			*out_file << "-steps " << dval << " in " << this->rcml_number_of_pqcsteps << " steps"
			          << "\n";
		} // end -steps
		//-------------------------------------------------------------------------------------------------------------
		if (line_string.find("KNOBS") != string::npos)
			...
			*out_file << "\n"
			...
			while (line_string.find("#ende") == string::npos)
			{
				...
				{ // keyword found
					...
					////m_pcs = PCSGet("MASS_TRANSPORT", speciesname);// CB HS update
					// m_pcs = cp_vec[cp_name_2_idx[speciesname]]->getProcess();
					// idx = m_pcs->GetNodeValueIndex(speciesname)+1; // new time level
					// dval = m_pcs->GetNodeValue(index,idx);
					*out_file << "-m " << dval << "\n";
				}
				// time discretization for phreeqc
...
// WH: run IPQC
	...
	if (returnCode == 1)
	{
		// Sets the selected-output file switch on, so that phreeqc will write output to the SELECTED_OUTPUT file
		// "phout_sel.dat"
		...
		if (this->rateflag[index] > 0)
		{
			...
			if (m_rei)
			{
				if (m_rei->unitconversion)
				{
					...
					// unitfactor_s = (1 - m_rei->node_porosity[index]) * MOLH2OPERKG / (m_rei->water_conc[index] *
					// m_rei->node_porosity[index] * m_rei->GetWaterSaturation(index));
					// if(unitfactor_s ==0) unitfactor_s = (1 - m_rei->node_porosity[index]) * MOLH2OPERKG /
					// (m_rei->water_conc[index] * m_rei->node_porosity[index] * 1);
				}
			}
			/* skip one line, if keyword steps larger than 1 even more lines */
			for (j = 0; j < anz; j++)
			...
			for (j = n1 + 3 + n2 + n3; j < n1 + 3 + n2 + n3 + n4; j++)
			{
				if (ein >> dval)
				{
					...
					pcs_vector[pqc_process[j]]->SetNodeValue(index, pqc_index[j] + dix, dval);
					if (index < 2)
						cout << " Read gas phase for " << pqc_names[j] << " " << dval << "\n";
				}
			}
			/*--------------------Read the concentration of all (exclusively) kinetic species -------*/
			for (j = n1 + 3 + n2 + n3 + n4; j < n1 + 3 + n2 + n3 + n4 + n5; j++)
	...
	for (size_t j = 0; j < np; j++) // for all processes
	{
		...
		if (pcs->getProcessType() == FiniteElement::MASS_TRANSPORT)
		{
			...
			for (long i = 0; i < nodenumber; i++)
			{
				// get concentration values
				val_in[comp][i]
				    = pcs->GetNodeValue(i, pcs->GetNodeValueIndex(pcs->pcs_primary_function_name[0]) + timelevel);
				// MX, 01.2005
...
void REACT::SetNeighborNodesActive(long startnode, long level, int* help)
{
	...
	/*OK411
	   ...
	   // DisplayMsg(" RSNNA: startnode: "); DisplayLong(startnode); DisplayMsg(", level: "); DisplayLong(level);
	   DisplayMsgLn("");
	   // Ende rekursiv
	   if(level == 0) {
	   help[startnode] = 1;
...
void REACT::GetTransportResults2Element()
{
	...
	for (size_t j = 0; j < np; j++) // for all processes
	{
		...
		if (pcs->getProcessType() == FiniteElement::MASS_TRANSPORT)
		{
			...
			{
				// get concentration values
				val_in[comp][i] = pcs->GetElementValue(i, pcs->GetElementValueIndex(cname) + timelevel);
				// MX, 01.2005
				if ((val_in[comp][i] < 0.0) && (strcmp(name[comp], "pe") != 0))
				{
					if (abs(val_in[comp][i]) > MKleinsteZahl)
...
int REACT::CheckNoReactionNodes(void)
{
	...
	if (aktueller_zeitschritt
	    < 2) // do not in the very first calculation before first time step and in the first time step
...
// TODO: don't try to run phreeqc if output is not ok
	...
	npunch = this->rcml_number_of_master_species + this->rcml_number_of_equi_phases + this->rcml_number_of_ion_exchanges
	...
	if (ok)
	{
		...
		if (!ok)
			std::cout << " [ERROR] ExecuteReactionsPHREEQCNewLib: Error reading phreeqc's output"
			          << "\n";
		else
			std::cout << " [OK]"
			          << "\n";
