...
void SplitMPI_Communicator::Call_IPhreeqc(void)
{
	...
	for (;;)
	{
		...
		MPI_Recv(string_DDC, signal + 1, MPI_CHAR, MPI_ANY_SOURCE, MPI_ANY_TAG, MPI_COMM_WORLD, &status);
		...
		MPI_Send(&strlength, 1, MPI_INT, status.MPI_SOURCE, rank, MPI_COMM_WORLD);
		MPI_Send(message, tmp.length() + 1, MPI_CHAR, status.MPI_SOURCE, rank, MPI_COMM_WORLD);
	}
