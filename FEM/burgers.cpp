...
**************************************************************************/
void SolidBurgers::UpdateBurgersProperties(double s_eff,
...
only. Programing: 06/2014 TN Implementation
**************************************************************************/
void SolidBurgers::CalResidualBurgers(
    const double dt, const KVec& strain_curr, const KVec& stress_curr,
    ...
    const KVec& strain_Max_t, Eigen::Matrix<double, 18, 1>& res)
{
    ...
    res.block<6, 1>(12, 0) = 1. / dt * (strain_Max_curr - strain_Max_t) -
                             GM / (2. * etaM) * stress_curr;
}
...
**************************************************************************/
void SolidBurgers::CalJacobianBurgers(const double dt,
                                      ...
                                      const KVec& eps_K_i)
{
    ...
    if (s_eff > 0.)
    {
        ...
        const KVec eps_K_aid =
            1. / (etaK * etaK) * (GM * sig_i - 2. * GK * eps_K_i);

        // build G_21
        Jac.block<6, 6>(6, 0) += 0.5 * eps_K_aid * dmu_vK.transpose() +
                                 1. / etaK * eps_K_i * dG_K.transpose();

        // build G_31
        Jac.block<6, 6>(12, 0) +=
            GM / (2. * etaM * etaM) * sig_i * dmu_vM.transpose();
    }
}
...
strain. Implementation fully implicit only. Programing: 06/2014 TN
Implementation
**************************************************************************/
