...
REACTINT::REACTINT(void)
{
    ...
    // double mv[8];
    // mv[0]=0; mv[1]=2.71; mv[2]=0; mv[3]=0; mv[4]=0; mv[5]=2.71; mv[6]=0;
    // mv[7]=0; double test = Density_CO2brine(393.15, 202, 2.71, 0.0); test =
    // Density_CO2_MultiBrine(393.15, 202, mv, 0);
...
bool REACTINT::Read(ifstream* rfd_file)
{
    ...
    while (!new_keyword)
    {
        ...
        {
            new_keyword = true;
            rfd_file->seekg(index);  // Dateipointer zurücksetzen, sonst ist das
                                     // nächste keyword weg
            break;
        }
        ...
        {  // subkeyword found
            ...
            if (dummystring.compare("VOLUME") == 0)
                ...
                cout << " Warning in REACTINT::Read - No valid keyword for "
                        "$MOL_PER (unit conversions)."
                     << "\n";
            in.clear();
        }
        ...
        // co2 species name
        if (line_string.find("$DISSOLVED_NEUTRAL_CO2_SPECIES_NAME") !=
            string::npos)
        {  // subkeyword found
            in.str(GetLineFromFile1(rfd_file));
            in >> NeutralCO2name;
        ...
        {  // subkeyword found
            ...
            if (dummystring.compare("RECORD") == 0)
                ...
                cout << " Warning in REACTINT::Read - No valid keyword for "
                        "$RESIDUAL (for equilibrium reaction input "
                        "value)."
                     << "\n";
            in.clear();
        }
        ...
        if (line_string.find("$UPDATE_INITIAL_SOLID_COMPOSITION") !=
            string::npos)
        {  // subkeyword found
            icSolidUpdate = true;
        }
            ...
            while (1)
            {
                ...
                for (j = 1; j < (int)string2vector(line_str2).size(); j += 2)
                {
                    ...
                    else if (ncomp_x.substr(0, 3) == "pow")
                    {
                        ...
                        this->pow_stoi_init.push_back(
                            (double)atof(ncomp_x.substr(3).c_str()));
                    }
                    else
...
void REACTINT::InitREACTINT(void)
{
    ...
    if (unitconversion)
    {
        if (constantdensity == false)
        {
            ...
            {  // set the species relevant for water concentration
                ...
                {
                    if (cp_vec[i]->compname.compare(water_conc_species[j]) ==
                            0 &&
                        cp_vec[i]->transport_phase == 0)
                    {                                     // species found
                        if (cp_vec[i]->iupac_formula[0])  // a formula has been
                                                          // defined
                            id = formula2index(cp_vec[i]->iupac_formula);
                        else  // otherwise use species name directly. attention,
                              // this could result in wrong concentration
                            // of basis species!!!
        ...
        // this should be called as well after transport and before any reaction
        // calculations necessary here as well for calculation of initial
        // equilibrium
        CalcWaterConc();
    }  // if(unitconversion)
    ...
    if (s_water_limit && WaterSatLimit > 0)
        for (i = 0; i < nnodes; i++)
            dried_out_nodes.push_back(
                false);  // initialize for eclipse coupling
    ...
    if (vle_flag == true)
    {
        ...
        for (i = 0; i < long(this->VLE_conditions.size()); i++)
        {
            ...
            {
                if (pcs_vector[j]->pcs_primary_function_name[0] ==
                    VLE_conditions[i].aq_name)
                {
        ...
        for (i = 0; i < (int)pcs_rename0_init.size(); i++)
        {
            ...
            if (is_idx == 0)
            {
                cout << " Warning!!!, line " << i + 1
                     << " can not be found in PCS list ! "
                     << "\n";
                exit(0);
            }
            cout << " " << setw(12) << pcs_rename0_init[i] << " <--- ";
            for (jx = 0; jx < (int)pcs_rename1_init[i].size(); jx++)
                cout << "    " << pcs_rename1_stoi_init[i][jx] << "  "
                     << pcs_rename1_init[i][jx];
            cout << "\n";
        }
    }
    ...
    if (pcs_rename_pre_flag == true)
    {
        ...
        for (i = 0; i < (int)pcs_rename0_pre.size(); i++)
        {
            ...
            for (jx = 0; jx < (int)pcs_rename1_pre[i].size(); jx++)
                cout << "    " << pcs_rename1_stoi_pre[i][jx] << "  "
    ...
    if (pcs_rename_post_flag == true)
    {
        ...
        for (i = 0; i < (int)pcs_rename0_post.size(); i++)
        {
            ...
            for (jx = 0; jx < (int)pcs_rename1_post[i].size(); jx++)
                cout << "    " << pcs_rename1_stoi_post[i][jx] << "  "
                     << pcs_rename1_post[i][jx];
            cout << "\n";
        }
    }
        ...
        {  // node loop
            ...
            for (i = 0; i < (int)pcs_rename0_idx_init.size(); i++)
            {
                ...
                for (ix = 0; ix < (int)pcs_rename1_idx_init[i].size(); ix++)
                {
                    ...
                    {
                        value += pow(m_pcs->GetNodeValue(ii, f),
                                     this->pow_stoi_init[ic]);
                        ic++;
                    }
                    else
                        value += m_pcs->GetNodeValue(ii, f) *
                                 pcs_rename1_stoi_init[i][ix];
                }
        ...
        {  // node loop
            ...
            for (i = 0; i < (int)pcs_rename0_idx_pre.size(); i++)
            {
                ...
                for (ix = 0; ix < (int)pcs_rename1_idx_pre[i].size(); ix++)
                {
                    ...
                    {
                        value += pow(m_pcs->GetNodeValue(ii, f),
                                     this->pow_stoi_pre[ic]);
                        ic++;
                    }
                    else
                        value += m_pcs->GetNodeValue(ii, f) *
                                 pcs_rename1_stoi_pre[i][ix];
                }
...
void REACTINT::ReactionPostProcessing(bool initial)
{
    ...
    if (initial)  // only after initial reactions, i.e. before first real time
                  // step
            ...
            {  // node loop
                ...
                for (i = 0; i < (int)pcs_rename0_idx_post.size(); i++)
                {
                    ...
                    for (ix = 0; ix < (int)pcs_rename1_idx_post[i].size(); ix++)
                    {
                        ...
                        {
                            value += pow(m_pcs->GetNodeValue(ii, f),
                                         this->pow_stoi_post[ic]);
                            ic++;
                        }
                        else
                            value += m_pcs->GetNodeValue(ii, f) *
                                     pcs_rename1_stoi_post[i][ix];
                    }
...
double REACTINT::GetPressure(long node)
{
    ...
    if (aktueller_zeitschritt == 0)
        timelevel = 0;
    // get the Pressure info Todo: check this function for correct pressure
    // conversion
    m_pcs_p = PCSGetFlow();
    ...
    switch (flowtype)
    {
        ...
        case 1212:  // Multiphase Flow
            idxp =
                m_pcs_p->GetNodeValueIndex("PRESSURE1") + timelevel;  // p_cap
            idxp2 =
                m_pcs_p->GetNodeValueIndex("PRESSURE2") + timelevel;  // p_nw
            factor = 1 / 1.0e+5;
            ppmodel = true;
            break;
...
void REACTINT::CopyAllConcentrationsToOtherTimeLevel(bool forward)
{
    ...
    for (i = 0; i < int(cp_vec.size()); i++)
        cp_vec[i]->getProcess()->CopyTimestepNODValues(
            forward);  // true: new->old
    ...
    if (aktueller_zeitschritt == 1)
    {
        ...
        for (i = 0; i < nnode; i++)
        {
            for (j = 0; j < long(Temp_GHP_xyz.size()); j++)
                if (XYZ[i][0] ==
                    Temp_GHP_xyz[j][0])  // x cordinate matches -> only regular
                                         // mesh so far
...
void REACTINT::ReadRestartNodePoro(long nnodes)
{
    ...
    // now set the new element porosities from node_porosity for mass transport
    // and other processes loop over all elements
...
void REACTINT::CalcWaterConc(void)
{
    ...
    {
        m_cp = cp_vec[j];
        if (m_cp->compname.compare(WaterSpeciesName) == 0 ||
            m_cp->compname.compare("H2O") == 0 ||
    ...
    if (constantdensity)
    {
        for (i = 0; i < nnodes; i++)
        {
            water_conc[i] =
                m_mfp->Density() *
                MOLH2OPERKG;  // here density of pure H2O phase must be used
    ...
    {
        m_cp = cp_vec[j];
        if (m_cp->compname.compare(this->NeutralCO2name) == 0 ||
            m_cp->compname.compare("CO2_w") == 0 ||
    ...
    for (i = 0; i < nnodes; i++)
    {
        ...
        for (j = 0; j < nc; j++)
        {
            // get concentration; this requires existence of vectors sp_pcsind
            // and sp_var_ind todo: what about double species, e.g. primary and
            // secondary species?? conc =
            // pcs_vector[sp_pcsind[j]]->GetNodeValue(i,sp_varind[j]);// CB HS
            // update if(timeflag &! cp_vec[j]->mobil)
            //  conc = cp_vec[j]->getProcess()->GetNodeValue(i,sp_varind[j]-1);
            // else
            conc = cp_vec[j]->getProcess()->GetNodeValue(
                i, sp_varind[j]);  // all concentrations are available at new TL
            if (conc < 0)
                conc = 0;
        ...
        {
            // cv_CO2 =
            // pcs_vector[sp_pcsind[idxCO2]]->GetNodeValue(i,sp_varind[idxCO2]);//
            // CB HS update if(timeflag &! cp_vec[idxCO2]->mobil)
            //  cv_CO2 =
            //  cp_vec[idxCO2]->getProcess()->GetNodeValue(i,sp_varind[idxCO2]-1);
            // else
            cv_CO2 = cp_vec[idxCO2]->getProcess()->GetNodeValue(
                i, sp_varind[idxCO2]);  // all concentrations are available at
                                        // new TL
        }
        else
            cv_CO2 = 0;
        // calculate and store water concentration for this node
        water_conc[i] = density::concentration_water(
            TT, PP, cv, cv_CO2);  // CB removed from ifdef OGS_FEM_CAP
        // set the corrsponding node value of H2O transport process
        // pcs_vector[sp_pcsind[widx]]->SetNodeValue(i,1,water_conc[i]);// CB HS
        // update
        cp_vec[widx]->getProcess()->SetNodeValue(i, 1, water_conc[i]);
    }
 ...
 **************************************************************************/
void REACTINT::CalcUnitConversionFactors(long index, double* fl, double* fs,
                                         bool molal)
{
    if (molal)
    {  // mol/m³-->mol/kgw
        *fl = MOLH2OPERKG /
              water_conc[index];  // mol/kgH2O * m³liquid/mol = m³l/kg
        *fs = (1 - node_porosity[index]) * MOLH2OPERKG /
              (water_conc[index] * node_porosity[index] *
               GetWaterSaturation(index));
        if (*fs == 0)
            *fs = (1 - node_porosity[index]) * MOLH2OPERKG /
                  (water_conc[index] * node_porosity[index] * 1);
    }
    else
...
double REACTINT::GetCO2SolubilityDuan(long node)
{
    ...
    if (Naidx >= 0)
        Sal = cp_vec[Naidx]->getProcess()->GetNodeValue(
            node, sp_varind[Naidx]);  // mol/m³l
...
void REACTINT::SetInitialPorosityAndPermToElementValue()
{
    ...
    if (k == 0)
    {
        cout << " Warning in REACINT: molar_weight has not been defined for "
                "ANY solid phase species."
             << "\n";
    ...
    if (kk == 0)
    {
        cout << " Warning in REACINT: mineral_density has not been defined for "
                "ANY solid phase species."
             << "\n";
        cout << "  No porosity update possible. "
             << "\n";
    ...
    if (idx_n < 0)
        cout << " Warning in REACINT: No POROSITY ele index found with PCS "
             << m_pcs_flow->getProcessType() << "."
             << "\n";
    idx_k = m_pcs_flow->GetElementValueIndex("PERMEABILITY");
    if (idx_k < 0)
        cout << " Warning in REACINT: No PERMEABILITY ele index found with PCS "
             << m_pcs_flow->getProcessType() << "."
             << "\n";
    idx_k_yy = m_pcs_flow->GetElementValueIndex("PERMEABILITY_YY");
    if (idx_k_yy < 0)
        cout << " Warning in REACINT: No PERMEABILITY_YY ele index found with "
                "PCS "
             << m_pcs_flow->getProcessType() << "."
             << "\n";
    idx_k_zz = m_pcs_flow->GetElementValueIndex("PERMEABILITY_ZZ");
    if (idx_k_zz < 0)
        cout << " Warning in REACINT: No PERMEABILITY_ZZ ele index found with "
                "PCS "
             << m_pcs_flow->getProcessType() << "."
             << "\n";
    ...
    for (long n = 0; n < n_ele; n++)
    {
        ...
        if ((m_mat_mp->porosity_model == 13) && (aktueller_zeitschritt == 0))
        {
            ...
            m_pcs_flow->SetElementValue(n, idx_n + 1,
                                        n_initial);  // new time level

            // Isotropic permeability
            if (m_mat_mp->permeability_tensor_type == 0)
            {
                k_initial_xx = m_mat_mp->permeability_tensor[0];
                m_pcs_flow->SetElementValue(n, idx_k,
                                            k_initial_xx);  // old time level
                m_pcs_flow->SetElementValue(n, idx_k + 1,
                                            k_initial_xx);  // new time level
            }
            ...
            else if (m_mat_mp->permeability_tensor_type == 1)
            {
                if (m_mat_mp->GetGeoDimension() == 2)
                {
                    ...
                    m_pcs_flow->SetElementValue(
                        n, idx_k, k_initial_xx);  // old time level
                    m_pcs_flow->SetElementValue(
                        n, idx_k + 1, k_initial_xx);  // new time level

                    m_pcs_flow->SetElementValue(
                        n, idx_k_yy, k_initial_yy);  // old time level
                    m_pcs_flow->SetElementValue(
                        n, idx_k_yy + 1, k_initial_yy);  // new time level
                }
                else if (m_mat_mp->GetGeoDimension() == 3)
                {
                    ...
                    m_pcs_flow->SetElementValue(
                        n, idx_k_zz, k_initial_zz);  // old time level
                    m_pcs_flow->SetElementValue(
                        n, idx_k_zz + 1, k_initial_zz);  // new time level
                }
            }
        }
    }
...
Task: Porosity variation owing to chemical reactions
      This fuction works when species/mineral concentration is given in [mol/m3
solid].
...
void REACTINT::PorosityVolumetricReactionUpdate()
{
    ...
    // the factors depend on units of concentration, molweight and
    // mineraldensity I presume SI units, mol, kg, m³ this converts mol/m³ to m³
    // porosity change
    if (unitconversion)
        unitfactor =
            1;  // V=C*MW/rho*unitfactor : [-] = [mol/m³] * [kg/mol] * [m³/kg]
    // this converts mol/L (or mol/kg) to L porosity change
    else
        unitfactor = 1000;  // V=C*MW/rho*unitfactor : [-] = [mol/kg] * [kg/mol]
                            // * [m³/kg] * [kg/cm³]
    ...
    // CB: Instead of using the initial porosity always, we have to properly
    // rescale concentrations
    //     of species by the new porosity or solid phase volume after each time
    //     step. see below ###
    ...
    for (long n = 0; n < nnodes; n++)
    {
        ...
        for (i = 0; i < nImmobileComp; i++)
        {
            ...
            idxC = cp_vec[l]->getProcess()->GetNodeValueIndex(
                cp_vec[l]->getProcess()->pcs_primary_function_name[0]);
            conc_old = cp_vec[l]->getProcess()->GetNodeValue(
                n, idxC);  // old timelevel
            conc_new = cp_vec[l]->getProcess()->GetNodeValue(
                n, (idxC + 1));  // new timelevel
            change_in_conc = conc_new - conc_old;
            // mineral_volume_fraction =
            // change_in_conc[mol/m³solid]*molar_volume[m³min/mol] the last
            // factor depends on units of concentration, molweight and
            // mineraldensity
            if (mineral_densities_vector[i] > 0)
                mineral_volume_fraction +=
                    change_in_conc *
                    (mineral_molecular_weights_vector[i] /
                     mineral_densities_vector[i]) *
                    unitfactor;
            else
            {
                cout << " Warning: mineral_density = 0 for a species in "
                        "REACTINT::PorosityVolumetricReactionUpdate()"
                     << "\n";
        ...
        if (KinReactData_vector.size() > 0)
        {
            ...
            {
                if (n == 1)
                    cout << " Attention: Lagneau-Benchmark with modified poro "
                            "update"
                         << "\n";
                int sp = 0;
                ...
                phasevolume = n_previous;
                conc_old = cp_vec[sp]->getProcess()->GetNodeValue(
                    n, idxC - 1);  // old timelevel
                conc_new = cp_vec[sp]->getProcess()->GetNodeValue(
                    n, (idxC));  // new timelevel
                change_in_conc = conc_new - conc_old;
                mineral_volume_fraction =
                    change_in_conc *
                    (cp_vec[sp]->molar_weight / cp_vec[sp]->mineral_density) *
                    unitfactor;
                node_porosity[n] =
                    n_previous - (phasevolume * mineral_volume_fraction);
            }
        }
        // ###
        // CB: Here, we should include a rescaling of concentrations in the
        // phases,
        ...
        for (i = 0; i < ns; i++)
        {
            // l    = sp_pcsind[i];
            idxC = sp_varind[i];
            conc_new = cp_vec[i]->getProcess()->GetNodeValue(
                n, idxC);  // new timelevel
            if (unitconversion)
            {  // liquid and solid phase concentrations mol/m³phase
                if (cp_vec[i]->transport_phase == 1)
                    conc_new *= (1 - n_previous) / (1 - node_porosity[n]);
                // for dissolved species, is this handled by dn/dt term in
                // transport equation?? then comment out next two lines
                else if (cp_vec[i]->transport_phase == 0)
                    conc_new *= n_previous / node_porosity[n];
            }
            else  // in this case, alls species are related to liquid phase
                  // mol/kgH2O
                conc_new *= n_previous / node_porosity[n];
...
void REACTINT::CopySymmetricConcentrationsInRadialModel()
{
    ...
    //  cout << "No CKinReactData available in
    //  CopySymmetricConcentrationsInRadialModel" << "\n";
    //   return;
    // }
...
void REACTINT::DumpSolidSpeciesMoles(void)
{
    ...
    {
        if (aktueller_zeitschritt == 1 ||
            aktueller_zeitschritt % ssp_outstep == 0 ||
            steps == int(aktueller_zeitschritt))
            plot = true;
        else
            plot = false;
    ...
    for (i = 0; i < nnodes; i++)
    {
        ...
        {
            if (cp_vec[j]->transport_phase ==
                1)  // plot total mol / m³ aquifer = c*(1-n)
                aus << cp_vec[j]->getProcess()->GetNodeValue(i, sp_varind[j]) *
                           (1 - node_porosity[i])
                    << " ";
        }
        aus << node_porosity[i] << "\n";
...
void REACTINT::DumpAllVariables()
{
    ...
    if (aktueller_zeitschritt % this->pcs_outstep == 0 ||
        step == int(aktueller_zeitschritt))
        plot = true;
    ...
    for (long i = 0; i < long(pcs_vector.size()); i++)
    {
        for (int j = 0; j < pcs_vector[i]->pcs_number_of_primary_nvals; j++)
        {
            ...
            idx = pcs_vector[i]->GetNodeValueIndex(
                      pcs_vector[i]->nod_val_name_vector[j * 2]) +
                  1;
            // Restartfiles ; each PV name is stored twice: old TL, new TL
            file = FileName + "_" + pcs_vector[i]->nod_val_name_vector[j * 2] +
                   ".dump";
...
void REACTINT::DumpMassIntegrals()
{
    ...
    {  // new file
        ...
        for (i = 0; i < nelems; i++)
        {
            ...
            for (j = 0; j < nnodes; j++)
            {
                ...
                for (k = 0; k < cp_vec.size(); k++)
                {
                    massintegrals[k] += cp_vec[k]->getProcess()->GetNodeValue(
                                            nod, sp_varind[k] - 1) /
                                        nnodes * volume;
                }
            }
    ...
    for (i = 0; i < nelems; i++)
    {
        ...
        for (j = 0; j < nnodes; j++)
        {
            ...
            {
                massintegrals[k] +=
                    cp_vec[k]->getProcess()->GetNodeValue(nod, sp_varind[k]) /
                    nnodes * volume;
            }
        }
    }
...
void REACTINT::PermeabilityPorosityUpdate()
{
    ...
    for (n = 0; n < n_ele; n++)
    {
        ...
        if (((m_mat_mp->permeability_tensor_type != 0) ||
             (m_mat_mp->permeability_tensor_type != 1)) &&
        ...
        n_t = m_pcs_flow->GetElementValue(
            n, idx_n + 1);  // new time level, recently set in
                            // PorosityVolumetricReactionUpdate
        ...
        if (m_mat_mp->permeability_tensor_type == 1)
        {
            if (m_mat_mp->GetGeoDimension() == 2)
            {
                ...
                if (m_mat_mp->permeability_porosity_updating_type == 0)
                {
                    k_t_new = m_mat_mp->KozenyCarmanNew(k_init, n_init, n_t);
                    k_t_new_yy =
                        m_mat_mp->KozenyCarmanNew(k_init_yy, n_init, n_t);
                }
            ...
            if (m_mat_mp->GetGeoDimension() == 3)
            {
                ...
                if (m_mat_mp->permeability_porosity_updating_type == 0)
                {
                    ...
                    k_t_new_yy =
                        m_mat_mp->KozenyCarmanNew(k_init_yy, n_init, n_t);
                    k_t_new_zz =
                        m_mat_mp->KozenyCarmanNew(k_init_zz, n_init, n_t);
                }
...
double GetNodePhaseVolume(long node, double theta, int phase)
{
    ...
    for (el = 0; el < (int)m_nod->getConnectedElementIDs().size(); el++)
    {
        ...
        {
            case 0:  // pore space
                poro = m_mat_mp->Porosity(
                    elem, theta);  // CB Now provides also heterogeneous
                                   // porosity, model 11
                break;
            case 1:                                // solid phase
...
double REACTINT::GetWaterSaturation(long index)
{
    ...
    switch (flowtype)
    {
        ...
        case 1212:  // Multiphase Flow
            m_pcs = PCSGetFlow();
            idx =
                m_pcs->GetNodeValueIndex("SATURATION1");  // Sat of water phase
...
// 2012.06.13 DL
void VLE_CalcNewPressure(double T, double& P, double& V_gas, double& V_liquid,
                         double CO2_gas_old, double CO2_gas_new,
                         double CO2_liquid_old, double CO2_liquid_new,
                         double H2O_liquid_old, double H2O_liquid_new,
                         double& rho)
{
...
void VLE_isobaric_fixphase(double T, double P, Phase_Properties2& vapor,
                           Phase_Properties2& liquid,
                           Phase_Properties2& /*solid*/, int /*f*/)
{
    ...
    liquid.mass =
        liquid.H2O * 18.01528 + liquid.CO2 * 44.009 + liquid.NaCl * 58.443;
...
void VLE_isochoric_fixphase(double T, double& P, Phase_Properties2& vapor,
                            Phase_Properties2& liquid, Phase_Properties2& solid,
                            int f)
{
    int i = 0, i_max = 31;
    ...
    if (i == i_max)
    {
        cout << "VLE_isochoric_fixphase() max.it.=" << i
             << " reached, dV=" << abs(V - V0) << " rho=";
