...
double dDELTA_ddelta(
    double delta, double A, double theta_fn, double beta, double B, double a)
{
    return (delta - 1) *
           (A * theta_fn * 2 / beta *
...
double d2DELTApowb_ddelta2(double b,
                           double delta_fn,
                           double d2DELTA_deriv,
                           double dDELTA_deriv)
{
...
double d2phi_ddeltadtau(
    double C, double D, double delta, double tau, double phi_fn)
{
    ...
    for (i = 0; i < limit[3]; i++)
    {
        ...
        else
        {
            ...
            else if (i < limit[3])
            {
                ...
                dDELTA_deriv = dDELTA_ddelta(
                    delta, K[6][i], THETA, K[11][i], K[7][i], K[4][i]);
                dDELTApowbddelta =
...
double CFluidProperties::phi_r_dd(double rho, double T) const
{
    ...
    for (i = limit[2]; i < limit[3]; i++)
    {
        ...
        dphiddelta = dphi_ddelta(K[8][i], delta, phifn);
        d2DELTA_deriv = d2DELTA_ddelta2(
            delta, dDELTA_deriv, K[6][i], thetafn, K[11][i], K[7][i], K[4][i]);
        d2DELTApowbddelta2 =
    ...
    switch (c)
    {
        case 1:  // H2O
            res = 1.5630770E+03 +
                  T * (1.6037550E+00 +
                       T * (-2.9327840E-03 +
                            T * (3.2161010E-06 - 1.1568270E-09 * T)));
            break;
        case 3:  // N2
            res = 9.7904300E+02 +
                  T * (4.1796390E-01 +
                       T * (-1.1762790E-03 +
                            T * (1.6743940E-06 - 7.2562970E-10 * T)));
            break;
        default:
            std::cout << "WARNING: Fluid not specified in "
                         "polynomial_heat_capacity. Setting cp to 1000.\n";
            return 1000.0;
    }
...
/**********************************************************************
 ...
 Temperature. Programming: NB Aug 2008
 ***********************************************************************/
...
/**********************************************************************
 ...
 Temperature. (Vesovic&Wakeham) Programming: NB 4.8.01 Nov 2008
 ***********************************************************************/
...
double Fluid_Viscosity(double rho, double T, double p, int fluid)
{
    ...
    switch (fluid)
    {
        ...
        case 1:  // WATER
            ...
            {
                return MaterialLib::Fluid::WaterViscosityIAPWS::getValue(T,
                                                                         rho);
            }
            break;
        case 2:  // METHANE
...
double preos(const CFluidProperties* mfp, double T, double P)
{
    ...
    b = 0.077796 * Ru * Tc / pc;
    P = P / 1000;  // P in kPa
    alpha = ((1 + (0.37464 + 1.5422 * omega - 0.26992 * omega * omega) *
                      (1 - sqrt(T / Tc)))) *
            ((1 + (0.37464 + 1.5422 * omega - 0.26992 * omega * omega) *
                      (1 - sqrt(T / Tc))));
...
double ch4_heat_conductivity(double rho, double T)
{
    ...
    rho_sigma_v =
        P_sigma / (R * T) *
        (1 + P_sigma * MathLib::fastpow(tau, 8) * (Z_c - 1) / P_c *
                 (1 + (J[0] * pow(T_star, beta) + J[1] * pow(T_star, 2 * beta) +
    ...
    for (i = 0; i < 7; i++)  // (Eq. 17)
        ...
        sum = sum + j[i] * MathLib::fastpow(delta, r[i]) *
                        MathLib::fastpow(tau, s[i]);
...
double vapour_pressure_ch4(double T)
{
    ...
    theta = (1 - T / Tc);
    h = Tc / T *
        (n1 * theta + n2 * pow(theta, 1.5) + n3 * theta * theta +
         n4 * pow(theta, 4.5));
...
specific_gas_constant) returns the thermal properties of a given fluid
...
// w)
double DuanMixCompressibility(double T,
                              double P,
                              double V,
...
double bip(int number, int fluid_a, int fluid_b, int i, int j, int k, double T)
{
    ...
    // parameters i,j,k  : needed for mixing rule T : bip depends on Temperature
