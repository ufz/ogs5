...
double CFluidProperties::MassFraction(long number, int comp, double* gp, double theta, CFiniteElementStd* assem)
{
	...
	if (mode == 0) // Gauss point values
		...
		gas_pressure = (1. - theta) * GetNodeVal(number, nidx0) + theta * GetNodeVal(number, nidx1);
	nidx0 = PCSGetNODValueIndex("TEMPERATURE1", 0);
	nidx1 = PCSGetNODValueIndex("TEMPERATURE1", 1);
	if (mode == 0) // Gauss point values
		...
		temperature = (1. - theta) * GetNodeVal(number, nidx0) + theta * GetNodeVal(number, nidx1);
	gas_density = Density();
	...
	{
		case 0: /* gas phase */
			mass_fraction_air_in_gas = ((gas_pressure - vapour_pressure) * FluidConstant::ComponentMolarMassAir())
		...
		case 1: /* liquid phase */
			...
			mass_fraction_air_in_liquid = FluidConstant::ComponentMolarMassAir()
			                              / (FluidConstant::ComponentMolarMassAir()
			                                 - FluidConstant::ComponentMolarMassWater()
			                                       * (1.0 - 1.0 / (henry_constant * (gas_pressure - vapour_pressure))));
...
double CFluidProperties::DensityTemperatureDependence(long number, int comp, double* gp, double theta)
{
	...
	dvapour_pressure_dT = FluidConstant::ComponentMolarMassWater() * Enthalpy(comp, temperature)
	                      / (FluidConstant::GasConstant() * temperature * temperature) * vapour_pressure;
