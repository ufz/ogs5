...
const std::string velocity_name[3][4] = {{"VELOCITY_X1", "VELOCITY_Y1", "VELOCITY_Z1", "NODAL_VELOCITY1"},
...
unsigned char CVTK::GetVTKCellType(const MshElemType::type ele_type)
{
	...
	switch (ele_type)
	{
		...
		case MshElemType::PYRAMID: // pyramid=14
			...
			break;
		default:
			std::cerr << "***ERROR: NO CORRESPONDING VTK CELL TYPE FOUND. (ELEMENT TYPE=" << ele_type << ")"
			          << "\n";
	}
	return cell_type;
}
...
bool CVTK::WriteXMLUnstructuredGrid(const std::string& vtkfile, COutput* out, const int time_step_number)
{
	...
	fin << ">"
	    << "\n";
	//  fin << "<VTKFile type=\"UnstructuredGrid\" version=\"0.1\" byte_order=\"LittleEndian\"
	//  compressor=\"vtkZLibDataCompressor\">"  << "\n";

	//# Unstructured Grid information
	fin << "  <UnstructuredGrid>"
	    << "\n";
	fin << "    <Piece NumberOfPoints=\"" << msh->GetNodesNumber(false) << "\" NumberOfCells=\""
	    << msh->ele_vector.size() << "\">"
	    << "\n";
	//....................................................................
	// Nodes
	...
	for (size_t i = 0; i < out->mfp_value_vector.size(); i++)
	{
		...
		if (output_data)
		{
			if (!useBinary)
			{
				...
				for (size_t j = 0; j < msh->GetNodesNumber(false); j++)
				{
					const double v = MFPGetNodeValue(msh->nod_vector[j]->GetIndex(), mfp_name,
...
bool CVTK::WriteElementValue(std::fstream& fin, bool output_data, COutput* out, CFEMesh* msh, long& offset)
{
	...
	for (int i = 0; i < (int)ele_value_index_vector.size(); i++)
	{
		...
		if (!useBinary || !output_data)
			WriteDataArrayHeader(fin, this->type_Double, out->getElementValueVector()[i], 0, str_format, offset);
		if (output_data)
		{
			if (!useBinary)
			{
