...
#define CP_FILE_EXTENSION \
    ".mcp" /* File extension for component properties input file */
#include <fstream>
#include <map>
#include <string>
#include <vector>
...
public:
    ...
    size_t idx; /* the unique index of this component. not effective, saved for
                   future*/
    ...
    int fluid_id;  // Requred to detect a particular fluid from *.mcp files: 0
                   // for CO2; 1 for H2O; 2 for CH4; 3 for N2
    double Vm;     // Vm [m3/kmol]: Molar volume used in interation parameter
                   // calculation
    ...
    int count_of_diffusion_model_values; /* Anzahl der Parameter zur
                                            Spezifikation des Diffusionsmodells
                                          */
    ...
    int count_of_decay_model_values; /* Anzahl und Werte zur Spezifikation der
                                      */
    double decay_model_values[10];   /* Parameter fuer Zerfallsprozess wie z.B.
                                        Zerfallsrate */
    int decay_function_name;
    int GetNumberDecayValuesCompProperties(
        int); /* Zugriff auf Number of Parameters */
    ...
    //      3-5  double k_acid, k_neutral,k_base; // dissolution/precipitation
    //      rate constants 6-11  double p1,q1,p2,q2,p2,q2; // exponents for
    //      omega 12,13, 14  double n_1, n_2, n_3; // exponents for acidic
    //      neutral and base cases for species one append for each species
    ...
    std::ios::pos_type Read(
        std::ifstream*); /* Lesefunktion f? eine Instanz von CompProperties */
    void Write(std::ofstream*); /* Schreibfunktion f? eine Instanz von
                                   CompProperties */

    /* Member - Functions */
    double CalcDiffusionCoefficientCP(long index, double theta,
                                      CRFProcess* m_pcs);
    double CalcDiffusionCoefficientCP_Method1(long index, double T, double P,
                                              double eta);
    // OK411 double CalcElementRetardationFactor( long index, double*gp, double
    // theta );
    double CalcElementRetardationFactorNew(long index, double* gp,
                                           CRFProcess* m_pcs);
