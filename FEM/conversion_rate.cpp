...
#endif
	{
// X_H = max(tol_l,X_H); //lower tolerance to avoid oscillations at onset of hydration reaction. Set here so that no
// residual reaction rate occurs at end of hydration.
		...
		if (X_H == tol_u || rho_s == rho_up)
			...
			dXdt = 13945.0 * exp(-89486.0 / R / T_s) * std::pow(p_r_g / p_eq - 1.0, 0.83) * 3.0
	...
	{
// X_D = max(tol_l,X_D); //lower tolerance to avoid oscillations at onset of dehydration reaction. Set here so that no
// residual reaction rate occurs at end of dehydration.
#ifdef SIMPLE_KINETICS // this is from P. Schmidt
	...
	if (T_s < T_eq) // oxidation
	{
		...
		else
		{
			...
			dXdt = 55271.0 * exp(-95.493e3 / (R * T_s)) * Avrami
			       * (X_D)*std::pow(-1.0 * log(X_D), (Avrami - 1.0) / Avrami) * std::pow(1.0 - T_s / T_eq, 0.86);
		}
		if (p_r_g <= 1.0e-3)
			dXdt = 0.0;
	}
...
double conversion_rate::get_ps(const double Tads)
{
	...
	const double K[]
	    = {c[0] * theta + c[1] * pow(theta, 2) + c[2] * pow(theta, 3) + c[3] * pow(theta, 4) + c[4] * pow(theta, 5),
...
double conversion_rate::get_hv(double Tads) // in kJ/kg
{
	...
	if (Tads <= 10.)
	{
		const double c[] = {2.50052e3,  -2.1068,     -3.57500e-1, 1.905843e-1, -5.11041e-2,
		                    7.52511e-3, -6.14313e-4, 2.59674e-5,  -4.421e-7};
		double hv = 0.;
	...
	{
		const double c[] = {2.50043e3,   -2.35209,    1.91685e-4, -1.94824e-5, 2.89539e-7, -3.51199e-9,
		                    2.06926e-11, -6.4067e-14, 8.518e-17,  1.558e-20,   -1.122e-22};
		double hv = 0.;
		for (size_t i = 0; i < sizeof(c) / sizeof(c[0]); i++)
			hv += c[i] * std::pow(Tads, static_cast<int>(i));
		return hv;
...
{
	const double c[] = {4.224,         -3.716e-3,    9.351e-5,      -7.1786e-7, -9.1266e-9,  2.69247e-10,
	                    -2.773104e-12, 1.553177e-14, -4.982795e-17, 8.578e-20,  -6.12423e-23};
	double cp = 0.;
...
double conversion_rate::characteristic_curve(const double A)
{
	...
	const double c[] = {0.34102920966608297,    -0.0013106032830951296, -0.00060754147575378876, 3.7843404172683339e-07,
	                    4.0107503869519016e-07, 3.1274595098338057e-10, -7.610441241719489e-11};
	double W = (c[0] + c[2] * A + c[4] * std::pow(A, 2) + c[6] * std::pow(A, 3))
	           / (1.0 + c[1] * A + c[3] * std::pow(A, 2) + c[5] * std::pow(A, 3)); // cm^3/g
	if (W < 0.0)
	{
...
double conversion_rate::get_entropy(const double Tads, const double A)
{
	...
	const double dAdlnW
	    = 2.0 * epsilon / (log(characteristic_curve(A + epsilon)) - log(characteristic_curve(A - epsilon)));
