...
public:
    ...
    std::vector<int>
        sp_varind;  // store node_value_idx+timelevel of all species
    std::vector<bool> dried_out_nodes;  // for eclipse coupling

    int nodenumber;
    std::vector<std::string> pcs_rename0_init, pcs_rename0_pre,
        pcs_rename0_post;
    std::vector<int> pcs_rename0_idx_init, pcs_rename0_idx_pre,
        pcs_rename0_idx_post;
    std::vector<std::vector<std::string> > pcs_rename1_init, pcs_rename1_pre,
        pcs_rename1_post;
    std::vector<std::vector<int> > pcs_rename1_idx_init, pcs_rename1_idx_pre,
        pcs_rename1_idx_post;
    std::vector<std::vector<double> > pcs_rename1_stoi_init,
        pcs_rename1_stoi_pre, pcs_rename1_stoi_post;
    ...
    void CalcUnitConversionFactors(long index, double* fl, double* fs,
                                   bool molal);
...
extern void VLE_CalcNewPressure(double T, double& P, double& V_gas,
                                double& V_liquid, double CO2_gas_old,
                                double CO2_gas_new, double CO2_liquid_old,
                                double CO2_liquid_new, double H2O_liquid_old,
                                double H2O_liquid_new, double& rho);
extern void VLE_isobaric_fixphase(double T, double P, Phase_Properties2& vapor,
                                  Phase_Properties2& liquid,
                                  Phase_Properties2& solid, int f);
extern void VLE_isochoric_fixphase(double T, double& P,
                                   Phase_Properties2& vapor,
                                   Phase_Properties2& liquid,
                                   Phase_Properties2& solid, int f);

extern void VLE_isobaric_transphase_H2(double T, double P,
                                       Phase_Properties2& vapor,
                                       Phase_Properties2& liquid);
extern void VLE_isochoric_transphase_H2(double T, double P,
                                        Phase_Properties2& vapor,
                                        Phase_Properties2& liquid);

extern void VLE_isobaric_fixphase_H2(double T, double P,
                                     Phase_Properties2& vapor,
                                     Phase_Properties2& liquid);
extern void VLE_isochoric_fixphase_H2(double T, double P,
                                      Phase_Properties2& vapor,
                                      Phase_Properties2& liquid);
