		...
		while ((abort == false) && (in_file.eof() == false))
		{
			...
			else
			{
				...
				if (this->SplittedString.size() > 0)
				{
					...
					{
						error = true;
						cout << "The number of columns in the textfile is not constant!"
						     << "\n";
						// system("Pause");
					}
					else
...
bool CDUMUXData::MakeNodeVector(void)
{
	...
	if (this->NodeData.size() < 1)
		...
		for (unsigned long i = 0; i < m_msh->nod_vector.size(); i++)
		{
			...
			//			m_NodeData = new CPointData_DuMux;
			PointDuMux* node_data(new PointDuMux(m_msh->nod_vector[i]->getData(),
			                                     -1.0E+99, // temperature
...
/*-------------------------------------------------------------------------
   ...
   Modification:
   -------------------------------------------------------------------------*/
int CDUMUXData::WriteInputForDuMux(CRFProcess* m_pcs, string Folder, long Timestep)
{
	...
	if (m_pcs->Tim->step_current > 1)
		// tempstring +=  " Restart";
		tempstring += " " + m_pcs->simulator_path + "_time=" + this->AddZero(this->TotalSimulationTime, 3, false)
		              + "_rank=00000.drs";
	...
	if (m_pcs->Tim->step_current > 1)
	{
		...
		indexConcentration_DIC = pcs_vector[this->ProcessIndex_CO2inLiquid]->GetNodeValueIndex(
		...
		for (unsigned long i = 0; i < m_msh->nod_vector.size(); i++)
		{
			...
			//			value = pcs_vector[this->ProcessIndex_CO2inLiquid]->GetNodeValue(i, indexConcentration_DIC) *
			//(this->Molweight_CO2 / 1000) / this->NodeData[i]->phase_density[0];
			value = pcs_vector[this->ProcessIndex_CO2inLiquid]->GetNodeValue(i, indexConcentration_DIC)
			        * (this->Molweight_CO2 / 1000) / this->NodeData[i]->getPhaseDensity()[0];
			// cout << i << " X_CO2 " << value << " C_CO2 " <<
			// pcs_vector[this->ProcessIndex_CO2inLiquid]->GetNodeValue(i, indexConcentration_DIC) << " Dichte " <<
			// this->NodeData[i]->phase_density[0] << " X_CO2_alt " << this->NodeData[i]->CO2inLiquid << "\n";
			temp.str("");
			temp.clear();
			temp << value;
			tempstring += " " + temp.str();
	...
	tempstring = "Knoten; X; Y; Z; P1; P2; S1; S2; rho1; rho2; xCO2_liq; mCO2_gas; mCO2_liq";
	...
	for (size_t i = 0; i < this->NodeData.size(); i++)
	{
		...
		tempstring = temp.str();
		// TF commented out since we want to use the improved PointDuMux class
		//		temp.str(""); temp.clear(); temp << this->NodeData[i]->x; tempstring += "; " + temp.str();
		//		temp.str(""); temp.clear(); temp << this->NodeData[i]->y; tempstring += "; " + temp.str();
		//		temp.str(""); temp.clear(); temp << this->NodeData[i]->z; tempstring += "; " + temp.str();
		...
		// TF commented out since we want to use the improved PointDuMux class
		//		temp.str(""); temp.clear(); temp.precision(12); temp << this->NodeData[i]->phase_pressure[0]; tempstring
		//+= "; " + temp.str();
		//		temp.str(""); temp.clear(); temp.precision(12); temp << this->NodeData[i]->phase_pressure[1]; tempstring
		//+= "; " + temp.str();
		//		temp.str(""); temp.clear(); temp.precision(12); temp << this->NodeData[i]->phase_saturation[0];
		// tempstring
		//+= "; " + temp.str();
		//		temp.str(""); temp.clear(); temp.precision(12); temp << this->NodeData[i]->phase_saturation[1];
		// tempstring
		//+= "; " + temp.str();
		//		temp.str(""); temp.clear(); temp.precision(12); temp << this->NodeData[i]->phase_density[0]; tempstring
		//+=
		//"; " + temp.str();
		//		temp.str(""); temp.clear(); temp.precision(12); temp << this->NodeData[i]->phase_density[1]; tempstring
		//+=
		//"; " + temp.str();
		//		temp.str(""); temp.clear(); temp.precision(12); temp << this->NodeData[i]->CO2inLiquid; tempstring += ";
		//"
		//+ temp.str();
		temp.str("");
		temp.clear();
		...
		concentration_CO2_water = pcs_vector[this->ProcessIndex_CO2inLiquid]->GetNodeValue(i, indexConcentration_DIC);
		// TF commented out since we want to use the improved PointDuMux class
		//		mass_CO2_gas = node_volume * this->NodeData[i]->phase_saturation[1] *
		// this->NodeData[i]->phase_density[1];
		//		mass_CO2_water = node_volume * this->NodeData[i]->phase_saturation[0] * concentration_CO2_water *
		// this->Molweight_CO2 * 0.001;
		mass_CO2_gas
		    = node_volume * this->NodeData[i]->getPhaseSaturation()[1] * this->NodeData[i]->getPhaseDensity()[1];
		mass_CO2_water = node_volume * this->NodeData[i]->getPhaseSaturation()[0] * concentration_CO2_water
		                 * this->Molweight_CO2 * 0.001;
...
/*-------------------------------------------------------------------------
   ...
   Task: Read the data of the current time step of the DuMux model from output file
   Return: nothing
   Programming: 09/2010 BG
   Modification:
   -------------------------------------------------------------------------*/
void CDUMUXData::ReadDuMuxData(CRFProcess* m_pcs, string Filename, long Timestep)
{
	...
	Filename = Filename.replace(Filename.length() - 8, 4, this->AddZero(Timestep - 1, 4, true));
	...
	// index    x  y  z  p1   p2    S1  S2  qx1  qy1  qz1  qx2  qy2  qz2 X_CO2inBrine X_NaClInBrine  T rho_liquid
	// rho_gas
	// check the header of the text file
	if (this->Phases.size() == 1)
		if ((TextFile->Header[0] != "index") || (TextFile->Header[1] != "x") || (TextFile->Header[2] != "y")
		    || (TextFile->Header[3] != "z")
		    || (TextFile->Header[4] != "p")
		    || (TextFile->Header[5] != "qx")
		    || (TextFile->Header[6] != "qy")
		    || (TextFile->Header[7] != "qz"))
		{
			cout << "The header of the DUMUX result file does not fit to the definition!"
			     << "\n";
			// system("Pause");
			exit(0);
		}
	if (this->Phases.size() == 2)
		if ((TextFile->Header[0] != "index") || (TextFile->Header[1] != "x") || (TextFile->Header[2] != "y")
		    || (TextFile->Header[3] != "z")
		    || (TextFile->Header[4] != "p1")
		    || (TextFile->Header[5] != "p2")
		    || (TextFile->Header[6] != "S1")
		    || (TextFile->Header[7] != "S2")
		    || (TextFile->Header[8] != "qx1")
		    || (TextFile->Header[9] != "qy1")
	...
	// Create the nodeData structure and check the coordinates of the nodes between OGS and DUMUX
	if (Timestep == 1)
	{
		this->MakeNodeVector();
		...
		{
			cout << "The number of nodes is not equal between OGS and DUMUX! " << this->NodeData.size() << ", "
		...
		for (long i = 0; i < TextFile->NumberOfRows; i++)
			// TF commented out since we want to use the improved PointDuMux class
			//			if ((atof(TextFile->Data_separated[i][1].data()) != this->NodeData[i]->x) ||
			//(atof(TextFile->Data_separated[i][2].data()) != this->NodeData[i]->y) ||
			//(atof(TextFile->Data_separated[i][3].data()) != this->NodeData[i]->z)) {
			if ((atof(TextFile->Data_separated[i][1].data()) != (*(this->NodeData[i]))[0])
			    || (atof(TextFile->Data_separated[i][2].data()) != (*(this->NodeData[i]))[1])
			    || (atof(TextFile->Data_separated[i][3].data()) != (*(this->NodeData[i]))[2]))
			{
				cout << "The node coordinates are not equal between OGS and DUMUX!"
				     << "\n";
				// system("Pause");
				exit(0);
			}
	...
	for (long i = 0; i < TextFile->NumberOfRows; i++)
	{
		...
		for (int j = 0; j < int(this->Phases.size()); j++)
		{
			column += 1;
			// TF commented out since we want to use the improved PointDuMux class
			//			this->NodeData[i]->phase_pressure[j] = atof(TextFile->Data_separated[i][column].data()) *
			// Multiplier;
			this->NodeData[i]->getPhasePressure()[j] = atof(TextFile->Data_separated[i][column].data()) * Multiplier;
		}
		if (this->Phases.size() > 1)
			for (int j = 0; j < int(this->Phases.size()); j++)
			{
		...
		for (int j = 0; j < int(this->Phases.size()); j++)
			for (int k = 0; k < 3; k++)
			{
				column += 1;
				// TF commented out since we want to use the improved PointDuMux class
				//				this->NodeData[i]->q[j][k] = atof(TextFile->Data_separated[i][column].data()) *
				// Multiplier;
				this->NodeData[i]->getQ()[j][k] = atof(TextFile->Data_separated[i][column].data()) * Multiplier;
			}
		column += 1;
		// TF commented out since we want to use the improved PointDuMux class
		//		this->NodeData[i]->CO2inLiquid = atof(TextFile->Data_separated[i][column].data()) * Multiplier;
		this->NodeData[i]->setCO2InLiquid(atof(TextFile->Data_separated[i][column].data()) * Multiplier);
		// cout << i << " " << this->NodeData[i]->CO2inLiquid << "\n";
		column += 1;
		// TF commented out since we want to use the improved PointDuMux class
		//		this->NodeData[i]->NaClinLiquid = atof(TextFile->Data_separated[i][column].data()) * Multiplier;
		this->NodeData[i]->setNaClInLiquid(atof(TextFile->Data_separated[i][column].data()) * Multiplier);
		column += 1;
		// TF commented out since we want to use the improved PointDuMux class
		//		this->NodeData[i]->temperature = atof(TextFile->Data_separated[i][column].data()) * Multiplier;
		this->NodeData[i]->setTemperature(atof(TextFile->Data_separated[i][column].data()) * Multiplier);
		column += 1;
		// TF commented out since we want to use the improved PointDuMux class
		//		this->NodeData[i]->phase_density[0] = atof(TextFile->Data_separated[i][column].data()) * Multiplier;
		this->NodeData[i]->getPhaseDensity()[0] = atof(TextFile->Data_separated[i][column].data()) * Multiplier;
		column += 1;
		// TF commented out since we want to use the improved PointDuMux class
		//		this->NodeData[i]->phase_density[1] = atof(TextFile->Data_separated[i][column].data()) * Multiplier;
		this->NodeData[i]->getPhaseDensity()[1] = atof(TextFile->Data_separated[i][column].data()) * Multiplier;
	}
	...
	for (size_t i = 0; i < this->NodeData.size(); i++)
	{
		...
		// TF commented out since we want to use the improved PointDuMux class
		//		concentration_CO2_water = this->NodeData[i]->CO2inLiquid * this->NodeData[i]->phase_density[0] /
		//(this->Molweight_CO2 * 1e-3);
		//		mass_CO2_gas = node_volume * this->NodeData[i]->phase_saturation[1] *
		// this->NodeData[i]->phase_density[1];
		//		mass_CO2_water = node_volume * this->NodeData[i]->phase_saturation[0] * concentration_CO2_water *
		// this->Molweight_CO2 * 0.001;
		concentration_CO2_water = this->NodeData[i]->getCO2InLiquid() * this->NodeData[i]->getPhaseDensity()[0]
		                          / (this->Molweight_CO2 * 1e-3);
		mass_CO2_gas
		    = node_volume * this->NodeData[i]->getPhaseSaturation()[1] * this->NodeData[i]->getPhaseDensity()[1];
		mass_CO2_water = node_volume * this->NodeData[i]->getPhaseSaturation()[0] * concentration_CO2_water
...
void CDUMUXData::WriteDataToGeoSys(CRFProcess* m_pcs)
{
	...
	for (unsigned long i = 0; i < m_msh->nod_vector.size(); i++)
	{
		...
		index = m_pcs->GetNodeValueIndex("PRESSURE1") + 1; //+1... new time level
		// TF commented out since we want to use the improved PointDuMux class
		//		value = this->NodeData[i]->getPhasePressure()[1] - this->NodeData[i]->getPhasePressure()[0];
		value = pnt_dumux->getPhasePressure()[1] - pnt_dumux->getPhasePressure()[0];
		...
		if (this->Phases.size() == 2)
		{
			...
			index = m_pcs->GetNodeValueIndex("PRESSURE2") + 1; //+1... new time level
			// TF commented out since we want to use the improved PointDuMux class
			//			value = this->NodeData[i]->phase_pressure[1];
			value = pnt_dumux->getPhasePressure()[1];
			m_pcs->SetNodeValue(i, index, value);
			// saturation of wetting phase
			index = m_pcs->GetNodeValueIndex("SATURATION1") + 1; //+1... new time level
			// TF commented out since we want to use the improved PointDuMux class
			//			value = this->NodeData[i]->phase_saturation[0];
			value = pnt_dumux->getPhaseSaturation()[0];
			m_pcs->SetNodeValue(i, index, value);
			// saturation of nonwetting phase
			// index = m_pcs->GetNodeValueIndex("SATURATION2") + 1; //+1... new time level
			// value = this->NodeData[i]->phase_saturation[1];
			// m_pcs->SetNodeValue(i,index,value);
			// transfer of velocities to OGS nodes
			index = m_pcs->GetNodeValueIndex("VELOCITY_X2"); //+1... new time level
			// TF commented out since we want to use the improved PointDuMux class
			//			value = this->NodeData[i]->q[1][0];
			value = pnt_dumux->getQ()[1][0];
			m_pcs->SetNodeValue(i, index, value);
			// transfer of velocities to OGS nodes
			index = m_pcs->GetNodeValueIndex("VELOCITY_Y2"); //+1... new time level
			// TF commented out since we want to use the improved PointDuMux class
			//			value = this->NodeData[i]->q[1][1];
			value = pnt_dumux->getQ()[1][1];
			m_pcs->SetNodeValue(i, index, value);
			// transfer of velocities to OGS nodes
			index = m_pcs->GetNodeValueIndex("VELOCITY_Z2"); //+1... new time level
			// TF commented out since we want to use the improved PointDuMux class
			//			value = this->NodeData[i]->q[1][2];
			value = pnt_dumux->getQ()[1][2];
			m_pcs->SetNodeValue(i, index, value);
			index = m_pcs->GetNodeValueIndex("DENSITY2"); //+1... new time level
			// TF commented out since we want to use the improved PointDuMux class
			//			value = this->NodeData[i]->phase_density[1];
	...
	if (this->ProcessIndex_CO2inLiquid == -1)
		for (int i = 0; i < int(pcs_vector.size()); i++)
		{
			...
			//    (n_pcs->nod_val_name_vector[0] == "CO2_w"))                          // "C(4)"...Phreeqc,
			//    "CO2_w"...Chemapp
	...
	if (this->ProcessIndex_CO2inLiquid == -1)
	{
		cout << "In the model exists dissolved gas but there is no dissolved C in water defined. Please ad the mass "
	...
	for (unsigned long i = 0; i < pcs_vector[this->ProcessIndex_CO2inLiquid]->nod_val_vector.size(); i++)
	{
		// TF abbreviation
		PointDuMux const* const pnt_dumux(this->NodeData[i]);
		// recalculate dissolve gas: c_CO2 [mol/m�] = w_CO2 [kg_CO2 / kg_liquid] * density_liq[kg/m�] / (Molweight_CO2
		// [g/mol] * 1e-3 [kg/g])
		// TF commented out since we want to use the improved PointDuMux class
		//		value = this->NodeData[i]->CO2inLiquid * this->NodeData[i]->phase_density[0] / (this->Molweight_CO2 *
		// 1e-3);
		value = pnt_dumux->getCO2InLiquid() * pnt_dumux->getPhaseDensity()[0] / (this->Molweight_CO2 * 1e-3);
		...
		{
			// TF commented out since we want to use the improved PointDuMux class
			//			cout << "Node: " << i << " Druck: " << this->NodeData[i]->phase_pressure[1] << " X_CO2: " <<
			// this->NodeData[i]->CO2inLiquid << " Dichte: " << this->NodeData[i]->phase_density[0] << " C(4): " <<
			// value
			//<< "\n";
			cout << "Node: " << i << " Druck: " << pnt_dumux->getPhasePressure()[1]
			     << " X_CO2: " << this->NodeData[i]->getCO2InLiquid() << " Dichte: " << pnt_dumux->getPhaseDensity()[0]
			     << " dissolved CO2: " << value << "\n";
	...
	for (int k = 0; k < int(this->Phases.size()); k++)
	{
		if (k == 0)
		{
			i_ind[0] = m_pcs->GetNodeValueIndex("VELOCITY_X1"); // get index of velocity
			i_ind[1] = m_pcs->GetNodeValueIndex("VELOCITY_Y1"); // get index of velocity
			i_ind[2] = m_pcs->GetNodeValueIndex("VELOCITY_Z1"); // get index of velocity

			////  check all possibilities for grid orientation (ccord_flag)
			// int coordinateflag = this->m_msh->GetCoordinateFlag();
			// i_ind[0] = m_pcs_fm->GetNodeValueIndex("VELOCITY_X1")+1; // get index of velocity
			// if(coordinateflag == 11)
			// i_ind[0] = m_pcs_fm->GetNodeValueIndex("VELOCITY_Y1")+1; // get index of velocity
			// if(coordinateflag == 12)
			// i_ind[0] = m_pcs_fm->GetNodeValueIndex("VELOCITY_Z1")+1; // get index of velocity
			// i_ind[1] = m_pcs_fm->GetNodeValueIndex("VELOCITY_Y1")+1; // get index of velocity
			// if(coordinateflag == 22)
			// i_ind[1] = m_pcs_fm->GetNodeValueIndex("VELOCITY_Z1")+1; // get index of velocity
			// i_ind[2] = m_pcs_fm->GetNodeValueIndex("VELOCITY_Z1")+1; // get index of velocity
		}
		else
		{
			i_ind[0] = m_pcs->GetNodeValueIndex("VELOCITY_X2"); // get index of velocity
			i_ind[1] = m_pcs->GetNodeValueIndex("VELOCITY_Y2"); // get index of velocity
			i_ind[2] = m_pcs->GetNodeValueIndex("VELOCITY_Z2"); // get index of velocity

			//  check all possibilities for grid orientation (ccord_flag)
			// int coordinateflag = this->m_msh->GetCoordinateFlag();
			// i_ind[0] = m_pcs_fm->GetNodeValueIndex("VELOCITY_X2")+1; // get index of velocity
			// if(coordinateflag == 11)
			// i_ind[0] = m_pcs_fm->GetNodeValueIndex("VELOCITY_Y2")+1; // get index of velocity
			// if(coordinateflag == 12)
			// i_ind[0] = m_pcs_fm->GetNodeValueIndex("VELOCITY_Z2")+1; // get index of velocity
			// i_ind[1] = m_pcs_fm->GetNodeValueIndex("VELOCITY_Y2")+1; // get index of velocity
			// if(coordinateflag == 22)
			// i_ind[1] = m_pcs_fm->GetNodeValueIndex("VELOCITY_Z2")+1; // get index of velocity
		...
		// string tempstring;
		// ostringstream temp;
		// vec_string.push_back("Element; X; Y; Z; v_Geosys_x; v_Geosys_y; v_Geosys_z; v_DuMux_x; v_DuMux_y;
		// v_DuMux_z");
		...
		for (long i = 0; i < (long)m_msh->ele_vector.size(); i++)
		{
			...
			{ // Configure Element for interpolation of node velocities to GP velocities
				...
				tempstring = tempstring + fem->Cal_GP_Velocity_DuMux(i_ind, m_pcs, k);
		...
		//	path = path.substr(0,position);
		//	temp.str(""); temp.clear(); temp << timestep; tempstring = temp.str();
		//	string aus_file = path + "\\CheckVelocity_" + Phases[k] + "_" + tempstring + ".csv";
		//	ofstream aus;
...
void CDUMUXData::ExecuteDuMux(CRFProcess* m_pcs, string folder)
{
	...
	if (UsePrecalculatedFiles == false)
		...
		{
			tempstring = DuMuxExe + " " + folder + "DuMux.dgf" + " " + folder + "dataForDumux.dat";
			cout << tempstring << "\n";
...
int CDUMUXData::RunDuMux(long Timestep, CRFProcess* m_pcs)
{
	...
	Executable_Filename = m_pcs->simulator_model_path; // path to eclipse input data
	if (Timestep == 1)
	{
	...
	{
		cout << "There are problems with writing input data for DuMux! The run is terminated."
		     << "\n";
		// system("Pause");
		exit(0);
	}
	...
	if (Timestep == 1)
	{
		...
		{
			cout << "Warning: The DuMux simulator model path is not part of the GeoSys model path!!!"
			     << "\n";
			cout << root_folder << "\n";
			cout << geosys_folder << "\n";
			// system("Pause");
	...
	if (ReadPrecalculatedFiles == false)
	{
		do
		{
			...
			number_loops += 1;
			Filename = folder + "dataForGeoSys" + AddZero(m_pcs->Tim->step_current - 1, 4, true) + ".dat";
		} while ((CheckIfFileExists(Filename) == false) && (number_loops <= maximum_loops));
	}
...
//	- in st-file: Masse einlesen und mit aktueller Dichte in Volumenstrom umrechnen
