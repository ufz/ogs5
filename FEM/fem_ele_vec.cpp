...
CFiniteElementVec::CFiniteElementVec(process::CRFProcessDeformation* dm_pcs,
                                     ...
                                     const int order)
	: CElement(C_Sys_Flad,
	           order), pcs(dm_pcs), PressureC(NULL), PressureC_S(NULL), PressureC_S_dp(NULL),
	  b_rhs(NULL)
{
	int i;
...
void CFiniteElementVec::LocalAssembly(const int update)
{
	...
	if(dynamic)
	{
		...
		for(size_t i = 0; i < dim; i++)
			for(j = 0; j < nnodesHQ; j++)
			{
				...
				(*dAcceleration)(i * nnodesHQ + j) = pcs->GetNodeValue(nodes[j],
				                                                       Idx_dm0[i]);
				// Increment of displacement
				// du = v_n*dt+0.5*a_n*dt*dt+0.5*beta2*da*dt*dt
				// a_n = a_{n+1}-da
				Disp[j + i *
				     nnodesHQ] =  pcs->GetNodeValue(nodes[j],Idx_Vel[i]) * dt \
				                 + 0.5 * dt * dt *
	...
	if(Flow_Type > 0 && Flow_Type != 10)
	{
		...
		if((Flow_Type == 1 ||
		    Flow_Type == 2) &&
		   (smat->SwellingPressureType == 3 || smat->SwellingPressureType == 4))
		{
			double fac = 1.0;
			if(Flow_Type == 1)
				fac = -1.0;
...
void CFiniteElementVec::LocalAssembly_continuum(const int update)
{
	...
	if(PoroModel == 4)
		...
		deporo =
		        h_pcs->GetElementValue(Index,
		                               h_pcs->GetElementValueIndex("n_sw_rate")) /
		        (double)ele_dim;
	if(T_Flag)
		for (i = 0; i < nnodes; i++)
		{
	...
	for (gp = 0; gp < nGaussPoints; gp++)
	{
		...
		case 11:                  //WX: 08.2010
		{
			...
			switch(smat->DirectStressIntegrationDPwithTension(gp, De, eleV_DM, dstress,
			                                                  update, mm))
			{
			case 1:
			{
		...
		case 3:                   // Generalized Cam-Clay model
			...
			if(smat->SwellingPressureType == 3)
			{
				...
				smat->CalStress_and_TangentialMatrix_CC_SubStep(gp,
				                                                eleV_DM,
				                                                dstress,
				                                                ConsistDep,
				                                                update);
			 ...
			 */
				...
				smat->CalStress_and_TangentialMatrix_CC(gp,
				                                        eleV_DM,
				                                        dstress,
				                                        ConsistDep,
				                                        update);
	...
	if(ep > 0.0)                          // in yield status
	{
		if(!eleV_DM->Localized)
		{
			...
			h_loc = pr_stress[2] / normXi + 0.5 * (smat->Al + smat->Xi)
			        - 0.5 *
			        sqrt(2.0 / (1.0 - smat->Poisson_Ratio())) * (smat->Al - smat->Xi);
			detA = 1.0e8;
					...
					for(i = 0; i < 2; i++)
					{
						...
						for(j = 0; j < 2; j++)
							Aac[i * 2 +
							    j] +=
							        (smat->K + smat->G /
							         3.0) * n_jump[i] * n_jump[j]
							        - (4.5 * smat->Al *
							           smat->Xi * smat->K * smat->K *
							           n_jump[i] *
							           n_jump[j] + 3.0 * smat->G *
							           smat->K
							           * (smat->Al * Nj[i] *
						...
						for (size_t jj = 0; jj < ele_dim; jj++)
							f_j +=
							        ...
							        (*DtD)(ii,
							               jj) *
							        (*PDB)(ele_dim * j + jj, l);
					(*Stiffness)(i + k * nnodesHQ, l * nnodesHQ +
					             j) -= f_j / Jac_e;
				}
		}
