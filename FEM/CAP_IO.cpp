...
bool CAP_check_file(void)
{
	...
	for (i = 0; i < (int)ext_name0.size(); i++)
	{
		...
		in_file_old
		    = locfilepath + getPathSepatator() + "_CHEM_STATE" + getPathSepatator() + proj_name + "." + ext_name0[i];
	...
	for (i = 0; i < (int)ext_name.size(); i++)
	{
		...
		in_file_old
		    = locfilepath + getPathSepatator() + "_CHEM_STATE" + getPathSepatator() + proj_name + "." + ext_name[i];
...
int OGS_keyword_check(std::string in_file_new, std::string in_file_old, std::string ext_name)
{
	...
	vector<string> key_new, key_old, subkey_new, subkey_old, line_new, line_old, pies;
	...
	if (in_new.good() && in_old.good())
	{
		...
		for (i = 0; i < (int)key_new.size(); i++)
			cout << ext_name << " file: Line. " << key_new[i] << "   -> not consistent !" << endl;
		for (i = 0; i < (int)key_old.size(); i++)
			cout << ext_name << " file: Line. " << key_old[i] << "   -> changed !" << endl;
		for (i = 0; i < (int)subkey_new.size(); i++)
			cout << ext_name << " file: Line. " << subkey_new[i] << "   -> not consistent !" << endl;
		for (i = 0; i < (int)subkey_old.size(); i++)
			cout << ext_name << " file: Line. " << subkey_old[i] << "   -> changed !" << endl;
		for (i = 0; i < (int)line_new.size(); i++)
			cout << ext_name << " file: Line. " << line_new[i] << "   -> not consistent !" << endl;
		for (i = 0; i < (int)line_old.size(); i++)
			cout << ext_name << " file: Line. " << line_old[i] << "   -> changed !" << endl;
	}
	else
	{
		if (!in_new.good() && in_old.good())
...
void read_file(void)
{
	...
	if (in.good())
	{
		while (1)
		{
			...
			std::cout << "CHEM_STATE pushback: size: " << CHEM_STATE.size() << " CAP_Time: " << CAP_Time
			          << " CaP_Node: " << CAP_Node << " CAP_icount: " << CAP_icount << " f:" << file_name << "\n";
		}
	}
	in.close();
	file_name += "_AP";
		...
		else
		{
			for (i = 0; i < (int)CHEM_STATE[0].size(); i++)
				if (OPTION == CHEM_STATE[0][i])
					break;
			if (INDEX > 0)
				VAL[0] = atof(CHEM_STATE[PHASE_CONSTI_MAP[INDEXP][INDEX - 1]][i].c_str());
			else if (INDEX == 0)
				VAL[0] = atof(CHEM_STATE[PHASE_CONSTI_MAP[INDEXP][INDEX]][i].c_str());
		}
		return 0;
	}
	else
	...
	else if (CAP_MODE == 2) // file mode, without real chemapp
	{
		for (i = 0; i < (int)CHEM_STATE[0].size(); i++)
			if (OPTION == CHEM_STATE[0][i])
				break;
		if (INDEXC > 0)
			VAL[0] = atof(CHEM_STATE[PHASE_CONSTI_MAP[INDEXP][INDEXC - 1]][i].c_str());
		else if (INDEXC == 0)
			VAL[0] = atof(CHEM_STATE[PHASE_CONSTI_MAP[INDEXP][INDEXC]][i].c_str());
		return 0;
	}
	else
		return -1; // CB should not occur
...
int CAP_tqinpc(CHP NAME, LI INDEXP, LIP INDEXC, LIP NOERR)
{
	...
	else
	{
		for (i = 0; i < CHEM_STATE.size(); i++)
		{
			if (strcmp(NAME, CHEM_STATE[i][2].c_str()) == 0 && INDEXP == atoi(CHEM_STATE[i][0].c_str()))
