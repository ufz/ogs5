...
LinkedTriangle::LinkedTriangle(std::vector<GEOLIB::Point*> const& pnt_vec,
                               size_t pnt_a, size_t pnt_b, size_t pnt_c,
                               LinkedTriangle* tri_a, LinkedTriangle* tri_b,
                               LinkedTriangle* tri_c)
    : GEOLIB::Triangle(pnt_vec, pnt_a, pnt_b, pnt_c)
{
    if (tri_a)
        _neighbor_triangles[0] = tri_a;
    else
        _neighbor_triangles[0] = NULL;
    if (tri_b)
        _neighbor_triangles[1] = tri_b;
    else
        _neighbor_triangles[1] = NULL;
    if (tri_c)
        _neighbor_triangles[2] = tri_c;
    else
        _neighbor_triangles[2] = NULL;
}
...
void LinkedTriangle::setNeighborTriangleByPointIdx(size_t idx,
                                                   LinkedTriangle* tri)
{
    if (idx == _pnt_ids[0])
...
{
    if (tri == _neighbor_triangles[0])
        return 0;
    if (tri == _neighbor_triangles[1])
        return 1;
    if (tri == _neighbor_triangles[2])
        return 2;
    return 3;
}
