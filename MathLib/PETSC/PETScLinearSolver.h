...
public:
	...
	void Config(const PetscReal tol, const PetscInt maxits, const KSPType lsol, const PCType prec_type,
	...
	void GetVecValues(const int v_type, PetscInt ni, const PetscInt ix[], PetscScalar y[]) const;
	...
	void setArrayValues(int arr_idx, PetscInt ni, const PetscInt ix[], const PetscScalar y[],
	                    InsertMode iora = ADD_VALUES);
	...
	void addMatrixEntries(const int m, const int idxm[], const int n, const int idxn[], const PetscScalar v[]);
...
private:
	...
	*/
	void gatherLocalVectors(PetscScalar local_array[], PetscScalar global_array[]);
