...
void PETScLinearSolver::MatrixCreate(PetscInt m, PetscInt n)
{
    ...
    MatSetUp(A);  // KG44 this seems to work with petsc 3.3 ..the commands below
                  // result in problems when assembling the
...
// kg44 quick fix to compile PETSC with version PETSCV3.4
    ...
    else
    {
        ...
        PetscPrintf(PETSC_COMM_WORLD,
                    "\n================================================");
        PetscPrintf(PETSC_COMM_WORLD,
                    "\nLinear solver %s with %s preconditioner", slv_type,
                    prc_type);
                                           ...
                                           PetscScalar global_array[])
{
    ...
    MPI_Allgather(&m_size_loc, 1, MPI_INT, &i_cnt[0], 1, MPI_INT,
                  PETSC_COMM_WORLD);
    ...
    MPI_Allgatherv(local_array, m_size_loc, MPI_DOUBLE, global_array, &i_cnt[0],
