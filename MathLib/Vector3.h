...
public:
	...
	TemplateVector(const GEOLIB::TemplatePoint<T>& rhs) : GEOLIB::TemplatePoint<T>(rhs[0], rhs[1], rhs[2]) {}
	...
	TemplateVector operator-() const { return TemplateVector(-this->x[0], -this->x[1], -this->x[2]); }
	...
	TemplateVector operator*(const TemplateVector& pV) const
	{
		return TemplateVector(this->_x[1] * pV[2] - this->_x[2] * pV[1],
		                      this->_x[2] * pV[0] - this->_x[0] * pV[2],
		                      this->_x[0] * pV[1] - this->_x[1] * pV[0]);
	}

	/// Cross product with another vector
	TemplateVector Cross(const TemplateVector& pV) const { return *this * pV; }
	friend double Dot(const TemplateVector& p1, const TemplateVector& p2) { return p1.Dot(p2); }
	friend TemplateVector Cross(const TemplateVector& p1, const TemplateVector& p2) { return p1 * p2; }
	...
	double LenSqr(void) const { return scpr(this->getData(), this->getData(), 3); }
	/// Returns the length
	double Length(void) const { return sqrt(LenSqr()); }
	...
	double CosAng(const TemplateVector& pV) { return this->Dot(pV) / (Length() * pV.Length()); }
	/// Comparison if equal
