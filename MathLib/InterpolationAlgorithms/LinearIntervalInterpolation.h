...
public:
     ...
     */
    LinearIntervalInterpolation(NUMERIC_TYPE a, NUMERIC_TYPE b, NUMERIC_TYPE c,
                                NUMERIC_TYPE d);
    /**
    ...
    : _m(d - c), _n(0.0)
{
    if (b == a)
    {
        throw std::runtime_error(
            "LinearIntervalInterpolation::LinearIntervalInterpolation: a == b, "
            "empty interval");
    }
    _m /= (b - a);
    _n = c - _m * a;
...
template <typename NUMERIC_TYPE>
inline NUMERIC_TYPE LinearIntervalInterpolation<NUMERIC_TYPE>::operator()(
