...
{
public:
	InverseDistanceInterpolation(const std::vector<T1> domain_points, const std::vector<T2> supporting_points);
...
InverseDistanceInterpolation<T1, T2>::InverseDistanceInterpolation(
    const std::vector<T1> domain_points, const std::vector<T2> supporting_points)
    : _interpolation_exponent(2.0), _domain_points(domain_points), _supporting_points(supporting_points),
...
template <class T1, class T2>
double InverseDistanceInterpolation<T1, T2>::getDistance(size_t pnt_idx, size_t support_idx) const
{
...
template <class T1, class T2>
double InverseDistanceInterpolation<T1, T2>::getSumOfDistances(size_t pnt_idx) const
{
...
template <class T1, class T2>
void InverseDistanceInterpolation<T1, T2>::calculateDistancesToSupportingPoints()
{
	...
	for (size_t n = 0; n < nPoints; n++)
	{
		...
		for (size_t i = 0; i < nSupportingPoints; i++) // Distance to each weather station
		{
			...
			double dist = std::max(
			    DBL_MIN,
			    sqrt(((*_supporting_points[i])[0] - x) * ((*_supporting_points[i])[0] - x)
