...
{
/**
 * Inverse distance interpolation, calculating distances from all domain points
 * to all supporting points. The template classes T1 and T2 are expecting
 * GEOLIB::Point Types.
 */
...
public:
    ...
    /// Returns the distance between point pnt_idx and supporting point
    /// support_idx.
    ...
    const T1 getDomainPoint(size_t pnt_idx) const
    {
        return _domain_points[pnt_idx];
    };
    const T2 getSupportingPoint(size_t support_idx) const
    {
        return _supporting_points[support_idx];
    };
    ...
    void setInterpolationExponent(double exponent)
    {
        _interpolation_exponent = exponent;
    };

private:
    /// Calculates the distances from all domain points to all supporting
    /// points.
...
template <class T1, class T2>
InverseDistanceInterpolation<T1, T2>::InverseDistanceInterpolation(
    const std::vector<T1> domain_points,
    const std::vector<T2>
        supporting_points)
    : _interpolation_exponent(2.0),
      ...
      _distances(
          Matrix<double>(_domain_points.size(), _supporting_points.size()))
{
...
template <class T1, class T2>
double InverseDistanceInterpolation<T1, T2>::getDistance(
    size_t pnt_idx, size_t support_idx) const
{
    if (pnt_idx < _domain_points.size() &&
        support_idx < _supporting_points.size())
        return _distances(pnt_idx, support_idx);
    return -1;
}

template <class T1, class T2>
double InverseDistanceInterpolation<T1, T2>::getSumOfDistances(
    size_t pnt_idx) const
{
    if (pnt_idx < _domain_points.size())
...
template <class T1, class T2>
void InverseDistanceInterpolation<T1,
                                  T2>::calculateDistancesToSupportingPoints()
{
    ...
    for (size_t n = 0; n < nPoints; n++)
    {
        ...
        for (size_t i = 0; i < nSupportingPoints;
             i++)  // Distance to each weather station
        {
            // ignore z-coordinate for the moment, assuming elevation is
            // ignorable compared to x-y-distance
            double dist = std::max(DBL_MIN,
                                   sqrt(((*_supporting_points[i])[0] - x) *
                                            ((*_supporting_points[i])[0] - x) +
                                        ((*_supporting_points[i])[1] - y) *
                                            ((*_supporting_points[i])[1] -
                                             y)));  // good old pythagoras
