...
bool isPointInTriangle(const double q[3], const double a[3], const double b[3], const double c[3], double eps)
{
	if (sqrt(MathLib::sqrDist(q, a)) < eps || sqrt(MathLib::sqrDist(q, b)) < eps || sqrt(MathLib::sqrDist(q, c)) < eps)
	{
		return true;
	}
	...
	double y[2] = {v[0] * (q[0] - a[0]) + v[1] * (q[1] - a[1]) + v[2] * (q[2] - a[2]),
	...
	if (lower <= y[0] && y[0] <= upper && lower <= y[1] && y[1] <= upper && y[0] + y[1] <= upper)
	{
		double const q_projected[3]
		    = {a[0] + y[0] * v[0] + y[1] * w[0], a[1] + y[0] * v[1] + y[1] * w[1], a[2] + y[0] * v[2] + y[1] * w[2]};
...
void getNewellPlane(const std::vector<GEOLIB::Point*>& pnts, Vector& plane_normal, double& d)
{
	...
	{
		plane_normal[0]
		    += ((*(pnts[i]))[1] - (*(pnts[j]))[1]) * ((*(pnts[i]))[2] + (*(pnts[j]))[2]); // projection on yz
		plane_normal[1]
		    += ((*(pnts[i]))[2] - (*(pnts[j]))[2]) * ((*(pnts[i]))[0] + (*(pnts[j]))[0]); // projection on xz
		plane_normal[2]
		    += ((*(pnts[i]))[0] - (*(pnts[j]))[0]) * ((*(pnts[i]))[1] + (*(pnts[j]))[1]); // projection on xy
...
void rotatePointsToXY(Vector& plane_normal, std::vector<GEOLIB::Point*>& pnts)
{
	...
	// sqrt (v_1^2 + v_2^2)
	double h0(sqrt(plane_normal[0] * plane_normal[0] + plane_normal[1] * plane_normal[1]));
	// 1 / sqrt (v_1^2 + v_2^2)
	double h1(1 / h0);
	// 1 / sqrt (h0 + v_3^2)
