...
template <class T>
Matrix<T>::Matrix(size_t rows, size_t cols) : nrows(rows), ncols(cols), data(new T[nrows * ncols])
{
}

template <class T>
Matrix<T>::Matrix(const Matrix& src) : nrows(src.getNRows()), ncols(src.getNCols()), data(new T[nrows * ncols])
{
...
template <class T>
void Matrix<T>::axpy(T alpha, const T* x, T beta, T* y) const
{
	for (size_t i(0); i < nrows; i++)
	{
		y[i] += beta * y[i];
...
template <class T>
T* Matrix<T>::operator*(const T* x) const
{
	T* y(new T[nrows]);
	for (size_t i(0); i < nrows; i++)
	{
...
template <class T>
Matrix<T>* Matrix<T>::operator+(const Matrix<T>& mat) const throw(std::range_error)
{
...
Matrix<T>* Matrix<T>::operator*(const Matrix<T>& mat) const throw(std::range_error)
{
	...
	if (ncols != mat.getNRows())
		throw std::range_error("Matrix::operator*, number of rows and cols should be the same!");
...
template <class T>
Matrix<T>* Matrix<T>::getSubMatrix(size_t b_row, size_t b_col, size_t e_row, size_t e_col) const throw(std::range_error)
{
...
template <class T>
void Matrix<T>::setSubMatrix(size_t b_row, size_t b_col, const Matrix<T>& sub_mat) throw(std::range_error)
{
...
template <class T>
T& Matrix<T>::operator()(size_t row, size_t col) throw(std::range_error)
{
...
template <class T>
T& Matrix<T>::operator()(size_t row, size_t col) const throw(std::range_error)
{
	if ((row >= nrows) | (col >= ncols))
		throw std::range_error("Matrix: op() const range error");
	return data[address(row, col)];
...
template <class T>
T sqrFrobNrm(const Matrix<T>& mat)
{
	T nrm((T)(0));
	size_t i, j;
