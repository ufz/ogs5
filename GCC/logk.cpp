...
double LOGK::KsNaCl(double T, double P){
	return 1.188919E+03-2.636581E+00*T-1.561700E+05/T+1.795657E-03*T*T+3.237887E-03*P+5.409459E-05*T*P+7.200665E-06*P*P-2.992790E-08*T*P*P;
...
double LOGK::lnK(double T, double P, double dV, double dk, double lnKs){
	...
	if (T>=373.15)
		...
		Ps=1.0;
	return -1.0/8.314/T*(-1.0*dV/10.0*(P-Ps)+0.5*dk/10000.0*(P-Ps)*(P-Ps))+lnKs;
}
...
double LOGK::logK_CO2(double T, double P){
	...
	Vphi = 35.663-5.960e-2*(T-273.15) +6.308e-4*(T-273.15)*(T-273.15);
	return log10(exp(lnphi)*P/exp(lnkH + Vphi*(P-Ps)/R/T)); //return eq CO2 activity in aqueous phase
}
