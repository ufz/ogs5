...
public:
    ...
    static std::vector<std::string>
        species;  // name of species in the last line of input file (eqbrm.in)
    static double T, P;
    ...
    std::vector<std::string> chemical_species,
        chemical_element;  // name and index of species and element
    ...
    double AAW;
    double* xstoi_water;  // check "H2O" in reactions, 0 no water, 1 or -1 water
                          // stoi number

    int activity_model;  // 0-9

    /* Method */
    void eqbrm(int N, int M, double** V, double* B, double** G, double* LOGK,
               double* Z, double* A, double* GAMMA, double& AW);
    void eqbrm_multiphases(int N, int M, double** V, double* B, double** G,
                           double* LOGK, double* VW, double* Z, int* P,
