...
double HKF::HKFcalcw(double T, double P, int ghs){
	...
	GG= -125005.0-HH;
	if(ghs==0)      res=GG;
	else if(ghs==1) res=HH;
	else if(ghs==2) res=SS;
	return res;
}
...
double HKF::HKFcalc(double T, double P, int ghs, std::string name, int sub, int type){
	...
	if(ghs==0) res= Ions[0].G;
	else if(ghs==1) res= Ions[0].H;
	else if(ghs==2) res= Ions[0].S;
	return res;
}
	...
	if(is_OK){
		...
		for(i=0;i<(int)slop98.size();i++){
			...
			for(j=0;j<(int)spec.size();j++){
				...
				if(name_type[j]==0 && (int)piesx.size()>0){
					if(spec[j].name0==piesx[0]) is_spec=1;
				}
				else if(name_type[j]==1 && (int)piesx.size()>1){
					if(spec[j].name0==piesx[1]) is_spec=1;
				}
				else if(name_type[j]==2 && (int)piesz.size()>0){
					if(spec[j].name0==piesz[0]) is_spec=1;
				}
				else if(name_type[j]==3 && (int)piesz.size()>1){
					if(spec[j].name0==piesz[1]) is_spec=1;
				}
				else {
					//cout << " OK load " << endl;
					for(js=0;js<(int)piesx.size();js++)
						if(spec[j].name0==piesx[js]) is_spec=1;
		...
		if((int)id_spec.size()>0){
			...
			for(i=0;i<(int)spec.size();i++){
				...
				if(cek==0) {
					cekz=0;
					cout << endl << " Warning! " << spec[i].name0 << " (" << name_type[i] << ") is NOT in the species list!" << endl;
				}
			}
			if(cekz==1)
				return 1;//OK
...
int HKF::HelgesonEquation(double T, double P, vector<SpeciesData> &spec){
	...
	if(is_aq==1){
		...
		Gc1 = -(T*log(T/Tr)-T+Tr);
		Gc2 = -(((1.0/(T-theta))-(1.0/(Tr-theta)))*(theta-T)/theta - T/theta/theta*log(Tr*(T-theta)/T/(Tr-theta)));
		Ga1 = P-Pr;
	...
	for(i=0;i<(int)spec.size();i++){
		if(spec[i].type==5){
			...
			case 4:
				...
				spec[i].G= dG -dS*(T-Tr) +a0*(T-Tr-T*log(T/Tr)) +(-1.0*c0-b0*T*Tr*Tr)*(T-Tr)*(T-Tr)/(2.0*T*Tr*Tr) +
...
double HKF::diel_water(double T, double P){ //T K, P bar
...
double HKF::gfunc(double T, double P){
	...
	if(Dw>=1.0) return 0.0;//Fig.6 & p811 Ref.(1)
		...
		if(T<155.0 || P>1000.0) return  ag*pow((1.0-Dw),bg);//Eq(24) Ref.(1) Fig.6
			...
			Tf= (T-155.0)/300.0;
			Pf= 1000.0-P;
			return ag*pow((1.0-Dw),bg)-(pow(Tf,4.8)+a1*pow(Tf,16))*(a2*pow(Pf,3.0)+a3*pow(Pf,4.0));//Eq(32) & Eq(33) Ref.(1)
		}
	}
}
