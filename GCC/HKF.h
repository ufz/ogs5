...
public:
    ...
    static int HelgesonEquation(double T, double P,
                                std::vector<SpeciesData>& spec);

    static double HKFcalc(double T, double P, int ghs, std::string name,
                          int sub, int type);
    // T K, P bar,
    ...
    static int HKF_OGS_loadparam(std::string species_name0, int& type,
                                 double& charge, double param[][4]);
    static int HKF_OGS_calc(double T, double P, double& G, double& H, double& S,
                            int type, double charge, double param[][4]);
