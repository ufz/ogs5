...
TEST(GEO, PointInPolygon)
{
    ...
    const std::vector<GEOLIB::Polyline*>* plys(
        _geo->getPolylineVec(unique_name));
    std::vector<GEOLIB::Point*>* pnts_in_polygon(
        new std::vector<GEOLIB::Point*>);
    std::vector<GEOLIB::Point*>* pnts_outside_of_polygon(
        new std::vector<GEOLIB::Point*>);
    ...
    for (size_t j(0); j < 320; j++)
    {
        ...
        {
            pnts.push_back(
                new GEOLIB::Point(0.9 + k / 100.0, -1.1 + j / 100.0, 0.0));
        }
    }
    ...
    ASSERT_EQ(static_cast<int>(pnts.size()),
              static_cast<int>(pnts_in_polygon->size() +
                               pnts_outside_of_polygon->size()));
